 ...
 ********************************************************************************/
	...
	/*Logic for Form lock*/
	// $userEntities = getEntityIds($dbh, $_SESSION['la_user_id']);
	// $entity_id = array();
	// print_r($userEntities);
	// die();
	//check this before page submit
	// isFormLockedForUser(array('dbh' => $dbh, 'form_id' => $form_id, 'entities' => $userEntities, 'entity_user_id' => $_SESSION['la_user_id']));	
	...
	//GET ALL CASCADE FORMS INSIDE FORM
		...
		element_default_value FROM ".LA_TABLE_PREFIX."form_elements  WHERE form_id = {$form_id} and element_status='1'  and element_type <> 'page_break' AND NOT element_default_value = '' AND element_type = 'casecade_form'  ORDER BY element_position"; 
...
/* CASESCADE FORM UPDATE */
		...
		$query = "SELECT * FROM `".LA_TABLE_PREFIX."form_elements` WHERE `form_id` = :form_id";
		$param = array();
		$param[':form_id'] = $form_id;
		$form_element_count = la_get_row_count($dbh, $param, $query);
