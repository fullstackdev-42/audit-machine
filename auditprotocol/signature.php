 ...
 ********************************************************************************/	
	...
	$company_id = 1461784572;//(int) $params['company_id'];
	$field_name = str_replace(array("`","'",';'), '', $params['el']);
	$signature_hash  = $params['hash'];
	
	if(empty($form_id) || empty($company_id) || empty($field_name) || empty($signature_hash)){
	...
	$query 	= "select `data_value` from `".LA_TABLE_PREFIX."form_{$form_id}` where `company_id`=? and `field_name`=?";
	$params = array($company_id,$field_name);
