    ...
    <div class="content_header">
      <div class="content_header_title">
        <div style="float: left">
          <h2><?php echo "<a class=\"breadcrumb\" href='manage_forms.php?id={$form_id}'>".$form_name.'</a>'; ?> <img src="images/icons/resultset_next.gif" /> Micro Report</h2>
          <p>Edit, share and publish your form report</p>
        </div>
        <?php if(!empty($report_widgets_array)){ ?>
        <div style="float: right">
          <div class="dropui dropuiquick dropui-icon dropui-menu dropui-pink dropui-right"> <a href="javascript:;" class="dropui-tab"> Settings </a>
        ...
        <?php } ?>
        <div style="float: right;margin-right: 5px"> <a href="add_widget.php?id=<?php echo noHTML($form_id); ?>" class="bb_button bb_small bb_green"> <span class="icon-chart" style="margin-right: 5px"></span>Add New Micro Report </a> </div>
        <div style="clear: both; height: 1px"></div>
      </div>
    </div>
    <?php la_show_message(); ?>
    <div class="content_body">
      <div id="mr_report_shared" style="display: <?php if(!empty($report_shared_link)){ echo 'block'; }else{ echo 'none'; } ?>"> Report shared at &#8674; <span id="mr_report_shared_span"><?php echo $report_shared_link; ?><span> </div>
      <?php
						if(!empty($report_widgets_array)){
							$ssl_suffix = la_get_ssl_suffix();

							//display the sortable widget list
							echo '<ul id="widget_list_sortable" data-formid="'.noHTML($form_id).'" style="clear: both;display: none">';
      ...
      <div id="report_sort_pane_apply" style="display: none">
        <input type="button" id="mr_report_sort_pane_submit" value="Save Changes" class="button_text">
        <span id="cancel_report_sort_pane_span">or <a href="#" id="report_sort_pane_cancel">Cancel</a></span> </div>
      <?php
							//display the widgets
							echo "<ul id=\"mr_report_list\">\n";
							foreach ($report_widgets_array as $value) {
								$widget_url = '/auditprotocol/widget.php?key='.$value['access_key'];
					?>
      <li id="li_<?php echo $value['chart_id']; ?>">
        ...
        <div class="report_toolbar">
          <div> <span class="chart_idnum">#<?php echo noHTML($value['chart_id']); ?></span> <a id="widgetedit_<?php echo noHTML($value['chart_id']); ?>" href="<?php echo "edit_widget.php?key={$value['access_key']}"; ?>"><span class="icon-pencil" style="margin-right: 5px;font-size: 120%"></span>Edit</a> <a id="widgetcode_<?php echo $value['chart_id']; ?>" href="<?php echo "widget_code.php?key={$value['access_key']}"; ?>" style="margin-left: 10px"><span class="icon-paste" style="margin-right: 5px;font-size: 120%"></span>Widget Code</a> <a id="widgetdelete_<?php echo noHTML($value['chart_id']); ?>" href="#" class="delete_icon"><span class="icon-remove" style="margin-right: 5px;font-size: 120%"></span>Delete</a> <a href="<?php echo "widget.php?key={$value['access_key']}"; ?>" target="_blank" class="open_icon"><span class="icon-popout" style="margin-right: 5px;font-size: 120%"></span>Open</a> </div>
        </div>
      </li>
      <?php
							}
					...
					?>
      <div id="report_manager_empty"> <img src="images/icons/arrow_up.png" />
        <h2>Report Empty.</h2>
        <h3>Add micro reports to your report by clicking the button above.</h3>
      </div>
      <?php } ?>
      ...
      <div id="dialog-delete-widget" title="Are you sure you want to delete this widget?" class="buttons" style="display: none">
        ...
        <p> This action cannot be undone.<br/>
          <strong>Only this widget will be deleted. Any related data will remain intact.</strong><br/>
          <br/>
        </p>
      </div>
      <div id="dialog-share-report" title="Are you sure you want to share this report?" class="buttons" style="display: none">
        ...
        <p> <strong>This report will be made public.<br />
          Anyone who has the link can access. No sign-in required.</strong><br/>
          <br/>
          You can unshare it later at anytime you want.<br />
          <br />
        </p>
      </div>
      <div id="dialog-unshare-report" title="Are you sure you want to unshare this report?" class="buttons" style="display: none">
      	<img src="images/navigation/005499/50x50/Notice.png">
        <p> <strong>This report will be made private.<br />
          Nobody else will be able to view it anymore.</strong><br/>
          <br/>
          You can share it again later at anytime you want.<br />
          <br />
        </p>
      </div>
      <!-- end dialog boxes --> 
