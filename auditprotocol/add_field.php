<?php
	...
	/* 1. Get new field parameters 																	   			   */
	/***************************************************************************************************************/
	$element_type  	 			= strtolower(la_sanitize($_POST['element_type']));
	$form_id					= (int) la_sanitize($_POST['form_id']);
	$element_position			= (int) la_sanitize($_POST['position']); //the position of the element within the preview page
	$element_properties_input 	= la_sanitize($_POST['field_properties']);
	...
	$holder_id					= htmlentities(strtolower(la_sanitize($_POST['holder_id'])));
	...
	switch ($element_type) {
		...
		case 'casecade_form'    : $element_title = 'Cascade Form';break;
		case 'syndication'      : $element_title = 'Syndication';break;
		case 'video_player'     : $element_title = 'Media Player';break;
	}
	/***************************************************************************************************************/	
	/* 3. Get element id for this new element												   					   */
	/***************************************************************************************************************/
	$query = "select ifnull(max(`element_id`),0) + 1 as new_element_id from ".LA_TABLE_PREFIX."form_elements where form_id = ?";
	...
	if(($action == 'click_new') || ($action == 'drag_new')){
		...
		$element_properties['default_value'] = $element_type == "syndication" ? json_encode(array('feed_url' => '', 'theme_id' => 1, 'width' => '', 'height' => 300, 'scroll_bar' => 0, 'auto_scroll' => 1, 'scroll_speed' => 3, 'scroll_direction' => 'vertical', 'no_of_rss_msg' => 3, 'datetime' => 0)) : "";
		...
		$element_properties['file_max_selection'] = 15; //allow 15 files to be selected for each field
		$element_properties['file_enable_size_limit'] = 0;
		$element_properties['file_size_max'] = 2; //each file is limited to 2MB max
		$element_properties['file_select_existing_files'] = 1; //allow selecting existing files previously uploaded by the entity
		...
		$element_properties['policymachine_code'] = '';
		$element_properties['machine_code'] = '';
		$element_properties['label_background_color'] = '';
		$element_properties['label_color'] = '';
		...
		if($element_type == 'radio' || $element_type == 'checkbox'){
			...
			$element_properties['choice_other_score']	= '';
		}
		...
		if($element_type == 'page_break'){
			...
			$element_properties['page_break_bg_color']  = '';
		}
		
		//default value for Casecade Form
		if($element_type == 'casecade_form'){
			$element_properties['guidelines'] 	= 'Cascade Form.';			
			$element_properties['cascade_form_invisible'] = 0;
		}

        //default value for Video Player
        if($element_type == 'video_player'){
            $element_properties['media_type'] = 'video';
            $element_properties['video_loop'] = 0;
            $element_properties['video_auto_play'] = 0;
            $element_properties['video_url'] = '';
            $element_properties['video_source'] = 'local';
        }

        //default value for checkbox
        if($element_type == 'checkbox'){
        	$element_properties['enhanced_checkbox'] = 1;
		}

		//default value for multiple_choice
        if($element_type == 'radio'){
        	$element_properties['enhanced_multiple_choice'] = 1;
		}

		//default value for file_upload_synced
        if($element_type == 'file'){
        	$element_properties['file_upload_synced'] = 1;
		}
	...
	$query = "INSERT INTO `".LA_TABLE_PREFIX."form_elements` ($field_list) VALUES ($field_values);"; 
	$form_element_sth = la_do_query($query,$params,$dbh);

	// print_r($field_list);
	// print_r($field_values);


	//fetch recently added field ID
	$form_element_auto_id = la_last_insert_id($dbh);
	...
	if(in_array($element_type,array('radio','checkbox','select','matrix'))){
		if(($action == 'click_new') || ($action == 'drag_new')){ //if this is creating new field
			...
			if(in_array($element_type,array('radio','checkbox','select'))){
				$query = "INSERT INTO 
									 ".LA_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option`,`option_value`,`option_is_default`,`live`)
						  	   SELECT 
						  	   		 `form_id`, '{$element_id}' as new_element_id,`option_id`,`position`,`option`,`option_value`,`option_is_default`,'2' as `live` 
						  	   	 FROM 
						  	   	 	 ".LA_TABLE_PREFIX."element_options
						  	   	WHERE 
				...
				foreach($matrix_element_id_pair as $source_element_id=>$dest_element_id){
					$query = "INSERT INTO 
										 ".LA_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option`,`option_value`,`option_is_default`,`live`)
							  	   SELECT 
							  	   		 `form_id`, '{$dest_element_id}' as new_element_id,`option_id`,`position`,`option`,`option_value`,`option_is_default`,'2' as `live` 
							  	   	 FROM 
							  	   	 	 ".LA_TABLE_PREFIX."element_options 
							  	   	WHERE 
							  	   		 form_id = ? AND element_id = ? AND (live=1 OR live=2);"; 
...
EOT;
			...
			//build the options data for la_display_matrix() function
			...
			//build the matrix_children property for la_display_matrix() function
			$i=1;
	...
	$element_properties_obj->form_id = $form_id;
	$element_properties_obj->element_note = '';
	
	$display_func = 'la_display_'.$element_type;

	unset($element_properties_obj->element_id_auto);
	unset($element_properties_obj->form_id);
	
	// print_r($element_properties_obj);
	// die();

	if($element_type == "syndication" || $element_type == "video_player"){
		$element_markup = $display_func($element_properties_obj, false, $dbh);
	}else{
	...
	if($element_type == 'matrix'){
		...
		if(($action == 'click_new') || ($action == 'drag_new')){
			...
			$element_properties['options'][$element_id]->machine_code = ''; 
			...
			$element_properties['options'][$element_id]->column_data[1]->column_score = 0;
			...
			$element_properties['options'][$element_id]->column_data[2]->column_score = 0;
			...
			$element_properties['options'][$element_id]->column_data[3]->column_score = 0;
			...
			$element_properties['options'][$element_id]->column_data[4]->column_score = 0;
			...
			$element_properties['options'][$element_id + 1]->machine_code = '';
			...
			$element_properties['options'][$element_id + 2]->machine_code = '';
			...
			$element_properties['options'][$element_id + 3]->machine_code = '';
			...
			foreach($element_options_source as $row_element_id=>$row_data){
				...
				$element_properties['options'][$element_id + $i]->machine_code = $row_data['machine_code']; 
				...
				if($i==0){ //if this is first row, populate column data
					...
					foreach ($column_data as $option_id=>$column_value){
						...
						$element_properties['options'][$element_id]->column_data[$option_id]->column_score = $column_value['column_score'];
	...
	// if( $action == 'duplicate' ) {
	// 	$element_properties['is_db_live'] 	 	= '1';
	// }

	$element_properties['note'] = 1;
	$element_properties['status_indicator'] = 1;
	$element_properties['rich_text'] = 1;
	unset($element_properties['form_id']);
	unset($element_properties['element_id_auto']);
	// $element_properties['form_element_auto_id'] = $form_element_auto_id;
	...
	if( $action == 'duplicate' && $element_properties_input['status'] == 2 && $element_properties_input['type'] == 'video_player') {
		$response_data->cloned_from = $element_properties_input['id'];
	}
