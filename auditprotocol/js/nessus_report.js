		...
		buttons: [
			{
				...
				click: function() {
					...
					if(table_index == 0) {
						var selected_rows = tables[0].rows({ selected: true }).data();
						var critical_problems = 0;
						var areas_of_concern = 0;
						var potential_problems = 0;
						$.each(selected_rows, function(index, row){
							...
							$.each(cells, function(i, cell){
								//if(cell != "") {
									...
									switch($(tables[0].columns(i).header()).attr("template-code")) {
										case "CGRC-4569-1":
											critical_problems += parseInt(cell.replace(/(\r\n|\n|\r)/gm,""));
											break;
										case "CGRC-4569-2":
											critical_problems += parseInt(cell.replace(/(\r\n|\n|\r)/gm,""));
											break;
										case "CGRC-4569-3":
											areas_of_concern += parseInt(cell.replace(/(\r\n|\n|\r)/gm,""));
											break;
										case "CGRC-4569-4":
											potential_problems += parseInt(cell.replace(/(\r\n|\n|\r)/gm,""));
											break;
										case "CGRC-4569-5":
											potential_problems += parseInt(cell.replace(/(\r\n|\n|\r)/gm,""));
											break;
							...
							data.push({"template_code": "CGRC-5220", "value": critical_problems});
							data.push({"template_code": "CGRC-5221", "value": areas_of_concern});
							data.push({"template_code": "CGRC-5222", "value": potential_problems});
						});
					...
					$.ajax({
						...
						success: function(response_data){
							...
							if(response_data.status == "ok"){
								$("#go_to_form").attr("href", "edit_entry.php?form_id=" + response_data.form_id + "&company_id=" + response_data.company_id + "&entry_id=" + response_data.entry_id);
