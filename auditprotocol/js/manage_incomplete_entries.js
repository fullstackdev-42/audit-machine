...
$(function(){
	...
	$("#dialog-confirm-entry-delete").dialog({
		...
		buttons: [{
				...
				click: function() {
					...
					$("#entries_table input[type='checkbox']:not('#col_select'):checked").each(function(index) {
						selected_entries_id[index] = this.value;
   						//alert(this.value);
					});
					...
					$.ajax({
						   ...
						   data: {
								  	...
								  	incomplete_entries: $("#entries_table").data("incomplete"),
								  	selected_manage_entries: selected_entries_id
								  },
						   cache: false,
						   global: false,
						   dataType: "json",
		...
		if(full_selection == 1){ //all entries being selected
			...
			if(selected_checkboxes.length > 0){ //some entries being selected
				...
				/*$.each(selected_entries, function(index, value) {
					...
					alert(value);
					selected_entries_id[index] = $('#entry_'+index).val();
					//alert($('companyId_'+index).val());
				});*/
				//alert(selected_entries_id.join(','));
	...
	$("#entries_table tbody td:not('.me_action')").click(function(){
		...
		window.location.href = 'view_entry.php?form_id=' + form_id + '&entry_id=' + selected_entry_id + '&incomplete=1';
	});
