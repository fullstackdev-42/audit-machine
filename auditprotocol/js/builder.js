...
function saveVideoFiles(id) {
    if(VideoPlayer && Object.keys(VideoPlayer.files).length) {
        var formData = new FormData();
        for(var key in VideoPlayer.files) {
            formData.append(key, VideoPlayer.files[key], VideoPlayer.files[key].name);
        }
        formData.append('form_id', id);
        formData.append('duplicatedDraftFields', JSON.stringify(VideoPlayer.duplicatedDraftFields));
        $('#btn-form-saved-ok, #btn-form-saved-cancel').prop('disabled', true);
        $.ajax({
            type: 'POST',
            url: 'save_video_files.php',
            ...
            complete: function (event, request, settings) {
                setAjaxDefaultParam();
                $('#btn-form-saved-ok, #btn-form-saved-cancel').prop('disabled', false);
            }
        })
    }
}
function save_form() {
    $("#bottom_bar_msg").text("Please wait ... ITAM is saving ...");
    check_synch_fields();
    $("#form_builder_sortable").not('#feed_items').sortable("refreshPositions");
    ...
    var pull_from_form = $('#bottom_bar_save_form').data('pull_from_form');
	//console.log(e.length);
    if (e.length >= 1) {
        var d = new Array();
        e.each(function(g) {
        	var field_properties = $(this).data("field_properties");
			d[g] = field_properties;
        });
    }
	//console.log(d);
	...
	//console.log(a);
    var deleteEntries = localStorage.getItem("addRemoveOrReorder");
    $.ajax({
        type: "POST",
        async: true,
        url: isFieldChanges ? "save_form.php" : "save_form_details.php",
        data: {
            form_id: b,
            fp: f,
            ep: d,
        ...
        success: function(g) {
        	...
        	$('meta#csrf-token-meta').attr('content', g.csrf_token);
			$('input#post-csrf-token').val(g.csrf_token);
		},
		complete: function (event, request, settings) {
			setAjaxDefaultParam();
		}
...
function select_form_header(b) {
    ...
    if ($("#builder_tabs").tabs("option", "active") != 2) {
        $("#builder_tabs").tabs("option", "active", 2)
    }
    ...
    if (b == "pagination_header") {
        ...
        //var i = $("#form_builder_sortable li.page_break.dblive:not(#li_lastpage)");
        var g = "";
        var h = 1;
		//var total_element = i.length;
        $("#form_builder_sortable li.page_break.dblive:not(#li_lastpage)").each(function(k) {
            var l = $(this);
			var element_class = "";
            g += '<li><label for="pagetitleinput_' + l.attr('id') + '">' + h + '.</label><input type="text" value="' + l.data("field_properties").page_title + '" autocomplete="off" class="text' + element_class + '" id="pagetitleinput_' + l.attr('id') + '" /></li>';
            h++
        });
		$("#form_builder_sortable li.page_break.synched:not(.dblive)").each(function(k) {
            var l = $(this);
			var element_class = "";
            g += '<li><label for="pagetitleinput_' + l.attr('id') + '">' + h + '.</label><input type="text" value="' + l.data("field_properties").page_title + '" autocomplete="off" class="text' + element_class + '" id="pagetitleinput_' + l.attr('id') + '" /></li>';
            h++
        });
		$("#form_builder_sortable li#li_lastpage").each(function(k) {
            var l = $(this);
			var element_class = "";
            g += '<li><label for="pagetitleinput_' + l.attr('id') + '">' + h + '.</label><input type="text" value="' + l.data("field_properties").page_title + '" autocomplete="off" class="text' + element_class + '" id="pagetitleinput_' + l.attr('id') + '" /></li>';
            h++
        ...
        if ($("#form_prop_toggle_a").data("expand") == 1) {
            ...
            $("#form_prop_toggle_img").attr("src", "images/icons/resultset_up.gif");
            $("#all_form_properties .advanced_prop").show()
        } else {
            $("#form_prop_toggle_a").text("show more options");
            $("#form_prop_toggle_img").attr("src", "images/icons/resultset_next.gif");
		...
		$("#form_upload_template").val(d.upload_template);
		var upload_template_string = $('#form_upload_template').val();
		if(upload_template_string.trim() != ""){
			$('div.template-list-div').empty();
			var tmpArr = new Array();
			var tmpHtm = '';
			if(upload_template_string.indexOf(",") != -1){
				tmpArr = upload_template_string.split(",");
			}else{
				tmpArr = [upload_template_string];
			}
			for(i=0; i<tmpArr.length; i++){
				var datastr = tmpArr[i];
				var dataarr = datastr.split('|||');
				tmpHtm += '<div style="position:relative; height:20px; padding:2px;">'
							 + dataarr[0] +
							 '<a href="javascript:void(0)" data-image-str="'+ datastr +'" data-image="'+ dataarr[1] +'" class="btn-remove">X</a>'+
						   '</div>';
			}
			$('div.template-list-div').append(tmpHtm);
		}
		/* *********** */
		if(d.enable_template == 1){
			$("#form_enable_template").prop("checked", true);
            $("#form_template_upload_parent").show();
		}else{
			$("#form_enable_template").prop("checked", false);
            $("#form_template_upload_parent").hide();
        }
        if(d.enable_template_wysiwyg == 1){
            $("#form_enable_template_wysiwyg").prop("checked", true);
            $("#form_template_wysiwyg_parent").show();
		}else{
            $("#form_enable_template_wysiwyg").prop("checked", false);
            $("#form_template_wysiwyg_parent").hide();
        }
        if(d.template_wysiwyg_id){
            $("#form_template_wysiwyg_id").val(d.template_wysiwyg_id);
		}
		...
		if (d.enable_auto_mapping == 1) {
            $("#form_enable_auto_mapping").prop("checked", true);
			$("#form_enable_auto_mapping").val(1);
        } else {
            $("#form_enable_auto_mapping").prop("checked", false);
			$("#form_enable_auto_mapping").val(0);
        }
		$("#form_for_selected_company").val(d.for_selected_company);
		$("#form_for_selected_entity").val(d.for_selected_entity);
		
        if (d.framework_type == 1) {
            $("#framework_type_group").prop("checked", true);
            $("#framework_type").prop("checked", false);
            $("#show_framework_group").show()
            $("#show_framework").hide();
            $("#form_framework_group").val(d.framework_or_group_id);
        } else if (d.framework_type == 0) {
            $("#framework_type").prop("checked", true);
            $("#framework_type_group").prop("checked", false);
            $("#show_framework").show()
            $("#show_framework_group").hide();
            $("#form_framework").val(d.framework_or_group_id);
        } else {
            $("#show_framework").hide();
            $("#show_framework_group").hide();
        }
        ...
        $.ajax({
            ...
            success: function(e) {
                if (e.status == "ok") {
					...
					$('meta#csrf-token-meta').attr('content', e.csrf_token);
					$('input#post-csrf-token').val(e.csrf_token);
                }
            },
			complete: function (event, request, settings) {
				setAjaxDefaultParam();
...
function select_element(c) {
	...
	g.data("field_properties").position = $("#" + active_element).index();
    check_synch_fields();
...
//called for each selected form element
function load_properties(o) {
	$('#prop_machine_code').show(); // matrix policy machine code will be multiple
    var c = $(o);
    var u = c.data("field_properties");
    u.element_id_auto = o;
    var elementType = u.type;
    
    if (typeof ckeditor_label === 'object' && ckeditor_label !== null) {
        ckeditor_label.destroy()
        .then( () => {
            ckeditor_label = null;
        })
        .catch( error => {
    ...
    if (typeof ckeditor_guide === 'object' && ckeditor_guide !== null) {
        ckeditor_guide.destroy()
        .then( () => {
            ckeditor_guide = null;
        })
        .catch( error => {
    ...
    if( elementType != 'section' ) {
        //always display label background and color except 'section_break'
        $("#prop_element_label_background_color, #prop_element_label_color").show();
        if( u.label_background_color != "null" )
        	$("#element_label_background_color").val(u.label_background_color);
        if( u.label_color != "null" )
        $("#element_label_color").val(u.label_color);    
    }
	/* Casecade form list */
	$("#prop_default_casecade_form").hide();
    $("#prop_cascade_form_invisible").hide();
	$('#prop_field_notes').show();
	$('#prop_status_indicator').hide();
    $('#prop_enhanced_checkbox').hide();
    $('#prop_enhanced_multiple_choice').hide();
    $('#prop_rich_text').hide();
	$('#embed_field_main_title, #embed_field_textarea_content').hide().text('');
    $('#ec_code_type_field_properties').val('');
    $('#ec_code_type_field_user_properties').val('');

    if(u.type == "checkbox"){
        $('#prop_enhanced_checkbox').show();
        $('#element_enhanced_checkbox').prop('checked', parseInt(u.enhanced_checkbox));
    }
    if(u.type == "radio"){
        $('#prop_enhanced_multiple_choice').show();
        $('#element_enhanced_multiple_choice').prop('checked', parseInt(u.enhanced_multiple_choice));
    }
    if(u.type == "matrix" || u.type == "radio" || u.type == "checkbox" || u.type == "select" || u.type == "text" || u.type == "signature" || u.type == "textarea" || u.type == "file"){
    	$('#prop_status_indicator').show();
    	$('#element_status_indicator').prop('checked', parseInt(u.status_indicator));
    }
    if(u.type == "textarea"){
    	$('#prop_rich_text').show();
    	$('#element_rich_text').prop('checked', parseInt(u.rich_text));
    }
    var a = "";
    if (u.type == "matrix") {
        a = {
	...
	//$("#element_machine_code").val(u.machine_code);
    $("#prop_access_control").show();
    $("#element_instructions").val(u.guidelines);
    $("#prop_instructions").show();    
	$('#prop_machine_code, #embed_machine_code').show();
	$('#element_machine_code').val(u.machine_code);
	$('#element_field_notes').prop('checked', parseInt(u.note));
    ...
    if (u.type == "text"){
		...
		//alert($("#element_default_value").val());
                ...
                if (u.type == "email") {
                    ...
                    if (u.type == "signature") {
                        ...
                        if (u.type == "file") {
                            ...
                            if (u.file_upload_synced == 1) {
                                $("#prop_file_upload_synced").prop("checked", true);
                            } else {
                                $("#prop_file_upload_synced").prop("checked", false);
                            }
                            if (u.file_select_existing_files == 1) {
                                $("#prop_file_select_existing_files").prop("checked", true);
                            } else {
                                $("#prop_file_select_existing_files").prop("checked", false);
                            }
                            $("#element_unique_span").hide()
                        }
						else {
                                                    ...
                                                    if (u.type == "simple_phone") {
                                                        ...
                                                        if (u.type == "date" || u.type == "europe_date") {
                                                            ...
                                                            if (u.type == "section") {
																/*-----*/
                                                                ClassicEditor
                                                                    .create(document.querySelector('#element_label'), {
                                                                        fontSize: {
                                                                            options: [
                                                                        ...
                                                                        ckeditor_label = newEditor
                                                                        ckeditor_label.model.document.on('change:data', () => {
                                                                            setTimeout(function() {
                                                                                var g = $("#" + active_element);
                                                                                var y = g.data("field_properties");
                                                                                $("div#sectionbreakh3-"+active_element).html(ckeditor_label.getData());
                                                                                y.title = ckeditor_label.getData();
                                                                            }, 10);
                                                                        });
                                                                    })
                                                                    .catch(error => {
                                                                ...
                                                                ClassicEditor
                                                                    .create(document.querySelector('#element_instructions'), {
                                                                        fontSize: {
                                                                            options: [
                                                                                9, 11, 13, 14, 15, 16, 'default', 17, 18, 19, 21
                                                                            ],
                                                                        ...
                                                                        ckeditor_guide = newEditor;
                                                                        ckeditor_guide.model.document.on('change:data', () => {
                                                                            setTimeout(function() {
                                                                                var g = $("#" + active_element);
                                                                                var y = g.data("field_properties");
                                                                                $("div#sectionbreakp-"+active_element).html(ckeditor_guide.getData());
                                                                                y.guidelines = ckeditor_guide.getData();
                                                                            }, 10);
                                                                        });
                                                                    })
                                                                    .catch(error => {
																...
																/*//////*/
																$('#prop_field_notes').hide();
																$("#prop_machine_code, #embed_machine_code").hide();
                                                                $("#prop_options").hide();
																...
																if (u.type == "casecade_form") {
                                                                    if(u.cascade_form_invisible == 1){
                                                                        $("#element_cascade_form_invisible").prop("checked", true);
                                                                    } else {
                                                                        $("#element_cascade_form_invisible").prop("checked", false);
                                                                    }
																	$("#prop_options").hide();
																	$("#prop_access_control").hide();
																	$("#prop_section_options").hide();
																	$("#prop_policymachine_code").hide();
																	$("#prop_machine_code, #embed_machine_code").hide();
																	$("#prop_element_type").hide();
																	$("#prop_instructions").hide();
																	$("#prop_default_casecade_form").show();
                                                                    $("#prop_cascade_form_invisible").show();
																	$('#prop_field_notes').hide();
																	var _ele = parseInt(u.id);
																	$('#casecade_'+_ele).text(u.guidelines);
																	get_casecade_form_list({form_id:$("#form_header").data("form_properties").id, element_id:u.id, db_live:u.is_db_live, default_value:u.default_value});
																}else{
																	if (u.type == "radio" || u.type == "checkbox" || u.type == "select") {
																		var g = "";
																		var n = 0;
																		...
																		$.each(b, function(v, w) {
																			if (v > 0) {
																				...
																				if(u.type == "radio" || u.type == "checkbox" || u.type == "select"){
																					if(isNaN(parseInt(u.options[w].option_value))){
																						var ov = "";
																					}else{
																						var ov = u.options[w].option_value;
																					}
                                                                                    if(u.type == "radio" || u.type == "checkbox") {
                                                                                        var ois = u.options[w].option_icon_src;
                                                                                        var icon_img = "";
                                                                                        if (!ois) {
                                                                                            ois = "";
                                                                                        } else {
                                                                                            icon_img = "<img class='uploaded-icon-img' src='" + ois + "' alt='Icon' width='20' height='20'>";
                                                                                        }

                                                                                        j = '<li><input type="' + k + '" ' + t + ' title="Select this choice as the default." class="choices_default" name="choices_default" id="choicedef_' + f + '" /><input type="text" value="' + g.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="choice_' + f + '" style="width:150px;" /> <input type="text" value="' + ov + '" autocomplete="off" class="choice-value" id="choice_value_' + f + '" style="width:35px;" maxlength="5" placeholder="Score" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="choiceadd_' + f + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="choicedel_' + f + '"><input type="button" id="choice_upload_' + f + '" class="upload-icon" value="Upload Icon" />' + icon_img + '</li>';
                                                                                    } else {
                                                                                        j = '<li><input type="' + k + '" ' + t + ' title="Select this choice as the default." class="choices_default" name="choices_default" id="choicedef_' + f + '" /><input type="text" value="' + g.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="choice_' + f + '" style="width:150px;" /> <input type="text" value="' + ov + '" autocomplete="off" class="choice-value" id="choice_value_' + f + '" style="width:35px;" maxlength="5" placeholder="Score" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="choiceadd_' + f + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="choicedel_' + f + '"></li>';
                                                                                    }
																				}else{
																					j = '<li><input type="' + k + '" ' + t + ' title="Select this choice as the default." class="choices_default" name="choices_default" id="choicedef_' + f + '" /><input type="text" value="' + g.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="choice_' + f + '" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="choiceadd_' + f + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="choicedel_' + f + '"></li>';
																				}
																				e += j
																			}
																		});
                                                                        ...
                                                                        if(u.type == "radio") {
                                                                            ...
                                                                            if(u.enhanced_multiple_choice == 1) {
                                                                                $(".upload-icon").show();
                                                                                $("#prop_other_choices_icon").show();
                                                                                $(".uploaded-icon-img").show();
                                                                            } else {
                                                                                $(".upload-icon").hide();
                                                                                $("#prop_other_choices_icon").hide();
                                                                                $(".uploaded-icon-img").hide();
                                                                            }
                                                                        }
                                                                        if(u.type == "checkbox") {
                                                                            $("#element_choice_columns").val(u.choice_columns);
                                                                            $("#prop_choice_columns").show();
                                                                            if(u.enhanced_checkbox == 1) {
                                                                                $(".upload-icon").show();
                                                                                $("#prop_other_choices_icon").show();
                                                                                $(".uploaded-icon-img").show();
                                                                            } else {
																		...
																		if (u.type == "radio" || u.type == "checkbox") {
                                                                            ...
                                                                            $("#prop_other_choices_score").val(u.choice_other_score);
                                                                            var ois = u.choice_other_icon_src;
                                                                            var icon_img = "";
                                                                            if (!ois) {
                                                                                ois = "";
                                                                            } else {
                                                                            ...
                                                                            $("#prop_other_choices_icon").parent().find(".uploaded-icon-img").remove();
                                                                            $(icon_img).insertAfter($("#prop_other_choices_icon"));
																			if (u.choice_has_other == 1) {
																				$("#prop_choices_other_checkbox").prop("checked", true);
																				$("#prop_other_choices_label").prop("disabled", false);
                                                                                $("#prop_other_choices_score").prop("disabled", false);
                                                                                $("#prop_other_choices_icon").prop("disabled", false);
																			} else {
																				$("#prop_choices_other_checkbox").prop("checked", false);
																				$("#prop_other_choices_label").prop("disabled", true);
                                                                                $("#prop_other_choices_score").prop("disabled", true);
                                                                                $("#prop_other_choices_icon").prop("disabled", true);
																			}
																			if (u.type == "radio") {
																		...
																		if (u.type == "matrix") {
																			...
																			$.each(b, function(v, w) {
																				if (v > 0) {
																					...
																					//g = w;
																					g = i[w].row_title;
																					_pmc = i[w].machine_code; // policy machine code
																					n = v;
																					if (v > 1) {
																						l = '<img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="matrixrowdel_' + choice_element_id + '">'
																					}
																					j = '<li><input type="text" value="' + g.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="matrixrow_' + choice_element_id + '" style="width:110px;" /> <input type="text" value="' + _pmc.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="matrix_policy_row_' + choice_element_id + '" style="width:110px;" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="matrixrowadd_' + choice_element_id + '">' + l + "</li>";
																					e += j
																				}
																			});
																			...
																			$.each(q, function(v, w) {
																				if (v > 0) {
																					f = w;
																					g = h[w].column_title;
																					_s = (!h[w].column_score) ? 0 : parseFloat(h[w].column_score);
																					n = v;
																					j = '<li><input type="text" value="' + g.replace(/\"/g, "&quot;") + '" autocomplete="off" class="text" id="matrixcol_' + f + '" style="width:110px;" /> <input type="text" value="' + _s + '" autocomplete="off" class="text choice-value" id="matrixcolscore_' + f + '" style="width:110px;" maxlength="5" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="matrixcoladd_' + f + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="matrixcoldel_' + f + '"></li>';
																					e += j
																				}
																			});
																			...
																			if (u.type == "page_break") {
																				$('#prop_field_notes').hide();
																				$("#all_properties li").hide();
																				$("#prop_page_break_button, #prop_page_break_background").show();
                                                                                $("#element_page_break_bg_color").val('');
                                                                                ...
                                                                                if (u.page_break_bg_color)
                                                                                    $("#element_page_break_bg_color").val(u.page_break_bg_color);
																			}
																		}
																	}
                                                                }
    ...
    if(elementType == 'syndication') {
        $('.leftCol, .rightCol, #prop_machine_code, #embed_machine_code, #prop_instructions, #prop_field_notes').hide();
        $('.feed-element, #prop_element_label_background_color, #prop_element_label_color').css('display', 'list-item');
        $('#element_feed_url').val('');
        Syndication.reload(u);
    }
    if(elementType == 'video_player') {
        $('.leftCol, .rightCol, #prop_machine_code, #embed_machine_code, #prop_instructions, #prop_field_notes').hide();
        $('.video-player-element:not(#prop_video_url), #prop_element_label_background_color, #prop_element_label_color').css('display', 'list-item');
        VideoPlayer.reload(u);
    }
    $("#element_label").focus().select();
    if( u.is_db_live == 0 ) {
        $('#embed_machine_code').hide();
    } else {
        //set attr for select 'Embed Machine Code'
        if(u.hasOwnProperty("element_id_auto")){
            var element_id_auto = u.element_id_auto;
            $('#ec_code_type_field_properties').attr('data-element_id_auto', element_id_auto);
        }
        if(u.hasOwnProperty("element_page_number")){
            var element_page_number = u.element_page_number;
            $('#ec_code_type_field_properties').attr('data-element_page_number', element_page_number);
        }
    }
}
function get_casecade_form_list(p){
	$.ajax({
		type: "POST",
        url: "list_casecade_forms_options.php",
        data: p,
        cache: false,
        success: function(r) {
			$('#element_default_casecade_form').html(JSON.parse(r).options);
			$('meta#csrf-token-meta').attr('content', JSON.parse(r).csrf_token);
			$('input#post-csrf-token').val(JSON.parse(r).csrf_token);
		},
		complete: function (event, request, settings) {
			setAjaxDefaultParam();
		}
...
function set_properties(B, w, selected_option_text) {
	//alert('Test');
    var g = $("#" + active_element);
    var y = g.data("field_properties");
	//alert(y);
    ...
    var u = k[1];
	//y.machine_code = $('#element_machine_code').val();
	if(B == 'element_note'){
		y.note = w;
	}
    if(B == 'element_label_background_color'){
        y.label_background_color = w;
        if( w ) {
            if( y.type == 'matrix' ) {
                g.find('caption').css('background-color', w);
            } else {
                g.find('label.description').css('background-color', w);
            }
        }
    }
    if(B == 'element_label_color'){
        y.label_color = w;
        if( w ) {
            if( y.type == 'matrix' ) {
                g.find('caption').css('color', w);
            } else {
                g.find('label.description').css('color', w);
            }
        }
    }
    if(B == 'element_page_break_bg_color'){
        y.page_break_bg_color = w;
    }
	if(B == 'element_status_indicator'){
		y.status_indicator = w;
	}
    if(B == 'element_enhanced_checkbox'){
        if(w == 1){
            $("#" + active_element + " .element_checkbox").removeClass("checkbox");
            $("#" + active_element + " .element_checkbox").addClass("enhanced_checkbox");
            $("#" + active_element + " .element_checkbox_label").removeClass("choice");
            $("#" + active_element + " .element_checkbox_label").addClass("enhanced_checkbox_label");
        } else {
            $("#" + active_element + " .element_checkbox").removeClass("enhanced_checkbox");
            $("#" + active_element + " .element_checkbox").addClass("checkbox");
            $("#" + active_element + " .element_checkbox_label").removeClass("enhanced_checkbox_label");
            $("#" + active_element + " .element_checkbox_label").addClass("choice");
        }
        y.enhanced_checkbox = w;
    }
    if(B == 'element_enhanced_multiple_choice'){
        y.enhanced_multiple_choice = w;
    }
	if(B == 'element_rich_text'){
		y.rich_text = w;
	}
	if(B == 'element_machine_code'){
          y.machine_code = w
	}
    if(B == 'element_file_upload_synced'){
          y.file_upload_synced = w;
    }
    if(B == 'element_file_select_existing_files'){
          y.file_select_existing_files = w;
    }
    if (B == "element_label") {
        if (y.type == "section") {
			//console.log('active_element: '+active_element);
            $("#" + active_element + " > div#sectionbreakh3-"+active_element).html(w);
            y.title = w
        } else {
            if (y.type == "matrix") {
                if (y.is_required === "0") {
                ...
                $("#" + active_element + " div:first > label").html(w.replace(/\n/g, "<br />") + M);
                y.guidelines = w
            } else {
                if (y.is_required === "0") {
                    M = ""
        ...
        if (B == "element_required") {
                ...
                if (B == "element_visibility") {
                    ...
                    if (B == "element_guidelines") {
                        if (y.type != "section") {
                            ...
                            $("#div#sectionbreakp-"+active_element).html(w);
                        }
                        y.guidelines = w
                    } else {
                                                                                                                                            ...
                                                                                                                                            if (B == "element_size") {
                                                                                                                                                ...
                                                                                                                                                if (B == "element_type_change") {
                                                                                                                                                    ...
                                                                                                                                                    if (g.data("field_properties").is_db_live == 0) {
                                                                                                                                                        g.html('<img src="images/loader_small.gif" style="vertical-align: middle"/> changing field type ...').addClass("delete_processing");
                                                                                                                                                        $.ajax({
                                                                                                                                                            type: "POST",
                                                                                                                                                            async: true,
                                                                                                                                                                    ...
                                                                                                                                                                    $.ajax({
                                                                                                                                                                        ...
                                                                                                                                                                        success: function(S) {
                                                                                                                                                                            if (S.status == "ok") {
                                                                                                                                                                        		...
                                                                                                                                                                        		$('meta#csrf-token-meta').attr('content', S.csrf_token);
					$('input#post-csrf-token').val(S.csrf_token);
				},
				complete: function (event, request, settings) {
					setAjaxDefaultParam();
				}
			...
			if(y.type == "casecade_form"){
				var _ele = parseInt(y.id);
				$('#casecade_'+_ele).text(selected_option_text);
				y.guidelines = selected_option_text;
			}else{
				$("#" + active_element + " :input").val(w)
                                                                                                                                                        ...
                                                                                                                                                        if (B == "element_choice_other_label") {
                                                                                                                                                            ...
                                                                                                                                                            if (B == "element_choice_has_other") {
                                                                                                                                                                ...
                                                                                                                                                                if (w == 1) {
                                                                                                                                                                    if(y.type == "checkbox") {
                                                                                                                                                                        if(y.enhanced_checkbox == 1){
                                                                                                                                                                            var A = "";
                                                                                                                                                                            A += '<input id="element_' + y.id + '_0" name="element_' + y.id + '" class="element element_checkbox enhanced_checkbox" type="' + y.type + '" value="0" />';
                                                                                                                                                                            A += '<label class="element_checkbox_label enhanced_checkbox_label other" for="element_' + y.id + '_0">' + y.choice_other_label + "</label>";
                                                                                                                                                                            A += '<input type="text" value="" class="element text other" name="element_' + y.id + '_other" id="element_' + y.id + '_other" />';
                                                                                                                                                                            A = "<span>" + A + "</span>";
                                                                                                                                                                            $("#" + active_element + " > div:nth-child(2)").append(A);
                                                                                                                                                                            $("#" + active_element + " > div:nth-child(2) > span").last().hide().slideDown();
                                                                                                                                                                            $("#prop_other_choices_label").prop("disabled", false).select().focus();
                                                                                                                                                                            $("#prop_other_choices_score").prop("disabled", false);
                                                                                                                                                                            $("#prop_other_choices_icon").prop("disabled", false);
                                                                                                                                                                        } else {
                                                                                                                                                                            var A = "";
                                                                                                                                                                            A += '<input id="element_' + y.id + '_0" name="element_' + y.id + '" class="element element_checkbox checkbox" type="' + y.type + '" value="0" />';
                                                                                                                                                                            A += '<label class="element_checkbox_label choice other" for="element_' + y.id + '_0">' + y.choice_other_label + "</label>";
                                                                                                                                                                            A += '<input type="text" value="" class="element text other" name="element_' + y.id + '_other" id="element_' + y.id + '_other" />';
                                                                                                                                                                            A = "<span>" + A + "</span>";
                                                                                                                                                                            $("#" + active_element + " > div:nth-child(2)").append(A);
                                                                                                                                                                            $("#" + active_element + " > div:nth-child(2) > span").last().hide().slideDown();
                                                                                                                                                                    ...
                                                                                                                                                                    $("#" + active_element + " div:nth-child(2) > span").last().slideUp("slow", function() {
                                                                                                                                                                        $(this).remove()
                                                                                                                                                                    });
                                                                                                                                                                    $("#prop_other_choices_label").prop("disabled", true);
                                                                                                                                                                    $("#prop_other_choices_score").prop("disabled", true);
                                                                                                                                                                    $("#prop_other_choices_icon").prop("disabled", true);
                                                                                                                                                                    //$("#prop_other_choices_score").val("");
                                                                                                                                                                }
                                                                                                                                                            } else {
                                                                                                                                                                if (B == "constraint") {
                                                                                                                                                                    if (y.type == "money") {
                                                                                                                                                        ...
                                                                                                                                                        if (B == "element_choice_other_score") {
                                                                                                                                                            y.choice_other_score = w;
                                                                                                                                                        }
                                                                                                                                                    }

                                                                                                                                                    if(B == "element_cascade_form_invisible"){
                                                                                                                                                        y.cascade_form_invisible = w;
                                                                                                                                                    }
                                                                                                                                                }
                                                                                                                                            }
                                                                                                                                        }
...
function add_element(a) {
    ...
    if ($("#li_lastpage").length == 0) {
        $('<li id="li_temp_' + d + '" class="click_processing"><img src="images/loader_small.gif" style="vertical-align: middle"/> creating field ...</li>').appendTo("#form_builder_sortable").hide().fadeIn()
    } else {
        $('<li id="li_temp_' + d + '" class="click_processing"><img src="images/loader_small.gif" style="vertical-align: middle"/> creating field ...</li>').insertBefore("#li_lastpage").hide().fadeIn()
    }
    $.ajax({
        type: "POST",
        async: true,
        ...
        success: function(f) {
			...
			$('meta#csrf-token-meta').attr('content', f.csrf_token);
			$('input#post-csrf-token').val(f.csrf_token);
        },
		complete: function (event, request, settings) {
			setAjaxDefaultParam();
		}
...
function add_matrix_row(d) {
    ...
    var g = '<tr id="mr_temp_' + a + '" class="matrix_row_processing"><td colspan="' + i + '"><img src="images/loader_small.gif" style="vertical-align: middle"/> creating new row ...</td></tr>';
    var k = $("#element_matrix_row li").index(d.parent()) + 1;
    $("#" + active_element + " table tr:eq(" + k + ")").after(g);
    var c = '<li id="li_mr_temp_' + a + '" class="li_mr_temp_holder"><input type="text" disabled="disabled" value="creating new row ..." autocomplete="off" class="text" id="matrixrow_' + a + '" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="matrixrowadd_' + a + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="matrixrowdel_' + a + '"></li>';
    d.parent().after(c);
    $.ajax({
        type: "POST",
        async: true,
        ...
        success: function(o) {
        	...
        	$('meta#csrf-token-meta').attr('content', o.csrf_token);
			$('input#post-csrf-token').val(o.csrf_token);
        },
		complete: function (event, request, settings) {
			setAjaxDefaultParam();
		}
...
function delete_matrix_row(g) {
    ...
    if ($("#element_matrix_row > li").size() <= 1) {
        $("#dialog-warning").dialog("option", "title", "Unable to delete this row!");
    ...
    var d = $("#mr_" + a).html();
    $("#mr_" + a).addClass("matrix_row_processing").html('<td colspan="' + e + '"><img src="images/loader_small.gif" style="vertical-align: middle"/> deleting row ...</td>');
    ...
    $.ajax({
        ...
        success: function(l) {
        	...
        	$('meta#csrf-token-meta').attr('content', l.csrf_token);
			$('input#post-csrf-token').val(l.csrf_token);
		},
		complete: function (event, request, settings) {
			setAjaxDefaultParam();
		}
...
function add_matrix_column(a) {
    ...
    var l = parseFloat(q.data("field_properties").last_option_id) + 1;
    ...
    var f = '<li><input type="text" value="" autocomplete="off" class="text" id="matrixcol_' + l + '" style="width:110px;" /> <input type="text" value="" autocomplete="off" class="text choice-value" id="matrixcolscore_' + l + '" style="width:110px;" maxlength="5" />  <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="matrixcoladd_' + l + '"><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="matrixcoldel_' + l + '"></li>';
    a.parent().after(f);
    $("#matrixcol_" + l).focus();
...
function delete_matrix_column(h) {
    ...
    if ($("#element_matrix_column > li").size() <= 1) {
        $("#dialog-warning").dialog("option", "title", "Unable to delete this column!");
...
function set_form_properties(l, a) {
    ...
    if (l == "form_title") {
        ...
        d.name = a
    } else if (l == "form_framework_type") {
        if (a == 1) {
            $("#show_framework").hide();
            $("#show_framework_group").show().focus().select()
        } else {
            $("#show_framework_group").hide();
            $("#show_framework").show().focus().select()
        }
        d.framework_type = a
    } else {
                                                                            ...
                                                                            if (l == "form_limit") {
                                                                                ...
                                                                                if (l == "form_pagination_type") {
                                                                                    ...
                                                                                    if (a == "steps") {
                                                                                        ...
                                                                                        if (a == "percentage") {
                                                                                            var h = '<h3 id="page_title_1">Page 1 of 1 - Page Title</h3><div class="la_progress_container"><div id="la_progress_percentage" class="la_progress_value" style="width: 1%"><span>1%</span></div></div>';
                                                                                            $("#pagination_header").append(h);
                                                                                            $("#page_title_1").html("Page 1 of " + e + " - " + j.eq(0).data("field_properties").page_title);
                                                                                            var c = Math.round((1 / e) * 100);
                                                                                            $("#la_progress_percentage > span").html(c + "%");
                                                                                            $("#la_progress_percentage").animate({
                                                                                                width: c + "%"
                                                                                            }, {
                                                                                                duration: 200,
                                                                                                queue: false
...
function reorganize_page_break() {
    ...
    var h = '<li class="no_fields_in_page" title="Drag the button on the right side here to add new field."><img src="images/navigation/FFFFFF/50x50/Forward.png" style="margin-bottom: 20px;background-color: #33BF8C;font-size: 50px;"><h3>This page has no fields!</h3><p><span style="color: #33BF8C; font-weight: bold; float: none">Drag the button</span> on the right sidebar here to add new field.</p></li>';
        ...
        if (i.not("#li_lastpage").length == 1) {
            if ($("#pagination_header").length == 0) {
                ...
                if (d == "steps") {
                    ...
                    if (d == "percentage") {
                        ...
                        $("#la_progress_percentage > span").html("50%")
                    }
                }
            }
            if ($("#li_lastpage").length == 0) {
            ...
            if (d == "steps") {
                ...
                if (d == "percentage") {
                    ...
                    $("#la_progress_percentage > span").html(b + "%")
                }
            }
        }
...
$(function() {
    ...
    $("#builder_tabs").tabs();
    $("#builder_tabs").bind("tabsactivate", function(c, d) {
        if (d.newTab.index() === 0) {
            $("#" + active_element).removeClass("highlighted");
            active_element = null;
            $("#form_header .highlighted").removeClass("highlighted");
    ...
    $("#form_builder_sortable").sortable({
        ...
        receive: function(g, i) {
            ...
            $("#form_builder_sortable .ui-draggable").attr("id", "li_temp_" + h).removeClass("box ui-draggable").addClass("drop_processing").html('<img src="images/loader_small.gif" style="vertical-align: middle"/> creating field ...').hide().fadeIn();
            ...
            switch (f) {
                ...
                case "btn_page_break":
                    c = "page_break";
                    break;
                case "btn_matrix_choice":
                    c = "matrix";
                    break;
                case "btn_syndication":
                    c = "syndication";
                    break;
                case "btn_signature":
                    c = "signature";
                    break;
				/* Casecade form */
				case "btn_casecade_form":
					c = "casecade_form";
                    break;
				/*****************/
                case "btn_video_player":
                    c = "video_player";
                    break;
                default:
                    c = "text";
            ...
            $.ajax({
                ...
                success: function(k) {
                	...
                	$('meta#csrf-token-meta').attr('content', k.csrf_token);
					$('input#post-csrf-token').val(k.csrf_token);
				},
				complete: function (event, request, settings) {
					setAjaxDefaultParam();
				}
        ...
        update: function(c, d) {
            if ($(d.item).hasClass("no_fields_in_page")) {
                ...
                if ($(d.item).hasClass("drop_processing") === false) {
                    $("#builder_tabs").tabs("option", "active", 1);
    ...
    $(document).ready(function() {
        ...
        if (isScrolledIntoView("#bottom_bar_limit")) {
            ...
            $("#bottom_bar").css("width", "100%");
        } else {
            $("#bottom_bar").css("position", "fixed");
            $("#bottom_bar").css("width", "calc(81% - 402px)");
        }
        $(window).scroll(function() {
            ...
            if ($("#builder_tabs").tabs("option", "active") === 0) {
                $("#sidebar").animate({
                    top: c
                }, {
                    duration: 500,
    ...
    $("#bottom_bar_add_field").click(function() {
        $("#builder_tabs").tabs("option", "active", 0);
        if (isScrolledIntoView("#navigation")) {
            sidebar_pos = sidebar_scroll_pos
        } else {
    ...
    $("#bottom_bar_save_form").click(function() {
        var cascade_emtpy_flag = false;
        var cascade_elements = $("#form_builder_sortable > li.casecade_form");
        cascade_elements.each(function(cascade_element) {
            if($(this).data("field_properties").default_value == "" || $(this).data("field_properties").default_value == 0) {
                cascade_emtpy_flag = true;
            }
        });
        if(cascade_emtpy_flag) {
            $("#dialog-warning").dialog("option", "title", "Unable to save. Your form contains empty cascade form widgets!");
            $("#dialog-warning-msg").html("Your form contains one or more empty cascade forms.<br />Please select forms that need to be cascaded or delete the empty cascade form widgets.");
            $("#dialog-warning").dialog("open");
            return false;
        }
        var enable_auto_mapping = $("#form_header").data("form_properties").enable_auto_mapping;

        //if auto mapping enabled, show user message to confirm request
        var pull_from_form = '';
        if( enable_auto_mapping ) {
            if( $('#bottom_bar_save_form').data('allow_syncsave') == 0 ) {
                $("#dialog-syncsave").dialog('open');
                return false;
            }
		...
		var isValid = true;
        var for_selected_entity = $("#form_header").data("form_properties").for_selected_entity;
        var data_counter = parseInt($(this).attr('data-counter'));
        var c = $("#form_header").data("form_properties").id;
        var e = $("#form_builder_sortable > li").not(".dblive").not(".synched").not(".li_pagination").not("#li_lastpage");
        var en = $("#form_builder_sortable li.dblive").not(".synched").not(".li_pagination");
        var em = $("#form_builder_sortable > li").not(".dblive").not(".li_pagination").not("#li_lastpage");
		...
		if(data_counter){
			...
			if($("#form_header").data("form_info").isFieldDelete == '1'){
				$("#form_header").data("form_info").isFieldDelete = '0';
				$("#dialog-confirm-edit").dialog('open');
				isFieldChanges = true;
				return false;
			}
		}
		if($(this).hasClass('disable-save') == true){
			return false;
		}
        if ($("#form_builder_sortable > li.no_fields_in_page").length > 0) {
            $("#dialog-warning").dialog("option", "title", "Unable to save. Your form contains empty pages!");
            $("#dialog-warning-msg").html("Your form contains one or more empty pages<br />Please add a field into the empty page or delete the empty page.");
            $("#dialog-warning").dialog("open");
            return false;
        }
        $("#bottom_bar_msg").text("Please wait ... ITAM is synching ...");
	...
	$("#element_machine_code").bind("keyup mouseout", function() {
		//alert('Test');
        set_properties("element_machine_code", $('#element_machine_code').val())
    });
    ...
    $("#element_choices").delegate("input.text", "keyup", function(p) {
        ...
        if (p.keyCode == 13) {
            ...
            var c = '<li><input type="' + m + '" title="Select this choice as the default." class="choices_default" name="choices_default" id="choicedef_' + k + '" /><input type="text" value="" autocomplete="off" class="text" id="choice_' + k + '" style="width:150px;" /> <input type="text" value="0" autocomplete="off" class="choice-value" id="choice_value_' + k + '" style="width:35px;" maxlength="5" placeholder="Score" /> <img class="add_choice" title="Add" alt="Add" src="images/icons/add.png" style="vertical-align: middle" id="choiceadd_' + k + '" ><img class="del_choice" title="Delete" alt="Delete" src="images/icons/delete.png" style="vertical-align: middle" id="choicedel_' + k + '" ><input type="button" id="choice_upload_' + k + '" class="upload-icon" value="Upload Icon" /> </li>';
            n.parent().after(c);
            r.data("field_properties").last_option_id = k;
            var o = "";
            if (f == "radio") {
                o = '<span><input type="radio" value="1" class="element radio" name="element_' + d + '" id="element_' + d + "_" + k + '"><label for="element_' + d + "_" + k + '" class="choice">&nbsp;</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + k + '" id="elementvalue' + d + "_" + k + '"></span>'
                if(r.data("field_properties").enhanced_multiple_choice == 1) {
                    $(".upload-icon").show();
                    $("#prop_other_choices_icon").show();
                    $(".uploaded-icon-img").show();
                } else {
                ...
                if (f == "checkbox") {
                    if(r.data("field_properties").enhanced_checkbox == 1){
                        o = '<span><input type="checkbox" value="1" class="element element_checkbox enhanced_checkbox" name="element_' + d + '" id="element_' + d + "_" + k + '"><label for="element_' + d + "_" + k + '" class="element_checkbox_label enhanced_checkbox_label">&nbsp;</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + k + '" id="elementvalue' + d + "_" + k + '"></span>'
                        $(".upload-icon").show();
                        $("#prop_other_choices_icon").show();
                        $(".uploaded-icon-img").show();
                    } else {
                        o = '<span><input type="checkbox" value="1" class="element element_checkbox checkbox" name="element_' + d + '" id="element_' + d + "_" + k + '"><label for="element_' + d + "_" + k + '" class="element_checkbox_label choice">&nbsp;</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + k + '" id="elementvalue' + d + "_" + k + '"></span>'
                        $(".upload-icon").hide();
                        $("#prop_other_choices_icon").hide();
                        $(".uploaded-icon-img").hide();
                    }
	...
	$("#element_choices").delegate("input.choice-value", "keyup", function(p) {
        var n = $(this);
        var g = n.attr("id").split("_");
		var h = active_element.split("_");
		var e = h[1];
		var v = g[2];
		var r = $("#" + active_element);
		var value = n.val();
        ...
        var _regex = /^-?\d+\.?\d*$/
        if(value != ''){
            if(!(value.match(_regex) || value == '-')){
                alert('Please enter numbers only');
                n.val(0);
                return false;
            }
            if(parseFloat(value) > _maxVal || parseFloat(value) < _minVal){
                alert('Score must be between -999 to 999');
                n.val(0);
                return false;
            }
            $('#elementvalue'+e+'_'+v).val(value);
            r.data("field_properties").options[v].option_value = value;
        } else {
            $('#elementvalue'+e+'_'+v).val(value);
            r.data("field_properties").options[v].option_value = value;
        }
    ...
    $("#element_choices").delegate("img.del_choice", "click", function(k) {
        if ($("#element_choices > li").size() <= 1) {
            $("#dialog-warning").dialog("option", "title", "Unable to delete this choice!");
    ...
    $("#element_label_background_color").bind("change", function() {
        set_properties("element_label_background_color", $(this).val())
    });
    $("#element_label_color").bind("change", function() {
        set_properties("element_label_color", $(this).val())
    });
    $("#element_page_break_bg_color").bind("change", function() {
        set_properties("element_page_break_bg_color", $(this).val())
    });
	...
	/** casecade form default value settings **/
	$("#element_default_casecade_form").bind("change", function() {
        set_properties("element_default_value", $(this).val(),  $('#'+$(this).attr('id')+' option:selected').text());
    });
    $("#element_cascade_form_invisible").bind("change", function() {
        set_properties("element_cascade_form_invisible", $(this).prop('checked') ? 1 : 0);
    });
	/** ************************************ **/
	$('#element_field_notes').bind('change', function(){
		set_properties("element_note", $(this).prop('checked') ? 1 : 0);
	});
	$('#element_status_indicator').bind('change', function(){
		set_properties("element_status_indicator", $(this).prop('checked') ? 1 : 0);
	});
    $('#element_enhanced_checkbox').bind('change', function(){
        set_properties("element_enhanced_checkbox", $(this).prop('checked') ? 1 : 0);
    ...
    $('#element_enhanced_multiple_choice').bind('change', function(){
        set_properties("element_enhanced_multiple_choice", $(this).prop('checked') ? 1 : 0);
        if($(this).prop('checked') == 1) {
            $(".upload-icon").show();
            $("#prop_other_choices_icon").show();
            $(".uploaded-icon-img").show();
	...
	$('#element_rich_text').bind('change', function(){
		set_properties("element_rich_text", $(this).prop('checked') ? 1 : 0);
	});
    $("#prop_other_choices_label").bind("keyup mouseout", function() {
        set_properties("element_choice_other_label", $(this).val())
    });
    $("#prop_other_choices_score").bind("keyup mouseout", function() {
        set_properties("element_choice_other_score", $(this).val())
    });
    ...
    $("#prop_file_upload_synced").bind("change", function() {
        var c;
        if ($(this).prop("checked") == true) {
            c = 1
        } else {
        ...
        set_properties("element_file_upload_synced", c)
    });
    ...
    $("#prop_file_select_existing_files").bind("change", function() {
        var c;
        if ($(this).prop("checked") == true) {
            c = 1
        } else {
        ...
        set_properties("element_file_select_existing_files", c)
    });
    ...
    $("#element_matrix_row").delegate("input.text", "keyup", function(g) {
		...
		if(h[0] == 'matrix'){
			...
			if (i.parent().hasClass("li_mr_temp_holder") == false) {
				...
				d.data("field_properties").options[c].machine_code = i.val();
			}
		}
    });
    ...
    $("#element_matrix_column").delegate("input.text", "keyup", function(h) {
		...
		if(i[0] == 'matrixcolscore'){
			d.data("field_properties").options[c].column_data[f].column_score = $('#matrixcolscore_'+f).val();
		}
    });
    ...
    //handle framework group
    $("#framework_type").click(function() {
        set_form_properties("form_framework_type", 0)
    });
    $("#framework_type_group").click(function() {
        set_form_properties("form_framework_type", 1)
    });
    ...
    $("#form_prop_toggle_a,#form_prop_toggle_img").click(function() {
        if ($("#form_prop_toggle_a").text() == "show more options") {
            ...
            if ($("#form_review").prop("checked") == true) {
                $("#all_form_properties .advanced_prop").slideDown("slow", function() {
                    adjust_main_height()
                })
            } else {
                $("#all_form_properties .advanced_prop").not("#form_prop_review").slideDown("slow", function() {
                    adjust_main_height()
                })
            }
    ...
    $("#pagination_title_list").delegate("input.text", "keyup", function(j) {
		...
		//console.log(k)
		//console.log($("#form_builder_sortable li.page_break"))
        //d.eq(c - 1).data("field_properties").page_title = h.val();
		//d.find('li#'+ k[1] + '_' + k[2]).data("field_properties").page_title = h.val();
		//console.log("#form_builder_sortable > li#"+ k[1] + '_' + k[2]);
		//console.log($("#form_builder_sortable > li#"+ k[1] + '_' + k[2]).data("field_properties").page_title = h.val());
		//console.log(d.find('li#'+ k[1] + '_' + k[2]).data("field_properties").page_title);
        //d.eq(c - 1).removeClass("synched")
		$("#form_builder_sortable > li#"+ k[1] + '_' + k[2]).data("field_properties").page_title = h.val();
		$("#form_builder_sortable > li#"+ k[1] + '_' + k[2]).removeClass("synched");
    });
    ...
    $("#dialog-syncsave").dialog({
        ...
        buttons: [{
            ...
            "class": "bb_button bb_small bb_green",
            click: function() {
                $('#bottom_bar_save_form').data('allow_syncsave', 1);
                $('#bottom_bar_save_form').data('pull_from_form', $( "#pull_from_form" ).val());
                $(this).dialog("close");
                $('#bottom_bar_save_form').click();
            }
        },
        {
            text: "Close",
    ...
    $("#dialog-confirm-field-delete").dialog({
        ...
        buttons: [{
            ...
            click: function() {
                ...
                var c = d.html();
                var e = $("#form_header").data("form_properties").id;
                d.html('<img src="images/loader_small.gif" style="vertical-align: middle"/> deleting field ...').addClass("delete_processing");
                $(this).dialog("close");
				$("#form_header").data("form_info").isFieldDelete = '1';
                $.ajax({
                    type: "POST",
                    async: true,
    ...
    $("#dialog-form-saved").dialog({
        ...
        buttons: [{
            ...
            click: function() {
                ...
                window.location.replace("edit_form.php?id=" + c + "&t=" + d)
            }
        }, {
            text: "I'm finished",
            ...
            click: function() {
                var c = $("#form_header").data("form_properties").id;
				var f = $("#form_header").data("form_properties").folder_id;
				if (f == '0'){
					window.location.replace("manage_forms.php?id=" + c + "&hl=1");
				} else {
					$('<form id="form-temp" action="manage_forms.php" method="post"><input type="hidden" value="'+f+'" name="folder" /></form>').appendTo('body');
					$('form#form-temp').submit();
				}
            }
        }]
    });
            ...
            click: function() {
                if ($("#bulk_insert_choices").val() == "") {
                    ...
                    $.each(c, function(l, m) {
                        ...
                        if (e == "radio") {
                            j += '<span><input type="radio" value="1" class="element radio" name="element_' + d + '" id="element_' + d + "_" + f + '"><label for="element_' + d + "_" + f + '" class="choice">' + m + '</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + f + '" id="elementvalue' + d + "_" + f + '"></span>'
                        } else {
                            if (e == "checkbox") {
                                if(k.data("field_properties").enhanced_checkbox == 1){
                                    j += '<span><input type="checkbox" value="1" class="element element_checkbox enhanced_checkbox" name="element_' + d + '" id="element_' + d + "_" + f + '"><label for="element_' + d + "_" + f + '" class="element_checkbox_label enhanced_checkbox_label">' + m + '</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + f + '" id="elementvalue' + d + "_" + f + '"></span>'
                                } else {
                                    j += '<span><input type="checkbox" value="1" class="element element_checkbox checkbox" name="element_' + d + '" id="element_' + d + "_" + f + '"><label for="element_' + d + "_" + f + '" class="element_checkbox_label choice">' + m + '</label><input type="hidden" value="0" class="elementvalue" name="elementvalue' + d + "_" + f + '" id="elementvalue' + d + "_" + f + '"></span>'
                                }
                            } else {
                                if (e == "select") {
        ...
        buttons: [{
            ...
            click: function() {
                if ($("#bulk_insert_rows").val() == "") {
                    ...
                    var i = $("#" + active_element + " table th").length;
                    var f = '<tr id="mr_temp_' + h + '" class="matrix_row_processing"><td colspan="' + i + '"><img src="images/loader_small.gif" style="vertical-align: middle"/> creating new rows ...</td></tr>';
    ...
    $("#social li").not('#feed_item li').draggable({
    ...
    $("#bottom_bar_duplicate_field").click(function() {
        ...
        $('<li id="li_temp_' + e + '" class="duplicate_processing"><img src="images/loader_small.gif" style="vertical-align: middle"/> duplicating field ...</li>').insertAfter("#" + active_element).hide().fadeIn();
        ...
        $.ajax({
            ...
            success: function(h) {
                if (h.status == "ok") {
                    ...
                    if (g.data("field_properties").type == "section") {
                        var field_title = h.field_properties.title;
                        field_title = field_title.replace(/\n/g, "<br />");
                        $("#li_" + h.element_id + " > h3").html(field_title);
                    } else {
                        if (g.data("field_properties").type == "page_break") {} else {
                            if (g.data("field_properties").type == "matrix") {} else {
                                var field_title = h.field_properties.title;
                                field_title = field_title.replace(/\n/g, "<br />");
                                $("#li_" + h.element_id + " > label").html(field_title);
                            }
                        }
                    }

                    //when duplicating media player fields update the VideoPlayer for new field
                    if( h.cloned_from ) {
                        var duplicatedDraftFields = VideoPlayer.duplicatedDraftFields;
                        if (h.cloned_from in duplicatedDraftFields) {
                            duplicatedDraftFields[h.cloned_from] = duplicatedDraftFields[h.cloned_from]+','+h.element_id;
                        } else {
                            duplicatedDraftFields[h.cloned_from] = h.element_id;
                        }
                        VideoPlayer.duplicatedDraftFields = duplicatedDraftFields;
                    }
				...
				$('meta#csrf-token-meta').attr('content', h.csrf_token);
				$('input#post-csrf-token').val(h.csrf_token);
			},
			complete: function (event, request, settings) {
				setAjaxDefaultParam();
			}
    ...
    $("#bottom_bar_delete_field").click(function() {
        var d = $("#" + active_element);
        if ($("#form_builder_sortable > li").size() == 1) {
            $("#dialog-warning").dialog("option", "title", "Unable to delete this field!");
...
$(document).ready(function() {
	...
	$('#upload-files').click(function(){
		upload_string = $('#form_upload_template').val();
		upload_string = upload_string.trim();
		if(upload_string != '')
		{
			if(upload_string.indexOf(",") != -1)
			{
				tmpArr = upload_string.split(",");
			}
			else
			{
				tmpArr = [upload_string];
			}
		}
	...
	$('input#image-file').live('change', function(){
		if (window.File && window.FileReader && window.FileList && window.Blob){
			...
			for(index in filesObj) {
				if(!isNaN(index)){
					var fname = $('input#image-file')[0].files[parseInt(index)].name; //get file name
					var fsize = $('input#image-file')[0].files[parseInt(index)].size; //get file size
					var ftype = $('input#image-file')[0].files[parseInt(index)].type; //get file type
                    //Disabled condition because the ftype variable was not accurately being set. Uses should know to only upload legit files right? :)
                    if('application/msword' != ftype && 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' != ftype && 'application/vnd.ms-excel' != ftype && 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' != ftype){
                        isValidArr.push({
                            'filename': fname
                        });
                    }
			...
			if(isValidArr.length == 0){
                ...
                $("#doc_loc_error_msg").html("Please upload only << .docx, .doc*, .xlsx, .xls >> files").show();
				return false;
			}
        }else{
		    //Error for older unsupported browsers that doesn't support HTML5 File API
            $("#doc_loc_error_msg").html("Please upgrade your browser, because your current browser lacks some new features we need!").show();
            return false
		}
	});
	var progressbox     = $('div#progressbox');
	var progressbar     = $('#progressbar');
	var statustxt       = $('#statustxt');
	var myform          = $("#upload-form");
	var completed       = '0%';
	$(myform).ajaxForm({
		beforeSend: function() {
            $("#doc_loc_error_msg").html("").hide();
			statustxt.empty();
		...
		uploadProgress: function(event, position, total, percentComplete) {
            ...
            if(percentComplete == 100) {
                progressbox.slideUp();
                $("#optimizebox").slideDown();
            }
		},
		complete: function(response) {
            $("#optimizebox").slideUp();
			var responseJson   = response.responseText;
			    responseJson   = JSON.parse(responseJson.trim());
			var responseString = responseJson.imagesNames;
			...
			for(i=0; i<responseArrayLength; i++){
				...
				{
					var datastr = responseArray[i];
						datastr = datastr.trim();
					if(datastr != 'FILE_OVERWRITE')
					{
						tmpArr.push(responseArray[i]);
						var dataarr = datastr.split('|||');
						$(".template-list-div").append('<div style="position:relative; height:20px; padding:2px;">'
														 + dataarr[0] +
														 '<a href="javascript:void(0)" data-image-str="'+ datastr +'" data-image="'+ dataarr[1] +'" class="btn-remove">X</a>'+
													   '</div>');
					}
			...
			$("#form_header").data("form_properties").upload_template = tmpStr;
			$("#form_upload_template").val(tmpStr);
			myform.resetForm();
			
			$('input#post-csrf-token').val(responseJson.csrf_token);
			$('meta#csrf-token-meta').attr('content', responseJson.csrf_token);
			setAjaxDefaultParam();
		}
	});
	$('.btn-remove').live('click', function(){
		var form_id = $("#form_header").data("form_properties").id;
		upload_string = $('#form_upload_template').val();
		var tmpArr = new Array();
		if(upload_string != ""){
			if(upload_string.indexOf(",") != -1){
				...
				tmpArr.push(upload_string);
			}
		}
		var _selector = $(this);
		var image_name = _selector.attr('data-image');
		var image_name_str = _selector.attr('data-image-str');
		var pos = tmpArr.indexOf(image_name_str);
		$.ajax({
			url:"processupload.php",
			type:"POST",
			data:{mode:"del_image", form_id:form_id, image_name:image_name, image_name_str:image_name_str},
			dataType:"html",
			beforeSend: function(){},
			success: function(response){
				...
				tmpArr.splice(pos, 1);
				var tmpStr = tmpArr.join(",");
				_selector.parent('div').remove();
				$("#form_header").data("form_properties").upload_template = tmpStr;
				$('#form_upload_template').val(tmpStr);
				$('meta#csrf-token-meta').attr('content', response.csrf_token);
				$('input#post-csrf-token').val(response.csrf_token);
			},
			complete: function (event, request, settings) {
				setAjaxDefaultParam();
			}
	...
	$("#form_enable_auto_mapping").bind("change", function() {
		if ($(this).prop("checked") == true) {
			$("#form_header").data("form_properties").enable_auto_mapping = 1;
            $('#bottom_bar_save_form_text').text("Sync and Save");
        } else {
			$("#form_header").data("form_properties").enable_auto_mapping = 0;
            $('#bottom_bar_save_form_text').text("Save Form");
		}
	});
    $("#form_framework").bind("change", function(){
		$("#form_header").data("form_properties").framework_or_group_id = $(this).val();
	});
    $("#form_framework_group").bind("change", function(){
		$("#form_header").data("form_properties").framework_or_group_id = $(this).val();
	});
	$("#form_for_selected_company").bind("change", function(){
		$("#form_header").data("form_properties").for_selected_company = $(this).val();
	});
	$("#form_for_selected_entity").bind("change", function(){
		$("#form_header").data("form_properties").for_selected_entity = $(this).val();
	});
	$("#form_enable_template").bind("change", function() {
		if ($(this).prop("checked") == true) {
			$("#form_header").data("form_properties").enable_template = 1;
            $("#form_template_upload_parent").show();
		} else {
			$("#form_header").data("form_properties").enable_template = 0;
            $("#form_template_upload_parent").hide();
		}
    });
    $("#form_enable_template_wysiwyg").bind("change", function() {
        $("#form_header").data("form_properties").template_wysiwyg_id = '';
		if ($(this).prop("checked") == true) {
            $("#form_header").data("form_properties").enable_template_wysiwyg = 1;
            $("#form_template_wysiwyg_parent").show();
		} else {
            $("#form_header").data("form_properties").enable_template_wysiwyg = 0;
            $("#form_template_wysiwyg_parent").hide();
		}
	});

    $("#form_template_wysiwyg_id").bind("change", function() {
        $("#form_header").data("form_properties").template_wysiwyg_id = $(this).val();
        $('#template_preview').html('');
        if( $(this).val() ) {
            $('#template_preview').html('<a target="_blank" href="/auditprotocol/create_template.php?id='+this.value+'">Preview Template</a>');
        }
	});

    $("#element_choices").delegate(".upload-icon", "click", function(c) {
        var id = $(this).attr("id");
        var option_id = id.split("_").pop();
        $("#upload-icon-option-id").val(option_id);
        $('#upload-icon-button').trigger('click');
    });

    $("#prop_choices_other").delegate("#prop_other_choices_icon", "click", function(c) {
        $("#upload-icon-option-id").val("other");
        $('#upload-icon-button').trigger('click');
    });
    
    $('input#upload-icon-button').live('change', function(){
        if (window.File && window.FileReader && window.FileList && window.Blob){
            var filesObj = $('input#upload-icon-button')[0].files;
            var fext = "";
            for(index in filesObj) {
                if(!isNaN(index)){
                    var fname = $('input#upload-icon-button')[0].files[parseInt(index)].name; //get file name
                    var fsize = $('input#upload-icon-button')[0].files[parseInt(index)].size; //get file size
                    fext = fname.split('.').pop().toLowerCase();
                }
            }
            if($.inArray(fext, ['gif','png','jpg','jpeg','ico','tiff','bmp']) == -1){
                alert('Please upload only image files');
                return false;
            }else{
                $('#upload-icon-form').trigger('submit');
            }
        }else{
            //Error for older unsupported browsers that doesn't support HTML5 File API
    ...
    $("#upload-icon-form").ajaxForm({
        complete: function(response) {
            var responseJson = response.responseText;
            responseJson = JSON.parse(responseJson.trim());
            if(responseJson.status == "ok") {
                var uploaded_icon_src = responseJson.uploaded_icon_src;
                var option_id = responseJson.option_id;
                if(option_id == "other") {
                    $("#prop_other_choices_icon").parent().find(".uploaded-icon-img").remove();
                    $('<img class="uploaded-icon-img" src="' + uploaded_icon_src + '" alt="Icon" width="20" height="20">').insertAfter($("#prop_other_choices_icon"));
                    var r = $("#" + active_element);
                    r.data("field_properties").choice_other_icon_src = uploaded_icon_src;
                } else {
                    $("#choice_upload_" + option_id).parent().find(".uploaded-icon-img").remove();
                    $('<img class="uploaded-icon-img" src="' + uploaded_icon_src + '" alt="Icon" width="20" height="20">').insertAfter($("#choice_upload_" + option_id));
                    var r = $("#" + active_element);
                    r.data("field_properties").options[option_id].option_icon_src = uploaded_icon_src;
                }
            } else {
                alert("Unable to upload the icon. Please try again later.");
            }
            $("#upload-icon-form").resetForm();
            setAjaxDefaultParam();
        }
    });
});
