$.fn.smartmarquee = function(vars) {
	...
	function marquee(obj) {
	    ...
	    var container = $(".feed-container", obj);
	    obj.hover(
			function() { self.stop(); },
	    ...
	    this.play = function(i,margin)
		{
			...
			var _vertical = function() {
				if (i < container.children().length-1) {
					var len = container.children().eq(i).outerHeight(); // .outerHeight(true)
					len += (container.children().eq(i).outerHeight(true)-len)/2;
					if (!margin) {
						margin = parseInt(container.css("margin-top")) - len;
						marginHandle = margin;
					}
					container.animate({"margin-top":margin+"px"}, options.duration, options.easing, function() {
					...
					container.animate({"margin-top":"0px"}, options.easing, function() {
			...
			var _horizontal = function() {
				if (i < container.children().length-1) {
					...
					container.animate({"margin-left":margin+"px"}, options.duration, options.easing, function() {
						timer = setTimeout(function() {
							self.play(++i);
						},options.interval);
					});
					...
					container.animate({"margin-left":"0px"}, options.easing, function() {
						setTimeout(function() { self.play(0); },options.interval);
					});
				}
			};
