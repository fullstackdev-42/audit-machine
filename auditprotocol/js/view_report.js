$(document).ready(function() {
	...
	var audit_dashboard_admin_table = $("#audit-dashboard-admin-table").DataTable({
		dom: 'Bfrtip',
		pageLength: 20,
		sPaginationType: "numbers",
		buttons: [
			{
				extend: 'csvHtml5',
				text: 'Save as CSV',
				filename: "Audit dashboard report of administrative users",
				title: "Audit dashboard report of administrative users",
				className: 'bb_button bb_small bb_green',
				exportOptions: {
					orthogonal: {
						display: ':null'
	...
	var audit_dashboard_user_table = $("#audit-dashboard-user-table").DataTable({
		dom: 'Bfrtip',
		pageLength: 20,
		sPaginationType: "numbers",
		buttons: [
			{
				extend: 'csvHtml5',
				text: 'Save as CSV',
				filename: "Audit dashboard report of portal users",
				title: "Audit dashboard report of portal users",
				className: 'bb_button bb_small bb_green',
				exportOptions: {
					orthogonal: {
						display: ':null'
	...
	var template_code_table = $("#template-code-table").DataTable({
		dom: 'Bfrtip',
		pageLength: 20,
		sPaginationType: "numbers",
		buttons: [
			{
				extend: 'csvHtml5',
				text: 'Save as CSV',
				filename: "Template code report",
				title: "Template code report",
				className: 'bb_button bb_small bb_green',
				exportOptions: {
					orthogonal: {
						display: ':null'
	...
	$(document).on('click', 'a.action-delete-note', function(e){
		e.preventDefault();
		var _selector = $(this);
		...
		if(confirm("Are you sure you want to delete this note?")) {
			$.ajax({
	            ...
	            success: function(response){
	                if(response == "deleted") {
	                    field_note_table.row($(_selector).parents('tr')).remove().draw();
	                } else {
	                	$("#ui-dialog-title-dialog-error").html('Delete Note');
					   	$("#dialog-error-msg").html("Something went wrong. Please try again later.");
						$("#dialog-error").dialog('open');
	...
	$(document).on('click', '#send-report-form #save_settings', function(e){
		var save_flag = true;
		var report_id = $("#send-report-form #report-id").val();
		if(report_id == "") {
			$("#ui-dialog-title-dialog-error").html('Save Settigs');
		   	$("#dialog-error-msg").html("Something went wrong. Please try again later.");
			$("#dialog-error").dialog('open');
            save_flag = false;
		}
		var frequency_type = 0;
		var frequency_date = "";
		var frequency_weekly = 0;
		var frequency_date_pick = 0;
		var frequency_quaterly = 0;
		var frequency_annually = 0;
		var following_up_days = $("#following_up_days").val();
        var recipients = "";
        var admin = [];
        var entity = [];
        var user = [];
        var select = $("#send_report_recipients");
        var options = select[0].options;
        if(options.selectedIndex == -1){
            $("#ui-dialog-title-dialog-error").html('Save Settigs');
		   	$("#dialog-error-msg").html("Please select recipients.");
			$("#dialog-error").dialog('open');
            save_flag = false;
        } else {
        ...
        if(save_flag){
			recipients = admin.toString()+";"+entity.toString()+";"+user.toString();
			if($("#frequency_type_1").prop("checked")) {
				frequency_type = 1;
				frequency_date = $("#frequency_date").val();
			} else if($("#frequency_type_2").prop("checked")) {
				frequency_type = 2;
			} else if($("#frequency_type_3").prop("checked")) {
				frequency_type = 3;
				frequency_weekly = $("#frequency_date_pick_3").val();
			} else if($("#frequency_type_4").prop("checked")) {
				frequency_type = 4;
				frequency_date_pick = $("#frequency_date_pick_4").val();
			} else if($("#frequency_type_5").prop("checked")) {
				frequency_type = 5;
				frequency_date_pick = $("#frequency_date_pick_5").val();
				frequency_quaterly = $("#frequency_quaterly").val();
			} else if($("#frequency_type_6").prop("checked")) {
				frequency_type = 6;
				frequency_date_pick = $("#frequency_date_pick_6").val();
				frequency_annually = $("#frequency_annually").val();
			}
        	...
        	$.ajax({
                ...
                type:"POST",
                data:{
                    mode:"send_field_note_report",
                    report_id: report_id,
                    frequency_type: frequency_type,
                    frequency_date: frequency_date,
                    frequency_weekly: frequency_weekly,
                    frequency_date_pick: frequency_date_pick,
                    frequency_quaterly: frequency_quaterly,
                    frequency_annually: frequency_annually,
                    following_up_days: following_up_days,
                ...
                success: function(response){
                    if(response != "success") {
                    	...
                    	$("#ui-dialog-title-dialog-success").html('Save Settigs');
					   	$("#dialog-success-msg").html("The settings has been saved successfully.");
