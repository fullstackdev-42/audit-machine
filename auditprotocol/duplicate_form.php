 ...
 ********************************************************************************/
	...
	$query 	= "select form_name, form_upload_template from `".LA_TABLE_PREFIX."forms` where form_id=?";
	...
	$form_upload_template = trim($row['form_upload_template']);	
	...
	while($row = la_do_fetch_result($sth)){
		if($row['Field'] == 'form_id' || $row['Field'] == 'form_name' || $row['Field'] == 'form_upload_template'){
	...
	if(!empty($form_upload_template)){
		// create folder and copy all templates to it
		createFolderPath(array(
				'dbh' => $dbh, 
	...
	// copy form relation to entities/company
	$query = "INSERT INTO `".LA_TABLE_PREFIX."entity_form_relation` (`entity_id`, `form_id`) SELECT `entity_id`, '{$new_form_id}' FROM `".LA_TABLE_PREFIX."entity_form_relation` WHERE `form_id` = ?";
