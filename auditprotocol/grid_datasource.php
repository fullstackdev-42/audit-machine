 ...
 ********************************************************************************/
	...
	**/
	...
	//---------------------------------------------------------------------------------------------
	//from this point below, the code is pretty much similar as la_display_entries_table() function
		...
		$option_label_parent_element = array();
		//get element options for matrix fields
		$query = "select DISTINCT 
						A.element_id,
						A.option_id,
						B.element_matrix_parent_id,
						A.option as 'option_label'
					from 
		...
		while($row = la_do_fetch_result($sth)){
			...
			if ($row['element_matrix_parent_id'] == 0) {
				$option_label_parent_element[$row['option_id']] = $row['option_label'];
			}
			$matrix_element_option_lookup[$element_id][$option_id] = $option_label_parent_element[$row['option_id']];
		}
					...
					if(!empty($row[$column_name])){
						...
						if(!empty($filename_array)){
							...
							foreach ($filename_array as $filename_value){
								...
								$filename_path = $la_settings['upload_dir']."/form_{$form_id}/files/{$filename_value}";
	...
	//---------------------------------------------------------------------------------------------
	//from this point above, the code is pretty much similar as la_display_entries_table() function
