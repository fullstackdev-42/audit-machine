 ...
 ********************************************************************************/
	...
	function display_itauditmachine($config){

		$form_id       = $config['form_id'];
		$show_border   = $config['show_border'];
		$itauditmachine_path = $config['base_path'];
		...
		if(la_is_form_submitted()){ //if form submitted
			...
			$input_array['itauditmachine_data_path'] = $itauditmachine_data_path;
			$input_array['itauditmachine_base_path'] = $itauditmachine_path;
				...
				if($submit_result['status'] === true){
					if(!empty($submit_result['form_resume_url'])){ //the user saving a form, display success page with the resume URL
						...
						if(is_numeric($target_page_id)){
							if(strpos($_SERVER['REQUEST_URI'],'?') === false){
								echo "<script type=\"text/javascript\">top.location = '{$_SERVER['REQUEST_URI']}?la_page={$target_page_id}'</script>";
							}else{
								echo "<script type=\"text/javascript\">top.location = '{$_SERVER['REQUEST_URI']}&la_page={$target_page_id}'</script>";
							}
							exit;
						...
						if(!empty($submit_result['next_page_number'])){ //redirect to the next page number
							...
							echo "<script type=\"text/javascript\">top.location = 'https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?id={$input_array['form_id']}&la_page={$submit_result['next_page_number']}'</script>";
							exit;
					...
					$form_params['itauditmachine_path'] = $itauditmachine_path;
					$form_params['itauditmachine_data_path'] = $itauditmachine_data_path;
			...
			if($form_properties['payment_enable_merchant'] != 1){
				...
				$commit_options['itauditmachine_path'] = $itauditmachine_path;
				$commit_options['itauditmachine_data_path'] = $itauditmachine_data_path;
			...
			echo "<script type=\"text/javascript\">top.location = 'https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?id={$form_id}&la_page={$origin_page_num}'</script>";
			exit;
		}else if(!empty($_POST['form_id_redirect'])){ //form payment being submitted
			$paid_form_id = (int) trim($_POST['form_id_redirect']);

			if($_SESSION['la_payment_completed'][$paid_form_id] === true){
