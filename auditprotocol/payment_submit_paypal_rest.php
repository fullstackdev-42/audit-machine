 ...
 ********************************************************************************/
	...
	$payment_record_id 	= $_SESSION['la_payment_record_id'][$form_id];
	$payment_data 		= la_sanitize($_POST['payment_properties']);
	...
	if(!empty($payment_enable_merchant) && $payment_merchant_type == 'paypal_rest'){
		...
		if($payment_price_type == 'fixed'){ 
			...
			$charge_amount = (double) la_get_payment_total($dbh,$form_id,$payment_record_id,0,'live');
		...
		if(!empty($payment_enable_recurring)){ //this is recurring payments
			...
			if(empty($access_token)){
				...
				$credit_card_type = la_get_credit_card_type($payment_data['card_number']);
				if(!empty($payment_data['card_cvc'])){
					$credit_card_cvv_json = '"cvv2": "'.$payment_data['card_cvc'].'",';
				}

				$billing_country_code = la_get_country_code($payment_data['billing_country']);
				$shipping_country_code = la_get_country_code($payment_data['shipping_country']);
...
EOT;
	...
	if($payment_success === true){
		$payment_status = "ok";
		$_SESSION['la_payment_completed'][$form_id] = true;

		//revoke access to form payment page
		unset($_SESSION['la_form_payment_access'][$form_id]);
	...
	function la_get_credit_card_type($card_number){
