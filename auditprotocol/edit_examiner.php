 ...
 ********************************************************************************/	
	...
	if(empty($user_id)){
		$_SESSION['LA_DENIED'] = "The examiner doesn't exist.";
		$ssl_suffix = la_get_ssl_suffix();
		header("Location: restricted.php");
		exit;
	}

	$query = "SELECT * FROM ".LA_TABLE_PREFIX."users WHERE user_id=? AND `status` > 0 AND `is_examiner` = 1";
	...
	if(la_is_form_submitted()){ //if form submitted
		...
		if(!empty($error_messages)){
			...
			$query_delete = "DELETE FROM `".LA_TABLE_PREFIX."entity_examiner_relation` WHERE `user_id` = ?";
			la_do_query($query_delete, array($user_input['user_id']), $dbh);

			foreach ($entity_permission as $value) {
				$query_entity_permission = "INSERT INTO `".LA_TABLE_PREFIX."entity_examiner_relation` (`entity_id`, `user_id`) VALUES (?, ?)";
				la_do_query($query_entity_permission, array($value, $user_input['user_id']), $dbh);
			}
			
			//redirect to manage_users page and display success message
			$_SESSION['LA_SUCCESS'] = 'Examiner #'.$user_input['user_id'].' has been updated.';

			$ssl_suffix = la_get_ssl_suffix();						
			header("Location: edit_examiner.php?id=".$user_input['user_id']);
			exit;
		}
	}else{
		//populate user data
		...
		$entity_array = array();
		$query_entity = "SELECT DISTINCT `entity_id` FROM ".LA_TABLE_PREFIX."entity_examiner_relation WHERE `user_id` = ?";
          ...
          <li>
            <div id="au_box_user_profile" class="au_box_main gradient_blue">
              ...
              <div class="au_box_content" style="padding-bottom: 15px">
              	<label class="description">Examiner Photo</label>
                ...
                <label class="description <?php if(!empty($error_messages['user_name'])){ echo 'label_red'; } ?>" for="au_user_name">Name <span class="required">*</span> <img class="helpmsg" src="images/navigation/005499/16x16/Help.png" style="vertical-align: top" title="Full name of the examiner."/></label>
				...
				?>
                <label class="description <?php if(!empty($error_messages['user_phone'])){ echo 'label_red'; } ?>" for="au_user_phone">Phone Number <img class="helpmsg" src="images/navigation/005499/16x16/Help.png" style="vertical-align: top" title="Phone number of the examiner. Recommended format: +1-888-896-6207"/></label>
				...
				?>
				<label class="description" for="au_job_classification">Job Classification <img class="helpmsg" src="images/navigation/005499/16x16/Help.png" style="vertical-align: top" title="Job classification of the examiner."/></label>
                ...
                <label class="description" for="au_job_title">Job Title <img class="helpmsg" src="images/navigation/005499/16x16/Help.png" style="vertical-align: top" title="Job title of the examiner."/></label>
                <input id="au_job_title" name="au_job_title" class="element text large" value="<?php echo noHTML($user_input['job_title']); ?>" type="text">
                <label class="description" for="au_about_me">More Info <img class="helpmsg" src="images/navigation/005499/16x16/Help.png" style="vertical-align: top" title="More info of the examiner."/></label>
              ...
              <div class="au_box_content" style="padding-top: 10px;min-height: 90px;">
              	<select multiple="" id="entity_permission" name="entity_permission[]">
              		<?php
						...
						while ($row_entity = la_do_fetch_result($sth_entity)) {
							$selected = "";
							if(in_array($row_entity["client_id"], $entity_array)){
								$selected = "selected";
							}
						?>
							<option value="<?php echo $row_entity['client_id']; ?>" <?php echo $selected; ?>><?php echo $row_entity['company_name']; ?></option>
						<?php
						}
	                ?>
              	</select>
