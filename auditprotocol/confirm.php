 ...
 ********************************************************************************/
	...
	if(!empty($_POST['review_submit']) || !empty($_POST['review_submit_x'])){ //if form submitted
		...
		if($form_properties['payment_enable_merchant'] != 1){
			...
			if(empty($commit_result['form_redirect'])){
				header("Location: /auditprotocol/confirm.php?id={$form_id}&done=1");
				exit;
			}else{
				header("Location: /auditprotocol/confirm.php?id={$form_id}&done=1");
			...
			/*$query  = "select * from `".LA_TABLE_PREFIX."form_{$form_id}_review` where id=? limit 1";
	...
	elseif(!empty($_POST['review_back']) || !empty($_POST['review_back_x'])){
		//echo "Location: https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?".$_SESSION['casecade_back_session'][$_POST['id']][$_POST['la_page_from']];
		//echo '<pre>';print_r($_SESSION['casecade_back_session']);echo '</pre>';
		//echo '<pre>';print_r($_POST);echo '</pre>';die;
		//go back to form
		//echo $origin_page_num; exit;
