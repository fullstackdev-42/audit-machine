 ...
 ********************************************************************************/
	...
	$row_position			= (int) la_sanitize($_POST['position']); //the position of the new row within matrix table
	$matrix_parent_id		= (int) la_sanitize($_POST['matrix_parent_id']);
	$allow_multiselect		= (int) la_sanitize($_POST['allow_multiselect']);
	$row_holder_id			= la_sanitize($_POST['row_holder_id']);
	$prop_holder_id			= la_sanitize($_POST['prop_holder_id']);
	$total_column			= (int) la_sanitize($_POST['total_column']);
	$rows_titles			= la_sanitize($_POST['rows_titles']); //if this array exist, then bulk insert rows is happening
	$column_data			= la_sanitize($_POST['column_data']);
	...
	$element_properties['position'] 		= $row_position+1;
	...
	$prop_markup = <<<EOT
<li>
	<input type="text" id="matrixrow_{$element_id}" class="text" autocomplete="off" value="" style="width:110px;"> 
	<input type="text" id="matrix_policy_row_{$element_id}" class="text" autocomplete="off" value="" style="width:110px;"> 
