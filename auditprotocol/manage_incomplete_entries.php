 ...
 ********************************************************************************/	
	...
	$query = "select 
				...
				from 
					".LA_TABLE_PREFIX."forms A left join ".LA_TABLE_PREFIX."entries_preferences B 
				  on 
				  	A.form_id=B.form_id and B.user_id=? 
			   where 
			   		A.form_id = ?";
	$params = array($_SESSION['la_user_id'],$form_id);
	...
	if(empty($sort_by)){
		...
		$query = "select count(user_id) sort_count from ".LA_TABLE_PREFIX."entries_preferences where form_id=? and `user_id`=?";
		...
		if(!empty($sort_count)){ //update existing record
			$query = "update ".LA_TABLE_PREFIX."entries_preferences set entries_incomplete_sort_by = ? where form_id = ? and `user_id` = ?";
			$params = array($sort_by,$form_id,$_SESSION['la_user_id']);
			la_do_query($query,$params,$dbh);
		}else{ //insert new one
			$query = "insert into ".LA_TABLE_PREFIX."entries_preferences(`entries_incomplete_sort_by`,`form_id`,`user_id`) values(?,?,?)";
			$params = array($sort_by,$form_id,$_SESSION['la_user_id']);
			la_do_query($query,$params,$dbh);
		}
	...
	$query = "select element_name from ".LA_TABLE_PREFIX."column_preferences where form_id=? and user_id=? and incomplete_entries=1";
	$params = array($form_id,$_SESSION['la_user_id']);
	...
	$query = "SELECT COUNT(DISTINCT(company_id)) total_row FROM `".LA_TABLE_PREFIX."form_{$form_id}` WHERE `form_resume_enable`=1";	
	$params = array();
			...
			<div class="post manage_entries">
				...
				<div class="content_body">
					...
					<?php if($form_has_entries){ ?>
						...
						<div id="entries_actions" class="gradient_blue">
							<ul>
								...
								<li>
									<div style="border-left: 1px dotted #80b638;height: 35px;margin-top:5px"></div>
								</li>
								<li>
									<a id="entry_export" href="#"><span class="icon-file-download"></span>Export</a>
								</li>
							</ul>
							<img src="images/icons/30.png" style="position: absolute;left:5px;top:100%" />
						</div>
						<div id="entries_options" class="gradient_blue" data-formid="<?php echo $form_id; ?>">
							<ul>
								...
								<li>
									<a id="entry_filter" href="#"><!--<<span class="icon-binoculars"></span>--><img src="images/icons/find_16.png">  Filter Users</a> </li>
								</li>
							</ul>
						</div>
						...
						<?php 
							...
							$entries_options['column_preferences_user_id'] = $_SESSION['la_user_id'];
							
							$entries_options['form_resume_enable'] = 1;

							echo la_display_entries_table($dbh,$form_id,$entries_options); 
						?>
								...
								<optgroup label="Ascending">
									<?php 
										foreach ($columns_label as $element_name => $element_label) {
											...
											if($element_name != 'date_updated' && $element_name != 'ip_address'){
												if($element_name == 'date_created'){
													$element_name = 'date_created'; 
												}
