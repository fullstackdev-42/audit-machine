 ...
 ********************************************************************************/
	...
	if($verified === true){
		...
		$query = "select count(`id`) record_exist from ".LA_TABLE_PREFIX."form_{$form_id} where unix_timestamp(date_created) = ? and `id` = ? and `status` = 1";
		...
		if(!empty($error_message)){
			...
			$query = "select count(afp_id) record_exist from ".LA_TABLE_PREFIX."form_payments where form_id = ? and record_id = ? and `status` = 1";
			...
			if(!empty($row['record_exist'])){
				...
				$query = "UPDATE ".LA_TABLE_PREFIX."form_payments 
							 SET 
