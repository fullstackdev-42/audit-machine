		...
		if(!isset($input_array['password'])){ //if normal form submitted
			...
			if($submit_result['status'] === true){
				if(!empty($submit_result['form_resume_url'])){ //the user saving a form, display success page with the resume URL
					...
					header("Location: /auditprotocol/embed-nov-05-2015.php?id={$input_array['form_id']}&done=1");
					exit;
					...
					if(is_numeric($target_page_id)){
						header("Location: https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?id={$input_array['form_id']}&la_page={$target_page_id}");
						exit;
					...
					if(!empty($submit_result['next_page_number'])){ //redirect to the next page number
						...
						header("Location: /auditprotocol/embed-nov-05-2015.php?id={$input_array['form_id']}&la_page={$submit_result['next_page_number']}");
						exit;
