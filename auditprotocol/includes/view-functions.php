 ...
 ********************************************************************************/
	...
	function generateNoteIcon($param=array())
	{
		...
		if(strpos( $_SERVER['REQUEST_URI'], "edit_entry.php" ) !== false){//check if user entity or admin
			...
			if($row_entity) {//if user entity
				...
				$result = la_do_query($query, array(':form_id' => $param['form_id'], ':element_id' => $param['element_id'], ':company_id' => $company_id), $dbh);
				$row    = la_do_fetch_result($result);
				...
				$sth_notes = la_do_query($query_notes, array($param['form_id'], $param['element_id']), $dbh);
				...
				$query = "select `status` from `".LA_TABLE_PREFIX."form_element_note` where `form_id` = :form_id and `element_id` = :element_id and `admin_id` = :admin_id";
				$result = la_do_query($query, array(':form_id' => $param['form_id'], ':element_id' => $param['element_id'], ':admin_id' => $_SESSION['la_user_id']), $dbh);
				$row    = la_do_fetch_result($result);
				$note_status = $row["status"];
				//check assigned notes
				$assignee_id = $_SESSION['la_user_id'];
				$query_notes = "SELECT * FROM `".LA_TABLE_PREFIX."form_element_note` WHERE `form_id` = ? and `element_id` = ?";
				$sth_notes = la_do_query($query_notes, array($param['form_id'], $param['element_id']), $dbh);
		...
		if(strpos( $_SERVER['REQUEST_URI'], "view.php" ) !== false) {//if admin
			$query = "select `status` from `".LA_TABLE_PREFIX."form_element_note` where `form_id` = :form_id and `element_id` = :element_id and `admin_id` = :admin_id";
			$result = la_do_query($query, array(':form_id' => $param['form_id'], ':element_id' => $param['element_id'], ':admin_id' => $_SESSION['la_user_id']), $dbh);
			$row    = la_do_fetch_result($result);
			$note_status = $row["status"];
		...
		if(isset($param['is_design_mode']) && $param['is_design_mode'] == 1){
			...
			if(!$assigned_flag){
				$element_note = '<div><img id="img-'.$param['form_id'].'-'.$param['element_id'].'" class="element-note" data-form-id="'.$param['form_id'].'" data-element-id="'.$param['element_id'].'" data-company-id="'.$company_id.'" src="'.$img.'" style="cursor:pointer;"></div>';
			} else {
				$element_note = '<div><img id="img-'.$param['form_id'].'-'.$param['element_id'].'" class="element-note" data-form-id="'.$param['form_id'].'" data-element-id="'.$param['element_id'].'" data-company-id="'.$company_id.'" src="'.$img.'" style="cursor:pointer;"><img class="assigned-note" assignee-id="'.$assignee_id.'" role="'.$role.'" form-id="'.$param['form_id'].'" element-id="'.$param['element_id'].'" src="images/downarrow-orangex16.png" style="cursor:pointer;"></div>';
			}
		...
		if(isset($param['is_design_mode']) && $param['is_design_mode'] == 1){
			...
			if($param['element_status_indicator'] == 1){
				...
				if($row_indicator){
					...
					$sth_indicator = la_do_query($insert_indicator, array($param['form_id'], $param['element_id'], $param['company_id'], $param['entry_id'], 0), $dbh);
					$img_name = "Circle_Gray.png";
				}
				...
				$status_preserve_green = "<input type='hidden' id='".$param["form_id"]."_".$param["element_id"]."_status_preserve_green' name='status_preserve_green[".$param["form_id"]."_".$param["element_id"]."]'/ >";
				$img.= $status_preserve_green;
			}
		}
	...
	function viewCasecadeForm($params=array())
	{
		...
		while($row = la_do_fetch_result($sth)){
			...
			//echo '<pre>';print_r($element[$j]->populated_value);echo '</pre>';die;
	...
	function la_display_casecade_form($element){		
        ...
        //$element->title = empty($element->guidelines) ? str_replace(array('<strong>', '</strong>'), array('', ''), $element->title) :  str_replace(array('<strong>', '</strong>'), array('', ''), $element->guidelines);
		
		$element->title = empty($element->guidelines) ? $element->title : $element->guidelines;
...
EOT;
	...
	function la_display_keystone_viewer($dbh, $form_id, $element) {
		$server_url = 'https://'.$_SERVER['HTTP_HOST'].'/auditprotocol/includes/keystone-viewer.php';
		$result = <<<EOT
			<script>
	...
	function la_display_text($element, $keystone_viewer='', $casecade=false){
		...
		// Generate Note Html
		$noteHtml = generateNoteIcon(array('form_id' => $element->form_id, 'element_id' => $element->id, 'company_id' => $element->company_id, 'element_note' => $element->element_note, 'is_design_mode' => (isset($element->is_design_mode) ? $element->is_design_mode : 0)));
...
$element_markup = <<<EOT
		...
		{$keystone_viewer}
		<div style="width:100%; float:left;"><label style="{$label_styles}" class="description test-cascade" for="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}">{$element->title} {$span_required} {$statusIndicatorHtml}</label></div>
		<div>
...
$element_markup = <<<EOT
		<li id="li_{$element->id}" {$li_class}>
		{$keystone_viewer}
		<div style="width:100%; float:left;"><label style="{$label_styles}" class="description test-not-cascade" for="element_{$element->id}">{$element->title} {$span_required} {$statusIndicatorHtml}</label></div>
		<div>
			<input {$element_machine_code_html} {$element_machine_data_html} id="element_{$element->id}" name="element_{$element->id}" {$maxlength} class="element text {$element->size} socket-enabled" type="{$element_type}" value="{$element->default_value}"  {$input_handler} /> {$range_limit_markup} 
		</div>{$noteHtml}{$guidelines} {$error_message}
		</li>
EOT;
		}
...
function la_display_digital_signature($dbh, $form_id){
	...
	$start = $_REQUEST['entry_id'] ? $_REQUEST['entry_id'] - 1 : 0;
	...
	$sth = la_do_query($query, array($row1['company_id'], $form_id), $dbh);
	...
	if (isset($res)) {
		...
		$sth = la_do_query($query, array($cur_signer_id), $dbh);
		$res = la_do_fetch_result($sth);
		$cur_signer_full_name = $res['full_name'];
		}

	$digital_signature_img_link = "https://".$_SERVER['HTTP_HOST'].'/portal/digital_signature_img.php?q=';
	// var_dump($digital_signature_img_link, $cur_signed_signature_id);exit;
	if (isset($cur_signed_signature_id)) {
		...
		<div class="digital_signature_btn">
				<div style="margin-top: 30px; padding: 20px; height: 130px; width: 250px; border: 1px dashed #8EACCF;">
					<img src="{$digital_signature_img_link}{$cur_signed_signature_id}" width="200" height="100"/>
					<div style="float: right">Signed by {$cur_signer_full_name}</div>
					<div style="float: right">Signed at {$cur_sign_date}</div>
				</div>
				</div>
		EOT;
	} else {
	...
	if (isset($signature_id)) {
		$registered_signature_image = <<<EOT
			<div class="registered_signature_image" style="margin: 40px">
				<p>Signature Image:</p>
				<div style="margin-top: 30px; padding: 20px; height: 130px; width: 250px; border: 1px dashed #8EACCF;">
					<img src="{$digital_signature_img_link}{$signature_id}" width="200" height="100"/>
					<div style="float: right">Signed by {$cur_signer_full_name}</div>
					<div style="float: right">Signed at yyyy-mm-dd hh:mm:ss</div>
				</div> 
			</div>
		EOT;
	} else {
		$registered_signature_image = <<<EOT
			<div class="registered_signature_image" style="margin: 40px">
				<p>Signature Image:</p>
				<div style="margin-top: 30px; padding: 20px; height: 130px; width: 250px; border: 1px dashed #8EACCF;">
					<img src="data:image/png;base64," width="200" height="100"/>
					<div style="float: right">Signed by xxx xxx</div>
					<div style="float: right">Signed at yyyy-mm-dd hh:mm:ss</div>
				</div> 
			</div>
		EOT;
	...
	$signature_image_shown_in_dialog = isset($cur_signed_signature_id) 
		? $cur_signature_image 
		: $registered_signature_image;
	$sign_or_delete_btn = isset($cur_signed_signature_id) 
		? '<button id="del_sign_button" class="bb_button bb_red">Delete</button>'
		: '<button id="sign_button" class="bb_button bb_green">Click to Sign</button>';

	$digital_signature_api_url = "https://".$_SERVER['HTTP_HOST']."/portal/digital_signature.php";
	...
	$element_markup = <<<EOT
		...
		<div id="digital-signature-dialog" style="text-align:left; display: none">
			<div id="first">
				...
				{$signature_image_shown_in_dialog}
	...
	function la_display_signature($element, $casecade=false){
		...
		if(!empty($element->is_design_mode)){
	        ...
	        <div class="la_sig_wrapper {$element->size}">
	          ...
	          <input type="hidden" element_machine_code="{$element->element_machine_code}" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}]" id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}">
	        </div>
...
EOT;
	        ...
	        <div class="la_sig_wrapper {$element->size}">
	          <canvas class="la_canvas_pad" width="309" height="{$canvas_height}"></canvas>
	          <input type="hidden" element_machine_code="{$element->element_machine_code}" name="element_{$element->id}" id="element_{$element->id}">
	        </div>
...
EOT;
	...
	//File Upload
	function la_display_file_synced($element, $dbh, $casecade=false){
...
EOT;
					if(in_array(strtolower($fileExtension), array('png', 'bmp', 'gif', 'jpg', 'jpeg'))){
						...
						$startAnc = '<a class="entry_link entry-link-preview" href="#" data-identifier="image_format" data-ext="'.$fileExtension.'" data-src="'.$filename_path.'">';
						$endAnc = '</a>';
						...
						$startAnc = '<a class="entry_link entry-link-preview" href="#" data-identifier="other" data-ext="'.$fileExtension.'" data-src="'.$q_string.'">';
						$endAnc = '</a>';
					}
...
EOT;
	...
	//File Upload
	function la_display_file($element, $casecade=false){
		global $la_lang;
		$la_settings = la_get_settings(la_connect_db());
...
EOT;
	...
	//Date - Normal
	function la_display_date($element, $keystone_viewer='', $casecade=false){
		//echo '<pre>';print_r($element);echo '</pre>';die;
		global $la_lang;
...
$element_markup = <<<EOT
		...
		<span id="calendar_{$element->parent_form_id}_{$element->form_id}_{$element->id}">
		    <input type="hidden"  element_machine_code="{$element->element_machine_code}" value="" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_datepick]" id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_datepick">
...
$element_markup = <<<EOT
		...
		<span id="calendar_{$element->id}">
		    <input type="hidden"  element_machine_code="{$element->element_machine_code}" value="" name="element_{$element->id}_datepick" id="element_{$element->id}_datepick">
...
$element_markup = <<<EOT
		...
		<span id="calendar_{$element->parent_form_id}_{$element->form_id}_{$element->id}">
			<input type="hidden" value=""  element_machine_code="{$element->element_machine_code}" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_datepick]" id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_datepick">
			<div style="{$cal_img_style}"><img id="cal_img_{$element->parent_form_id}_{$element->form_id}_{$element->id}" class="datepicker" src="{$itauditmachine_path}images/calendar.gif" alt="Pick a date." /></div>	
		</span>
		{$calendar_script}
		{$guidelines} {$error_message}<div style="clear: both;">{$noteHtml}</div>
...
$element_markup = <<<EOT
		...
		<span id="calendar_{$element->id}">
			<input type="hidden" value=""  element_machine_code="{$element->element_machine_code}" name="element_{$element->id}_datepick" id="element_{$element->id}_datepick">
			<div style="{$cal_img_style}"><img id="cal_img_{$element->id}" class="datepicker" src="{$itauditmachine_path}images/calendar.gif" alt="Pick a date." /></div>	
		</span>
		{$calendar_script}
		{$guidelines} {$error_message}<div style="clear: both;">{$noteHtml}</div>
		...
		if(!empty($element->choice_has_other)){
			...
			if($casecade) {
				...
				if(isset($_REQUEST['entry_id'])){	
					$start = $_REQUEST['entry_id'] - 1;
					$query1  = "SELECT DISTINCT(company_id) FROM  `".LA_TABLE_PREFIX."form_{$element->form_id}` LIMIT ".$start.",1";
					$params1 = array();
				...
				$sth_check_existing_value = la_do_query($query_check_existing_value, array("element_".$element->id, $company_id), $dbh);
				...
				if(empty($res_check_existing_value["data_value"])) {
					...
					$sth_other_value = la_do_query($query_other_value, array("element_".$element->id."_other", $company_id), $dbh);
...
EOT;
	...
	$element_markup = <<<EOT
		<li id="li_{$element->id}" {$price_data_tag} {$li_class} {$input_type}>
		{$keystone_viewer}
		<div><label style="float:none;{$label_styles}" class="description">{$element->title} {$span_required} {$statusIndicatorHtml}</label></div>
...
$element_markup = <<<EOT
		...
		<span class="simple_name_2">
			<input {$element_machine_code_html} {$element_machine_data_html} {$field_type}  data-field_sub_type="lastname" id="element_{$element->id}_2" name="element_{$element->id}_2" data-field_sub_type="lastname" type="text" class="element text name-ele-ment socket-enabled" maxlength="255" size="14"  element_machine_code="{$element->element_machine_code}" value="{$default_value_2}" />
...
$element_markup = <<<EOT
		...
		<span class="simple_name_wmiddle_1">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_1" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_1]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text" maxlength="255" size="8" value="{$default_value_1}" />
			<label for="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_1">{$la_lang['name_first']}</label>
		</span>
		<span class="simple_name_wmiddle_2">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_2" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_2]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text" maxlength="255" size="8" value="{$default_value_2}" />
		...
		<span class="simple_name_wmiddle_3">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_3" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_3]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text" maxlength="255" size="14" value="{$default_value_3}" />
...
$element_markup = <<<EOT
		...
		<span class="simple_name_wmiddle_1">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" type="text" class="element text" maxlength="255" size="8"  element_machine_code="{$element->element_machine_code}" value="{$default_value_1}" />
			<label for="element_{$element->id}_1">{$la_lang['name_first']}</label>
		</span>
		<span class="simple_name_wmiddle_2">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" type="text" class="element text" maxlength="255" size="8"  element_machine_code="{$element->element_machine_code}" value="{$default_value_2}" />
			<label for="element_{$element->id}_2">{$la_lang['name_middle']}</label>
		</span>
		<span class="simple_name_wmiddle_3">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" type="text" class="element text" maxlength="255" size="14"  element_machine_code="{$element->element_machine_code}" value="{$default_value_3}" />
...
$element_markup = <<<EOT
		...
		<span class="fullname_1">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_1" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_1]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text" maxlength="255" size="2" value="{$default_value_1}" />
		...
		<span class="fullname_4">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_4" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_4]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text" maxlength="255" size="3" value="{$default_value_4}" />
...
$element_markup = <<<EOT
		...
		<span class="fullname_1">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" type="text" class="element text" maxlength="255" size="2"  element_machine_code="{$element->element_machine_code}" value="{$default_value_1}" />
		...
		<span class="fullname_4">
			<input id="element_{$element->id}_4" name="element_{$element->id}_4" type="text" class="element text" maxlength="255" size="3" element_machine_code="{$element->element_machine_code}"  value="{$default_value_4}" />
...
$element_markup = <<<EOT
		...
		<span class="namewm_ext">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_1" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_1]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text large" maxlength="255" value="{$default_value_1}" />
			<label for="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_1">{$la_lang['name_title']}</label>
		</span>
		<span class="namewm_first">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_2" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_2]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text large" maxlength="255" value="{$default_value_2}" />
			<label for="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_2">{$la_lang['name_first']}</label>
		</span>
		<span class="namewm_middle">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_3" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_3]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text large" maxlength="255" value="{$default_value_3}" />
		...
		<span class="namewm_last">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_4" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_4]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text large" maxlength="255" value="{$default_value_4}" />
			<label for="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_4">{$la_lang['name_last']}</label>
		</span>
		<span class="namewm_ext">
			<input id="element_{$element->parent_form_id}_{$element->form_id}_{$element->id}_5" name="{$element->parent_form_id}_{$element->form_id}[element_{$element->id}_5]" type="text"  element_machine_code="{$element->element_machine_code}" class="element text large" maxlength="255" value="{$default_value_5}" />
...
$element_markup = <<<EOT
		...
		<span class="namewm_ext">
			<input id="element_{$element->id}_1" name="element_{$element->id}_1" type="text" class="element text large" maxlength="255"  element_machine_code="{$element->element_machine_code}" value="{$default_value_1}" />
			<label for="element_{$element->id}_1">{$la_lang['name_title']}</label>
		</span>
		<span class="namewm_first">
			<input id="element_{$element->id}_2" name="element_{$element->id}_2" type="text" class="element text large" maxlength="255"  element_machine_code="{$element->element_machine_code}" value="{$default_value_2}" />
			<label for="element_{$element->id}_2">{$la_lang['name_first']}</label>
		</span>
		<span class="namewm_middle">
			<input id="element_{$element->id}_3" name="element_{$element->id}_3" type="text" class="element text large" maxlength="255"  element_machine_code="{$element->element_machine_code}" value="{$default_value_3}" />
			<label for="element_{$element->id}_3">{$la_lang['name_middle']}</label>
		</span>
		<span class="namewm_last">
			<input id="element_{$element->id}_4" name="element_{$element->id}_4" type="text" class="element text large" maxlength="255"  element_machine_code="{$element->element_machine_code}" value="{$default_value_4}" />
			<label for="element_{$element->id}_4">{$la_lang['name_last']}</label>
		</span>
		<span class="namewm_ext">
			<input id="element_{$element->id}_5" name="element_{$element->id}_5" type="text" class="element text large" maxlength="255"  element_machine_code="{$element->element_machine_code}" value="{$default_value_5}" />
...
$element_markup = <<<EOT
		...
		<span>
			<input {$element_machine_code_html} {$element_machine_data_html} {$field_type} data-field_sub_type="time_hh" id="element_{$element->id}_1" name="element_{$element->id}_1" class="element text socket-enabled" size="2" type="text" maxlength="2" value="{$default_value_1}" /> : 
			<label for="element_{$element->id}_1">{$la_lang['time_hh']}</label>
		</span>
		<span>
			<input {$element_machine_code_html} {$element_machine_data_html} {$field_type} data-field_sub_type="time_mm" id="element_{$element->id}_2" name="element_{$element->id}_2" class="element text socket-enabled" size="2" type="text" maxlength="2" value="{$default_value_2}" /> {$seconds_separator} 
...
EOT;
	...
	function la_display_section($element, $casecade=false){
		...
		$ck_element_id_h3_class = 'class="section_break_h3_no"';
		$ck_element_id_p_class = 'class="section_break_p_no"';
		$contentEditable = '';
		
		if(!empty($element->is_design_mode)){
			$ck_element_id_h3_class = 'class="section_break_h3"';
			$ck_element_id_p_class = 'class="section_break_p"';
			$contentEditable = '';
		}
        ...
        if($casecade){
			...
			$ck_element_id_p = "";
			if(!empty($element->is_design_mode)){
				$ck_element_id_h3 = "id=\"sectionbreakh3-li_{$element->parent_form_id}_{$element->form_id}_{$element->id}\"";
				$ck_element_id_p = "id=\"sectionbreakp-li_{$element->parent_form_id}_{$element->form_id}_{$element->id}\"";
			}
		
$element_markup = <<<EOT
		<li id="li_{$element->parent_form_id}_{$element->form_id}_{$element->id}" {$li_class}>
			<div style="width:100%;" {$contentEditable} {$ck_element_id_h3} {$ck_element_id_h3_class}>{$element->title}</div>
			<div style="width:100%;" {$contentEditable} {$ck_element_id_p} {$ck_element_id_p_class}>{$element->guidelines}</div>
		</li>
EOT;
		}else{
			$ck_element_id_h3 = "";
			$ck_element_id_p = "";
			if(!empty($element->is_design_mode)){
				$ck_element_id_h3 = "id=\"sectionbreakh3-li_{$element->id}\"";
				$ck_element_id_p = "id=\"sectionbreakp-li_{$element->id}\"";
			}
				...
				<table class="ap_table_pagination" width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td align="left" style="vertical-align: bottom">
							<input type="submit" disabled="disabled" element_machine_code="{$element->element_machine_code}" value="{$element->submit_primary_text}" id="btn_submit_{$element->id}" name="btn_submit_{$element->id}" class="btn_primary btn_submit{$btn_class}">
							<input type="submit" disabled="disabled" element_machine_code="{$element->element_machine_code}" value="{$element->submit_secondary_text}" id="btn_prev_{$element->id}" name="btn_prev_{$element->id}" class="btn_secondary btn_submit{$btn_class}">
							<input type="image" disabled="disabled" element_machine_code="{$element->element_machine_code}" src="{$element->submit_primary_img}" alt="Continue" value="Continue" id="img_submit_{$element->id}" name="img_submit_{$element->id}" class="img_primary img_submit{$image_class}">
							<input type="image" disabled="disabled" element_machine_code="{$element->element_machine_code}" src="{$element->submit_secondary_img}" alt="Previous" value="Previous" id="img_prev_{$element->id}" name="img_prev_{$element->id}" class="img_secondary img_submit{$image_class}">
						</td> 
...
EOT;
	...
	function la_display_matrix($element, $casecade=false){
		...
		// Generate Note Html
		$noteHtml = generateNoteIcon(array('form_id' => $element->form_id, 'element_id' => (!$element->id ? $element->matrix_parent_id : $element->id), 'company_id' => $element->company_id, 'element_note' => $element->element_note, 'is_design_mode' => (isset($element->is_design_mode) ? $element->is_design_mode : 0)));
...
<input  element_machine_code="{$element->element_machine_code}" id="captcha_response_field" name="captcha_response_field" class="element text small" type="text" /><div id="dummy_captcha_internal"></div>
EOT;
...
<input  element_machine_code="{$element->element_machine_code}" id="captcha_response_field" name="captcha_response_field" class="element text small" type="text" />
...
EOT;
	...
	function getFormCss($dbh){
		$sql = "select `form_view_css` from `".LA_TABLE_PREFIX."settings`";
		$res = la_do_query($sql,array(),$dbh);
		$row = la_do_fetch_result($res);
 		
		
		return '@import "/auditprotocol/view_form_default.css"';
		
		
//		return $row['form_view_css'];
	...
	function la_display_form($dbh, $form_id, $company_id, $entry_id, $form_params=array(), $showSubmit=true){
		...
		$form_view_css = getFormCss($dbh);
		$user_data = getUserDetailsFromId($dbh, $_SESSION['la_user_id']);
		...
		if(isset($_GET['casecade_form_page_number']) && $isCasecadeForm){
		 	$casecade_form_page_number = $_GET['casecade_form_page_number'];
		 	$casecade_element_position = $_GET['casecade_element_position'];
		...
		$switch_column_btn = '<div class="d-none d-lg-block" style="padding: 9px">
			<input id="switch-form-responsive" type="checkbox">
			<label for="switch-form-responsive">Responsive</label>
		</div>';
		$all_element_markup = '<div class="row m-0 all-elements"><div class="first-column col-12 p-0">';
		...
		foreach ($element as $key => $element_data){
			...
			if (($element_counts % 2 == 0 && $key == intval($element_counts/2)) || ($element_counts % 2 == 1 && $key == intval($element_counts/2) + 1)) {
				$all_element_markup .= <<<EOT
					...
					<div class="second-column col-12 p-0">
				EOT;
			}
			if($element_data->is_private && empty($edit_id)){ //don't show private element on live forms
				//continue;
			...
			if($element_data->type == 'casecade_form'){
				...
				$casecade_error_elements = isset($error_elements['casecade'][$element_data->default_value]) ? $error_elements['casecade'][$element_data->default_value] : array();
		 		...
		 		$view_cascade_form_array = [];
 		 		$view_cascade_form_array = array('dbh' => $dbh, 'form_id' => $element_data->default_value, 'parent_form_id' => $form_id, 'company_id' => $company_id, 'entry_id' => $entry_id, 'form_params' => array('populated_values' => $populated_values['element_'.$element_data->id][$element_data->default_value], 'error_elements' => $casecade_error_elements), 'parent_form_page_number' => $element_data->element_page_number, 'element_position' => $element_data->position, 'page_number' => (!$casecade_form_page_number ? 1 : $casecade_form_page_number));

				if( (isset($form_params['called_from'])) && ($form_params['called_from'] == 'edit_entry') ) {
					$view_cascade_form_array['called_from'] = 'edit_entry';
				}

 				$all_element_markup .= viewCasecadeForm($view_cascade_form_array);
 		
				$casecade_form_page_number = isset($_REQUEST['casecade_form_page_number']) ? $_REQUEST['casecade_form_page_number'] : 1;
	
 				$casecade_element_position = $element_data->position;
			
 				if( !isset($_GET['casecade_form_page_number']) ){ 
					break;
				}elseif(($total_pages+1) != $casecade_form_page_number && $_GET['casecade_form_page_number'] != 'NO_ELEMENTS'){ 
					break;	
				}elseif($_GET['casecade_form_page_number'] == 'NO_ELEMENTS'){ 
		...
		if(!empty($form->theme_id) && isset($_SESSION['admin'])){
		...
		//if called from edit_entry.php never show password form
		if( (isset($form_params['called_from'])) && ($form_params['called_from'] == 'edit_entry') )
			$show_password_form = false;
		...
		$button_markup_top =<<<EOT
			<li id="li_buttons_top" class="buttons">
		...
		if($form->page_total > 1 && $show_password_form === false || count($casecade_form_id_arr) && $show_password_form === false){
			...
			//echo '<pre>';print_r($page_breaks_data);echo '</pre>';
			
			// cascade pagination
 
			  
		
			if($isCasecadeForm && count($casecade_form_id_arr)){
			...
			// echo '<pre>';print_r($page_title_array);echo '</pre>';
			
			/*if(!count($page_breaks_data))
				continue;*/
			...
			if($form->pagination_type == 'steps'){
				...
				$pagination_header =<<<EOT
				<li id="pagination_header" class="li_pagination" style="background-color:#fffff;">
...
//				 		$page_number = $page_number - $_GET['casecade_form_page_number']; SOMETHING LIKE THIS
		...
		if(empty($la_settings['disable_itauditmachine_link'])){
			$powered_by_markup = 'Powered by ITAM, the <a href="https://continuumgrc.com/" target="_blank">IT Audit Machine</a>';
		}else{
			$powered_by_markup = '';
		}
		...
		$socket_scripts = '';
		$textare_socket_logic = '';
		$socket_connection_error_html = '';
		if( $form_params['called_from'] != 'view' && $form->form_enable_auto_mapping == 1) {
			$socket_scripts =<<<EOT
			...
			$socket_connection_error_html =<<<EOT
				<div class="bootstrap-alert bootstrap-alert-notification" id="socket-connection-error" role="alert">
					Connecting to sockets for automapping..
				</div>
			EOT;
		}
		  else {
		...
		$disabled_all_elements_js = "";

		if($integration_method == 'php'){
			if(!empty($edit_id) && $_SESSION['la_logged_in'] === true){
				$view_css_markup = '<link rel="stylesheet" type="text/css" href="css/edit_entry.css" media="all" />';
			}else{
				//$view_css_markup = "<link rel=\"stylesheet\" type=\"text/css\" href=\"{$itauditmachine_path}{$css_dir}view.css\" media=\"all\" />";
				$view_css_markup = $form_view_css;
			}
            ...
            if(isset($_SESSION['admin']) && $_SESSION['admin'] == true){
            	...
            	$form_markup = <<<EOT
					...
					<!--<link rel="stylesheet" type="text/css" href="{$itauditmachine_path}{$css_dir}view.css" media="all" />-->
					<style type="text/css">
						{$form_view_css}
					</style>
					...
					<script type="text/javascript" src="{$itauditmachine_path}js/jquery.smartmarquee.js"></script>
					...
					<body id="main_body" class="{$container_class}">
						<div id="form_container" class="{$form_container_class}">
							...
							<form id="form_{$form->id}" class="itauditm {$form->label_alignment}" {$form_enc_type} method="post" data-highlightcolor="{$field_highlight_color}" action="#main_body" data-useremail="{$user_email}" data-formid="{$form->id}" data-domain="{$_SERVER['HTTP_HOST']}">
							<div style="display:none;">
								<input type="hidden" name="post_csrf_token" value="{$post_csrf_token}" />
							</div>
								{$form_desc_div}						
								<ul class="{$ul_class}" style="background-color:{$page_background}">
								{$pagination_header}
								{$form->error_message}
			...
			$form_markup = <<<EOT
				...
				{$custom_script_js}
				<style>
				#main_body form li span label { display: initial; } .address label { display: block !important; }
				</style>
				...
				<body id="main_body" class="{$container_class}">
					<div id="form_container" class="{$form_container_class}">
						{$css_theme_content}
						<form id="form_{$form->id}" data-selected_entity_id="{$company_id}" entity_id="{$form->form_for_selected_company}" data-selected_entry_id="{$entry_id}" class="itauditm {$form->label_alignment}" {$form_enc_type} method="post" data-highlightcolor="{$field_highlight_color}" action="#main_body" data-useremail="{$user_email}" data-formid="{$form->id}"  data-domain="{$_SERVER['HTTP_HOST']}">
							<div style="display:none;">
								<input type="hidden" name="post_csrf_token" value="{$post_csrf_token}" />
							</div>
							{$form_desc_div}
	...
	function la_display_raw_form($dbh,$form_id){
		...
		foreach ($element as $element_data){
            ...
            //$aaa[$ko] = array();
            //$aaa[$ko] = array('type' => $element_data->type, 'li' => call_user_func('la_display_'.$element_data->type,$element_data));
            //array_push($aaa, array('type' => $element_data->type, 'li' => call_user_func('la_display_'.$element_data->type,$element_data));
            
			if( !empty( $element_data->title ) ) {
            	$new_title = new DOMDocument();
				$encoded = mb_convert_encoding($element_data->title, 'HTML-ENTITIES', "UTF-8");
				@$new_title->loadHTML($encoded);
				$element_data->title = $new_title->saveHTML();
            }
            if($element_data->type == 'syndication'){
                $all_element_markup .= call_user_func('la_display_'.$element_data->type,$element_data,false,$dbh);
            }else{
        ...
        //echo '<pre style="color:#000">';print_r($aaa);echo '</pre>';exit();
		...
		$form_markup =<<<EOT
			<div id="main_body" class="{$container_class}">
				...
				<div id="form_container">
					...
					<h1><a>{$form->name}</a></h1>
					<form id="form_builder_preview" class="itauditm {$form->label_alignment}" method="post" action="#main_body">
						{$form_desc_div}				
						<ul class="{$ul_class}" id="form_builder_sortable" title="Click field to edit. Drag to reorder.">
						{$pagination_header}	
						{$all_element_markup}
						{$pagination_footer}
						</ul>
...
EOT;
		...
		if($integration_method == 'php'){
			$form_markup = <<<EOT
<!--<link rel="stylesheet" type="text/css" href="{$itauditmachine_path}{$css_dir}view.css" media="all" />-->
<link rel="stylesheet" type="text/css" href="{$itauditmachine_path}view_form_default.css" media="all" />
<style type="text/css">
	{$form_view_css}
</style>
{$theme_css_link}
...
<div id="main_body" class="integrated">
	<div id="form_container">
		<h1 class="form-head-h1"><a class="form-head-h1-a">Continuum GRC IT Audit Machine</a></h1>
...
EOT;
            ...
            if(isset($_SESSION['la_user_id'])){
            	$form_success_markup = '<div><a href="manage_forms.php">Click here</a> to go to My Forms</div>';
            ...
            if($row[$key_new]['element_type'] != 'page_break'){            
                if($row[$key_new]['element_type'] == 'checkbox'){
                    ...
                    if($entry_details[$key]['default_value'] == 1){
                        ...
                        /*$entry_detail .= '<img src="images/icons/checkbox_16.gif" />'.$row1['option'].' ';
...
<div id="main_body" class="integrated">
	<div id="form_container">
		<form id="form_{$form->id}" class="itauditm" method="post" action="{$self_address}">
          <div style="display:none;">
            <input type="hidden" name="post_csrf_token" value="{$_SESSION['csrf_token']}" />
          </div>
		    <div class="form_description">
