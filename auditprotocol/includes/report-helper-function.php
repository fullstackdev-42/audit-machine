		...
		$entry_details_query = '
								...
								ON  e.element_id = si.element_id AND e.form_id = si.form_id AND si.company_id = ? AND si.entry_id = ?
									...
									order by
										...
										e.element_position
								';
		$entry_details = la_do_query( $entry_details_query, array($company_id, $entry_id, $temp_form_id), $dbh );

		while($data = la_do_fetch_result($entry_details)) {
			# STATUS
			if ( in_array( $data['element_type'],
				...
				if ( isset( $data['indicator'] ) && $data['indicator'] === '0' ) {
					$cntStatusArr[0]++;
					$statusElementArr[$temp_form_id."_".$data["element_id"]]['field_link'] = "edit_entry.php?form_id=".$temp_form_id."&company_id=".$company_id."&entry_id=".$entry_id."&la_page=".$data["element_page_number"]."&element_id_auto=".$data["id"];
	...
	foreach ($form_ids as $top_form_id) {
		...
		while ($row_note = la_do_fetch_result($sth_note)) {
			if(!in_array($row_note["form_element_note_id"], $noteIdArr)) {
				...
				//get field links
				if(isset($row_note['company_id'])) {
					$entry_id = 0;
					$temp_entry_id = 0;
					$query_entry_id = "SELECT DISTINCT(company_id) FROM `".LA_TABLE_PREFIX."form_{$top_form_id}`";
					$sth_entry_id = la_do_query($query_entry_id, array(), $dbh);
					while($row_entry = la_do_fetch_result($sth_entry_id)) {
						$temp_entry_id += 1;
						if ($row_entry["company_id"] == $row_note["company_id"]) {
							$entry_id = $temp_entry_id;
						}
					}
					if($entry_id != 0) {
						$field_link = "/edit_entry.php?form_id=".$top_form_id."&entry_id=".$entry_id."&la_page=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
					} else {
						$field_link = "/view.php?id=".$top_form_id."&la_page=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
					}
				} else {
					$field_link = "/view.php?id=".$top_form_id."&la_page=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
				}
				array_push($field_notes, array("form_id" => $form_id, "form_name" => $form_name, "element_id" => $row_note["element_id"], "element_title" => $element_title, "note_id" => $row_note["form_element_note_id"], "note" => $row_note["note"], "assigner" => $assigner, "assignees" => $assignees, "field_link" => $field_link, "status" => $row_note["status"], "date_created" => $row_note["create_date"]));
			}
		}
			...
			while ($row_note = la_do_fetch_result($sth_note)) {
				if(!in_array($row_note["form_element_note_id"], $noteIdArr)) {
					...
					if(isset($row_note['company_id'])) {
						...
						if($entry_id != 0) {
							$field_link = "/edit_entry.php?form_id=".$top_form_id."&entry_id=".$entry_id."&la_page=".$row_cascade_form["element_page_number"]."&casecade_element_position=".$row_cascade_form["element_position"]."&casecade_form_page_number=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
						} else {
							$field_link = "/view.php?id=".$top_form_id."&la_page=".$row_cascade_form["element_page_number"]."&casecade_element_position=".$row_cascade_form["element_position"]."&casecade_form_page_number=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
						}
					} else {
						$field_link = "/view.php?id=".$top_form_id."&la_page=".$row_cascade_form["element_page_number"]."&casecade_element_position=".$row_cascade_form["element_position"]."&casecade_form_page_number=".$res_element["element_page_number"]."&element_id_auto=".$res_element["id"];
					}
...
function getTemplateCodes($dbh, $form_ids) {
	$res = array();
	if(count($form_ids) > 0) {
		$form_id_string = implode(",", $form_ids);
		$query = "SELECT `form_id`, `element_title`, `element_machine_code`, `element_type` FROM `".LA_TABLE_PREFIX."form_elements` WHERE `form_id` IN ({$form_id_string}) AND `element_status` = 1 AND `element_machine_code` != ''";
		$sth = la_do_query($query, array(), $dbh);
		while($row = la_do_fetch_result($sth)) {
			switch ($row["element_type"]) {
				case 'text'             : $element_type = 'Text';break;
				case 'textarea'         : $element_type = 'Paragraph';break;
				case 'name'             : $element_type = 'Name';break;
				case 'simple_name'      : $element_type = 'Name';break;
				case 'address'          : $element_type = 'Address';break;
				case 'phone'            : $element_type = 'Phone';break;
				case 'simple_phone'     : $element_type = 'Phone';break;
				case 'email'            : $element_type = 'Email';break;
				case 'time'             : $element_type = 'Time';break;
				case 'date'             : $element_type = 'Date';break;
				case 'europe_date'      : $element_type = 'Date';break;
				case 'number'           : $element_type = 'Number';break;
				case 'money'            : $element_type = 'Price';break;
				case 'url'              : $element_type = 'Web Site';break;
				case 'radio'            : $element_type = 'Multiple Choice';break;
				case 'checkbox'         : $element_type = 'Checkboxes';break;
				case 'select'           : $element_type = 'Drop Down';break;
				case 'matrix'           : $element_type = 'First Question';break;
				case 'file'             : $element_type = 'Upload a File';break;
				case 'signature'        : $element_type = 'Draw your signature into the box below.';break;
				case 'section'          : $element_type = 'Section Break';break;
				case 'page_break'       : $element_type = 'Page Break';break;
				case 'casecade_form'    : $element_type = 'Cascade Form';break;
				case 'syndication'      : $element_type = 'Syndication';break;
				case 'video_player'     : $element_type = 'Media Player';break;
			}
			array_push($res, array("form_id" => $row["form_id"], "element_title" => $row["element_title"], "element_machine_code" => $row["element_machine_code"], "element_type" => $element_type));
		}
	}
