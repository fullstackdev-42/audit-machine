 ...
 ********************************************************************************/
	...
	/*require ('../../policymachine/dompdf/lib/html5lib/Parser.php');
	require ('../../policymachine/dompdf/lib/php-font-lib/src/FontLib/Autoloader.php');
	require ('../../policymachine/dompdf/lib/php-svg-lib/src/autoload.php');
	require ('../../policymachine/dompdf/src/Autoloader.php');*/
...
EOD;
	...
	function lockForm($params=array()){
		$query = "SELECT `isFormLocked` FROM `".LA_TABLE_PREFIX."form_editing_locked` WHERE `form_id` = ? AND `entity_user_id` = ? AND `isFormLocked` = '1' AND `entity_id` = ?;";
		$result = la_do_query($query, array($params['form_id'], $params['entity_user_id'], $params['entity_id']), $params['dbh']);
		
		if(!$row = la_do_fetch_result($result)){
			$query = "INSERT INTO `".LA_TABLE_PREFIX."form_editing_locked` (`locked_id`, `form_id`, `entity_id`,`entity_user_id`, `isFormLocked`, `lockDateTime`, `isAdmin`) VALUES (NULL, ?,?, ?, '1', ?, 1);";
	...
	function unlockForm($params=array()){
		$query = "UPDATE `".LA_TABLE_PREFIX."form_editing_locked` SET `isFormLocked` = '0', `formUnlockedBy` = ?, `unlockDateTime` = ?, `formUnlockedByAdmin` = 1 WHERE `form_id` = ? AND `entity_user_id` = ? AND `isDeleted` = '0' AND `entity_id` = ?;";
	...
	function deleteLock($params=array()){
		...
		la_do_query($query, array($params['form_id'], $params['entity_user_id'], $params['entity_id'], 1), $params['dbh']);
	}
	...
	function deleteAllLock($params=array()){
		...
		la_do_query($query, array($params['entity_user_id'], 1), $params['dbh']);
	}
	...
	function isFormLockedForUser($params=array()){
		$locked = false;
		$user_id_redirect = $isAdmin = 0;
		// $entities = $params['entities'];
		$entry_id = $params['entry_id'];
		
		$info_parent = getLockDetails(array('form_id' => $params['form_id'], 'entity_id' => $params['entity_id'], 'dbh' => $params['dbh']));

		if(count($info_parent)){
			foreach($info_parent as $info){
				
				if($info['entity_user_id'] != $params['entity_user_id']){
					$user_id_redirect = $info['entity_user_id'];
		...
		if($locked){
			// echo $user_id_redirect;
			// header("location:entry_locked.php?id={$params['form_id']}&entry_id={$entry_id}&user_id={$user_id_redirect}&isAdmin={$isAdmin}&entity_id={$params['entity_id']}");
			header("location:view_entry.php?form_id={$params['form_id']}&entry_id={$entry_id}&user_id={$user_id_redirect}&isAdmin={$isAdmin}&entity_id={$params['entity_id']}&readonly=1");
			exit();
		}else{
			// foreach($entities as $ke => $ve){
				lockForm(array('form_id' => $params['form_id'],'entity_id' => $params['entity_id'], 'entity_user_id' => $params['entity_user_id'], 'dbh' => $params['dbh']));
			// }
		}
		// }
	}
	/*Lock Form Functions*/
	function getEntityName($dbh, $client_id){
		$primary_entity = "";
		
		$query = "SELECT `company_name` FROM `".LA_TABLE_PREFIX."ask_clients` WHERE `client_id` = ? LIMIT 1";
		$sth = la_do_query($query, array($client_id), $dbh);
		
		while($row = la_do_fetch_result($sth)){
			$primary_entity = $row['company_name'];
		}
	...
	function getEntityIds($dbh, $client_user_id){
		...
		while($row = la_do_fetch_result($sth)){
			array_push($entities, $row['client_id']);
		}

		$query1 = "SELECT `entity_id` FROM `".LA_TABLE_PREFIX."entity_user_relation` WHERE `client_user_id` = ?";
		$sth1 = la_do_query($query1, array($client_user_id), $dbh);
		
		while($row = la_do_fetch_result($sth1)){
			if(!in_array($row['entity_id'], $entities)){
	...
	function getEntities($dbh){
		$entities = array();
		$query = "SELECT `client_id`, `company_name` FROM ".LA_TABLE_PREFIX."ask_clients";
		$sth = la_do_query($query, array(), $dbh);
	...
	function getAllEntities($dbh) {
		$entities = array();
		
		$query = "SELECT * FROM `".LA_TABLE_PREFIX."ask_clients`";
		$sth = la_do_query($query, null, $dbh);
		
		while($row = la_do_fetch_result($sth)){
			$entities[$row['client_id']] = $row['company_name'];
		}
	...
	function getMatrixFieldNote($params=array()){
		$query = "SELECT IF(`t1`.`element_matrix_parent_id` = 0, `t1`.`element_note`, (SELECT `t2`.`element_note` FROM `".LA_TABLE_PREFIX."form_elements` `t2` WHERE `t2`.`form_id` = `t1`.`form_id` AND `t2`.`element_id` = `t1`.`element_matrix_parent_id`)) `element_note` FROM `".LA_TABLE_PREFIX."form_elements` `t1` WHERE `t1`.`form_id` = ? AND `t1`.`element_id` = ?";
		
		$resultset = la_do_query($query, array($params['form_id'], $params['element_id']), $params['dbh']);
		$row = la_do_fetch_result($resultset);
		
		return (int) $row['element_note'];
	}
	
 	function savePortalRegistrationFlagChange($params=array()){
		$dbh = $params['dbh'];
		$user_id = $params['user_id'];		
		
		if($params['portal_registration'] != $params['portal_registration_change']){
			la_do_query("INSERT INTO `".LA_TABLE_PREFIX."portal_self_registration` (`id`, `last_change`, `user_id`, `datetime_change`) VALUES (NULL, ?, ?, ?)", array(
				$params['portal_registration_change'], 
				$user_id,
				time()
			), $dbh);
		}
	...
	function checkMaxId($params=array())
	{
		$dbh = $params['dbh'];
		$form_id = $params['form_id'];
		$incrementBy = isset($params['incrementBy']) ? $params['incrementBy'] : 0;
		//$company_id = rand(1111111111,9999999999) + $incrementBy;
		$company_id = time() + $incrementBy;
		/*//echo "<br>incrementBy:{$params['incrementBy']}:{$company_id}<br>";
		// fetch max company_id from the table 
		$query_max = "SELECT ifnull(max(company_id),0) `company_id` FROM `".LA_TABLE_PREFIX."form_{$form_id}`";
		$result_max = la_do_query($query_max,array(),$dbh);
		$row_max = la_do_fetch_result($result_max);
		//echo 'here1';print_r($row_max);
		
		if($row_max && $row_max['company_id'] > 0){
			//echo "<br>here1:{$company_id}<br>";
			$company_id = ($row_max['company_id'] + time() + $incrementBy);
			//echo "<br>here:{$company_id}<br>";
		}*/		
		//echo "<br>incrementBy:{$params['incrementBy']}:{$company_id}<br>";
		// check company_id exist in the table or not
		$query_check  = "SELECT `company_id` FROM `".LA_TABLE_PREFIX."form_{$form_id}` where `company_id` = :company_id";
		$result_check = la_do_query($query_check,array(':company_id' => $company_id),$dbh);
		$row_check    = la_do_fetch_result($result_check);
		if($row_check){
			checkMaxId(array('dbh' => $dbh, 'form_id' => $form_id, 'incrementBy' => $incrementBy++));
			//echo "<br>here2:{$company_id}<br>";
		}
		
		return $company_id;
	}
	
	/* this function checks create date of form */
	function checkFormCreateDate($dbh, $form_id)
	{
		$query  = "select `action_datetime` from `".LA_TABLE_PREFIX."audit_log` where `form_id` = :form_id and `action_type_id` = :action_type_id order by `action_datetime` desc limit 1";
		$result = la_do_query($query,array(
			':form_id' => $form_id,
			':action_type_id' => 1
		),$dbh);
		
		if($row = la_do_fetch_result($result)){
			return $row['action_datetime'];
		}else{
			return 0;	
		}
	}
	
	/* this function checks whether form is empty or not*/
	function checkFormData($dbh, $form_id)
	{
		$query  = "SELECT COUNT(*) `counter` FROM `".LA_TABLE_PREFIX."form_{$form_id}`";
		$result = la_do_query($query,array(),$dbh);
		$row    = la_do_fetch_result($result);
		
		return ($row['counter'] == 0) ? false : true;
	}
		...
		if(!isset($user_id)){
			...
			if($action_type_id == 1){
				try{
					...
					$queryChk  = "select * from `".LA_TABLE_PREFIX."audit_log` where `form_id` = :form_id and `action_type_id` = :action_type_id and `user_id` = :user_id";
			...
			if($insertData){
				try{
					...
					$query = "INSERT INTO `".LA_TABLE_PREFIX."audit_log` (`id`, `user_id`, `form_id`, `action_type_id`, `action_text`, `user_ip`, `action_datetime`) VALUES (NULL, :user_id, :form_id, :action_type_id, :action_text, :user_ip, :action_datetime)";
				...
				try{
					...
					$query = "UPDATE `".LA_TABLE_PREFIX."audit_log` SET `action_datetime` = :action_datetime WHERE `form_id` = :form_id and `action_type_id` = :action_type_id and `user_id` = :user_id";
					@la_do_query($query,array(
						':user_id' => $user_id,
						':form_id' => $form_id,
						':action_type_id' => $action_type_id,
		...
		if(!isset($user_id)){
			...
			try{
				...
				foreach ($files as $key => $file) {
					...
					$queryChk  = "select * from `".LA_TABLE_PREFIX."audit_log` where `form_id` = :form_id and `action_type_id` = :action_type_id and `user_id` = :user_id and `action_text` = :action_text";
	...
	function addUserActivityBatch($user_ids, $action_type_id, $action_text, $user_ip, $dbh) {
		if(!empty($user_ids)){
			...
			$result_all_users = la_do_query($query_all_users,[],$dbh);
			$all_users = [];
			while($row_all_users = la_do_fetch_result($result_all_users)){
				$all_users[$row_all_users['user_id']] = $row_all_users['user_email'];
			}


			$query = "INSERT INTO `".LA_TABLE_PREFIX."audit_log`(`user_id`, `form_id`, `action_type_id`, `action_text`, `user_ip`, `action_datetime`) VALUES ";
			$queryArr = [];
			$dataArr = [];
			$action_datetime = time();
 			foreach ($user_ids as $user_id) {
 				...
 				array_push($queryArr, "(?,?,?,?,?,?)" );
	  			$dataArr = array_merge($dataArr, array($user_id,0,$action_type_id,$action_text,$user_ip,$action_datetime) );

	  			//add log for action_performed_on
	  			$action_text_performed_on = json_encode( array('action_performed_on' => $all_users[$user_id]) );
	  			array_push($queryArr, "(?,?,?,?,?,?)" );
	  			$dataArr = array_merge($dataArr, array($_SESSION['la_user_id'],0,$action_type_id,$action_text_performed_on,$user_ip,$action_datetime) );
	...
	/* this function will create folder of a duplicate form to store template */
 	function createFolderPath($params=array())
	{
		$dbh 	 		= $params['dbh'];
		$user_id 		= $params['user_id'];
		$form_id 		= $params['form_id'];
		$new_form_id 	= $params['new_form_id'];
		...
		$folder_path = $_SERVER['DOCUMENT_ROOT'].dirname($_SERVER['SCRIPT_NAME'])."/templates/".$user_id."/".$new_form_id;
		if(is_dir($folder_path) === false){
			@mkdir($folder_path, 0777, true);
		}
		...
		copyTemplateFromOldFormAndPasteToNewForm($params);
	}
	
	function copyTemplateFromOldFormAndPasteToNewForm($params)
	{
		$dbh 	 		= $params['dbh'];
		$user_id 		= $params['user_id'];
		$form_id 		= $params['form_id'];
		...
		$query 	= "SELECT template_id, template FROM `".LA_TABLE_PREFIX."form_template` WHERE `form_id` = :form_id";
		$params = array();
		$params[':form_id'] = $form_id;
		$sth = la_do_query($query,$params,$dbh);
		
		while($row = la_do_fetch_result($sth)){
			$source   = trim($row['template']);
			$tmpArr   = explode("/", $source); 
			$template = end($tmpArr);
			$dest     = $folder_path."/".$template;
			
			$insertQry = "insert into `".LA_TABLE_PREFIX."form_template` (template_id, form_id, template) values (NULL, :form_id, :template)";
			la_do_query($insertQry, array(':form_id' => $new_form_id, ':template' => $dest), $dbh);
		...
		la_update_template_file($dbh, $new_form_id);
	}
	...
	function generatePdf($content='')
	{
		...
		unset($dompdf);
		if (!file_exists('data/temp/form_data_pdf')) {
			mkdir('data/temp/form_data_pdf', 0777, true);
		}
		file_put_contents("data/temp/form_data_pdf/{$pdf_name}.pdf", $output);
		
		return array("pdffile_path" => "data/temp/form_data_pdf/{$pdf_name}.pdf", "pdffile_name" => "{$pdf_name}.pdf");
	}
	...
	function xcopy($source, $dest, $permissions = 0755)
	{
 	...
 	/* this function updates the form table's form_upload_template field */
	function la_update_template_file($dbh, $form_id)
	{
		$template_arr = array();
		$query 	= "SELECT template_id, template FROM `".LA_TABLE_PREFIX."form_template` WHERE `form_id` = :form_id";
		$params = array();
		...
		$sth = la_do_query($query,$params,$dbh);
		while($row = la_do_fetch_result($sth)){
			$tmpArr = explode("/", $row['template']); 
			$template = end($tmpArr);
			array_push($template_arr, $template."|||".$row['template_id']);
		}
		if(count($template_arr) > 0){
			$form_upload_template = implode(",", $template_arr);
			...
			$params_sv[':form_upload_template'] = $form_upload_template;
...
EOT;
    ...
    function la_send_webhook_notification($dbh,$form_id,$entry_id,$webhook_rule_id){
		...
		$template_data = la_get_template_variables($dbh,$form_id,$entry_id,$template_data_options);
    ...
    function la_send_notification($dbh,$form_id,$company_id,$entry_id,$to_emails,$email_param){    	
		...
		if (array_key_exists("link_and_file_name",$email_param))
	    	$template_data_options['link_and_file_name'] 	= $email_param['link_and_file_name'];

	    if (array_key_exists("email_entry_call",$email_param))
	    	$template_data_options['email_entry_call'] 	= $email_param['email_entry_call'];
		...
		//create mailer instance
		$s_mailer = Swift_Mailer::newInstance($s_transport);
		if(file_exists("./portal/data/form_{$form_id}/files") && is_writable("./portal/data/form_{$form_id}/files")){
			Swift_Preferences::getInstance()->setCacheType('disk')->setTempDir("./portal/data/form_{$form_id}/files");
		}else{
    	...
    	if( isset($email_param['is_inserted']) ) {
    		$action_datetime = get_session_time_db($dbh, $_SESSION['la_user_id']);
    	...
    	if(!empty($email_address)){
	    	...
	    	if($check_hook_file === true){
	    		$hook_emails = $la_hook_emails[$form_id];
	    		if(!empty($hook_emails)){
	    			$hook_element_id = $hook_emails['element_id'];

	    			//get the field type of this element_id
	    			$query = "select element_type from ".LA_TABLE_PREFIX."form_elements where form_id=? and element_id=? and element_status=1";
					...
					if($row['element_type'] == 'checkbox'){
						...
						$query = "select {$checkbox_element_names_joined} from ".LA_TABLE_PREFIX."form_{$form_id} where `company_id`=? AND `entry_id` = ?";
						$params = array($company_id, $entry_id);
						$sth = la_do_query($query,$params,$dbh);
						$row = la_do_fetch_result($sth);
						...
						if(!empty($selected_checkbox_array)){
							...
							foreach ($selected_checkbox_array as $selected_option_title) {
								$selected_hook_email = $la_hook_emails[$form_id][$selected_option_title];
		    			...
		    			$query = "select 
									...
									from 
										".LA_TABLE_PREFIX."form_{$form_id} A left join ".LA_TABLE_PREFIX."element_options B 
									  on 
									    B.form_id=? and A.field_name = 'element_{$hook_element_id}' and A.data_value = B.option_id and B.live=1 and B.element_id=?
									where 
										A.`company_id`=? AND `entry_id` = ?";
		    			$params = array($form_id,$hook_element_id,$company_id,$entry_id);
						$sth = la_do_query($query,$params,$dbh);	
						$row = la_do_fetch_result($sth);
						$selected_value = $row['selected_value'];

						$selected_hook_email = $la_hook_emails[$form_id][$selected_value];
    ...
    function la_send_logic_notifications_final_approval_status($dbh,$form_id,$entry_id,$options=array()) {
    	
    	$la_settings = la_get_settings($dbh);
		$itauditmachine_base_path = $la_settings['base_url'];


		//form approval status { check if form is approved or denied }
		$query  = "SELECT data_value as entry_status FROM `".LA_TABLE_PREFIX."form_{$form_id}` WHERE field_name='approval_status' AND company_id=:company_id";
		$result = la_do_query($query,array(':company_id' => $entry_id),$dbh);
		$row    = la_do_fetch_result($result);

		$approval_status = $row['entry_status'];
		//form approval status { check if form is approved or denied }
		...
		//echo '<pre>';print_r($email_logic_array);echo '</pre>';die;
		//evaluate the condition for each rule
		...
		if(!empty($email_logic_array)){
			...
			foreach ($email_logic_array as $value) {
				...
				while($row = la_do_fetch_result($sth)){
					...
					$condition_params['use_main_table'] = true;
					$condition_params['entry_id'] 		= $entry_id;  
					$current_rule_conditions_status[] = la_get_condition_status_from_table_final_approval_status($dbh,$condition_params);
				}

				/*added check for final approval status*/
				...
				$query = "SELECT 
								...
								rule_keyword 
							FROM 
								".LA_TABLE_PREFIX."email_logic_conditions_final_approval_status
						   WHERE 
						   		form_id = ? AND target_rule_id = ?";
				$params = array($form_id,$target_rule_id);
				...
				while($row = la_do_fetch_result($sth)){
					...
					if( $rule_condition == 'is_approved' && $approval_status == 1 ) {
						$current_rule_conditions_status[] = true;
					} else if ( $rule_condition == 'is_denied' && $approval_status == 2 ) {
						$current_rule_conditions_status[] = true;
					} else if( $rule_condition == 'is_approved' && $approval_status == 2 ) {
						$current_rule_conditions_status[] = false;
					} else if ( $rule_condition == 'is_denied' && $approval_status == 1 ) {
						$current_rule_conditions_status[] = false;
					}
		...
		if(!empty($email_logic_array)){
			...
			foreach ($email_logic_array as $value) {
				...
				while($row = la_do_fetch_result($sth)){
					...
					$condition_params['rule_keyword'] 	= $row['rule_keyword'];
					$condition_params['use_main_table'] = true;
					$condition_params['entry_id'] 		= $_SESSION['company_user_id'];  
					$current_rule_conditions_status[] = la_get_condition_status_from_table($dbh,$condition_params);
				}
    ...
    //send notifications based on logic in approver section
    function la_send_approver_notifications($dbh,$form_id,$options=array()){
    ...
    function la_send_resume_link($dbh,$form_name,$form_resume_url,$resume_email){
   	...
   	function la_send_tagging_notification($dbh,$form_name,$form_url,$form_email,$params=[]){
		...
		$email_content ="Hello,<br>User '{$params['user_name']}' has updated the form: <strong> {$form_name}</strong>.<br>";
		if( !empty($params['current_url']) )
			$email_content .= "Last page edited was <a href=\"{$params['current_url']}\">page {$la_page}</a>.<br>";

		$email_content .= "<a href=\"{$form_url}\">Click here</a> to view the form from beginning";
    ...
    function la_send_password_change_notification($dbh,$user_id){
    	global $la_lang;
    	...
    	$query = "select user_fullname, user_email from ".LA_TABLE_PREFIX."users where user_id=? and `status`=1";
...
The password for your admin account(%s) in %s has been changed.
Please contact an administrator or set your own password.

Thank you.
EOT;

		$email_template = nl2br($email_template);
    	$email_content = sprintf($email_template,$user_fullname,$user_email,$la_settings['base_url']);
    ...
    function la_send_one_time_url($dbh, $user_name, $user_email, $params = array()){
		...
		$email_content = '';		
		$one_time_url = $la_settings['base_url'].'one_time_password.php?utoken='.$params['token'];
...
EOT;
    ...
    function la_get_condition_status_from_table_final_approval_status($dbh,$condition_params){
		...
		if(in_array($element_type, array('text','textarea','simple_name','name','simple_name_wmiddle','name_wmiddle','address','phone','simple_phone','email','url'))){
			...
			//get the entered value on the table
			/*$query = "SELECT count(B.element_title) total_row  FROM( SELECT A.data_value,
								   (select 
								   		  `option` 
								   	  from 
								   	  	  ".LA_TABLE_PREFIX."element_options 
								   	 where 
								   	 	  ...
								   	 	  option_id = A.data_value and 
								   	 	  live = 1) element_title
							   FROM 
							  	   ".LA_TABLE_PREFIX."form_{$form_id}{$table_suffix} A
                                   
                                   WHERE A.field_name='{$element_name}'
                                   AND A.company_id={$record_id} ) B WHERE B.element_title {$where_operand} {$where_keyword}";*/
    ...
    function la_get_template_variables($dbh,$form_id,$company_id,$entry_id,$options=array()){
	    ...
	    if (array_key_exists("email_entry_call",$options))
	    	$entry_options['email_entry_call'] 	= $options['email_entry_call'];

	    if (array_key_exists("link_and_file_name",$options))
	    	$entry_options['link_and_file_name'] 	= $options['link_and_file_name'];
    	...
    	foreach ($entry_details as $data){
    		...
    		if($as_plain_text){
    			$data['value'] = htmlspecialchars_decode($data['plain_text_value'],ENT_QUOTES);
    			$data['value'] = str_replace('<br />', "\n", $data['plain_text_value']);
    			
    			if($data['element_type'] != 'file'){
    				$data['value'] = str_replace('&nbsp;', "", $data['plain_text_value']);
    			}
    	...
    	if(!$as_plain_text){
			...
			foreach ($entry_details as $data){
				...
				elseif ($data['element_type'] == 'file'){
					...
					$q_string = base64_encode("form_id={$form_id}&id={$_SESSION['company_user_id']}&el=element_{$element_id}&hash={$signature_hash}");
				...
				if($data['element_type'] == 'section'){
					...
					if(!empty($row_entity)) {
						$query_note = "SELECT * FROM `".LA_TABLE_PREFIX."form_element_note` WHERE `form_id` = ? and `element_id` = ? and `company_id` = ? and `admin_id` = ?";
						$result_note = la_do_query($query_note, array($form_id, $element_id, $row_entity["client_id"], 0), $dbh);
						...
						$query_note = "SELECT * FROM `".LA_TABLE_PREFIX."form_element_note` WHERE `form_id` = ? and `element_id` = ? and `admin_id` = ?";
						$result_note = la_do_query($query_note, array($form_id, $element_id, $_SESSION["la_user_id"]), $dbh);
						$row_note    = la_do_fetch_result($result_note);
						if(empty($row_note)) {
							$data_value = html_entity_decode(($data['value']));					
							$email_body .= "<tr {$row_style}>\n";
		...
		if(count($entry_details)){
			...
			foreach($entry_details as $k => $v){
				if(!empty($v['label'])){
					...
					$v_value = html_entity_decode($v['value']);
					
					array_push($html_data_table_tr_arr, '<tr><td>'.$v['label']."</td></tr><tr><td>{$v_value}</td></tr>");
				}
			}
		}
    ...
    function la_get_entry_details_field_html($data, $target_is_admin) {
    	if((empty($data['value']) || $data['value'] == '&nbsp;') && $data['value'] !== 0 && $data['value'] !== '0' && $data['element_type'] !== 'section'){
			return false;
		}				
		...
		if($data['element_type'] == 'section'){
			...
			$email_body .= '<td width="60%" style="border-bottom:1px solid #DEDEDE;padding:5px 10px;">'." {$data_value}".'</td>'."\n";
			$email_body .= '</tr>'."\n";
		}
		return $email_body;
    }
...
EOT;
    	...
    	// if( ENVIRONMENT == 'production' ) {
			...
			if(!defined('SERVER_ENVIRONMENT')) {
		    	...
		    	print_r('user_email:-');
		    	print_r($user_email);
		    	echo'in la_send_approval_email';
		    	die();
			}
...
EOT;
    ...
    //function is called for notification email acc. to logic in { Add User to Approvers }
    function la_send_approver_next_email($dbh,$form_id,$user_email) {
    	// print_r($user_email);
		// die();
    	global $la_lang;
		...
		$subject = 'IT Audit Machine Form You are next';
		$email_template =<<<EOT
Hello,

You are the next one to approve an entry. An entry for '%s' is waiting for your approval. The link to form is <a href="%s">%s</a>
...
EOT;
	...
	function get_maximum_file_upload_size() {
		return min(convert_size_to_bytes(ini_get('post_max_size')), convert_size_to_bytes(ini_get('upload_max_filesize')));
	}


	function convert_size_to_bytes($sSize) {
	...
	function send_entry_document($dbh, $form_id, $entry_id, $zip_attach_path) {
...
'%s' downloaded document file '%s' for form '%s' and <a href="%s">this</a> is link to entry.

EOT;

			$email_template = nl2br($email_template);
			$entry_link = $la_settings['base_url']."view_entry.php?form_id={$form_id}&entry_id={$entry_id}";
	...
	function custom_json_response($data) {
	...
	*/
	function addUserToChain($dbh, $user_id) {
		//return true;
		$query = "SELECT `eth_account`, `user_email` FROM `".LA_TABLE_PREFIX."users` WHERE `user_id` = ?";		
		...
		if(empty($row['eth_account'])){
			...
			$isEthAccount = $web3Ethereum->createAccount($row['user_email'].'.'.$user_id);

			//authorize this account
			$web3Ethereum->send('authorizeSigner',$isEthAccount);

			if( $isEthAccount ) {
				$eth_query = "UPDATE ".LA_TABLE_PREFIX."users SET eth_account = ? WHERE user_id = ?";
	...
	function la_send_email($dbh, $la_settings, $recipients, $subject, $content){
		//get settings first
    ...
    */
    function get_session_time_db($dbh, $user_id) {
    	$query = "SELECT action_datetime FROM ".LA_TABLE_PREFIX."audit_log where user_id = ? AND action_type_id = 6 ORDER BY id DESC LIMIT 1";
    	$sth = la_do_query($query,array($user_id),$dbh);
    	$row = la_do_fetch_result($sth);
    	
    	return ( !empty($row['action_datetime']) ) ? $row['action_datetime'] : false;
    }
    ...
    function getUserDetailsFromId($dbh, $user_id) {
		$query  = "SELECT user_email,user_fullname,eth_account FROM `".LA_TABLE_PREFIX."users` WHERE `user_id` = ?";
        $params = array($user_id);
        $sth = la_do_query($query,$params,$dbh);
	...
	function sendFieldNoteReport($dbh, $la_settings, $report_id) {
		$site_name = "https://".$_SERVER['SERVER_NAME'];
		...
		$result_form_name = la_do_query($query_form_name, array($form_id), $dbh);
		$row_form_name = la_do_fetch_result($result_form_name);
		$form_name = $row_form_name["form_name"];
		
		if($form_name != "") {
			//get entity info
			$company_id = $row_report["company_id"];
			$query_entity = "SELECT * FROM `".LA_TABLE_PREFIX."ask_clients` WHERE `client_id` = ?";
			$result_entity = la_do_query($query_entity, array($company_id), $dbh);
			$row_entity = la_do_fetch_result($result_entity);
			$contact_email = $row_entity["contact_email"];
			$entity_name = $row_entity["company_name"];

			//get recipients's emails
			$recipients = $row_report["recipients"];
			$admin = explode(",", explode(";", $recipients)[0]);
	    	$entity = explode(",", explode(";", $recipients)[1]);
	    	$users = explode(",", explode(";", $recipients)[2]);
	    	...
	    	foreach ($entity as $key => $entity_id) {
	    		$query_entity = "SELECT `company_name`, `contact_email` FROM ".LA_TABLE_PREFIX."ask_clients WHERE `client_id` = ?";
		    	$sth_entity = la_do_query($query_entity, array($entity_id), $dbh);
		    	while($row_entity = la_do_fetch_result($sth_entity)) {//fecth entity email first and add it to the list of emails
		    		array_push($user_emails, $row_entity["contact_email"]);
		    		$query_user_in_entity = "SELECT u.email FROM ".LA_TABLE_PREFIX."ask_client_users u LEFT JOIN ".LA_TABLE_PREFIX."entity_user_relation r ON u.client_id = r.entity_id WHERE r.client_user_id = u.client_user_id AND u.status = 0 AND r.entity_id = ?"; // fectch only active users belong to the selected entity
					$sth_user_in_entity = la_do_query($query_user_in_entity, array($entity_id), $dbh);
					while($row_user_in_entity = la_do_fetch_result($sth_user_in_entity)) {
			...
			if(count($admin_emails) > 0 || count($user_emails) > 0) {
				$formIDArr = array();
				if($row_report['multiple_form_report'] == 0) {
					array_push($formIDArr, $form_id);
				}
				if($row_report['multiple_form_report'] == 1) {
					$query_multiple_report = "SELECT `multiple_report_id`, `form_id` FROM `".LA_TABLE_PREFIX."form_multiple_report` WHERE `report_id` = {$report_id}";
					$res_multiple_report = la_do_query($query_multiple_report,array(),$dbh);

					while($row_multiple_report = la_do_fetch_result($res_multiple_report)) {
				...
				if(count($field_notes) > 0) {
					//create mail parameters			    		
					$subject = "Continuum GRC Field Note Report";
					...
					if(count($admin_emails) > 0) {
						$admin_content = "<h3>Continuum GRC Field Note Report</h3>";
						$admin_content .= "<p>Here is a field note report for the entry made by <b>{$entity_name}[{$contact_email}]</b> in the form <b>{$form_name}</b> on <b>{$site_name}</b>.</p>";
				    	$admin_content .= "<p>Report Details:</p>";
						$admin_content .= "<style>
										.status-active {
											color: #009300;
										}
										...
										#field_note_report_table td {
											padding: 8px 10px;
										}
									</style>
									<table id='field_note_report_table'>
										<thead>
											<th>Form ID</th>
											<th>Form Name</th>
											<th>Field Title</th>
						...
						foreach ($field_notes as $field_note) {
							...
							$admin_content .="<tr><td>".$field_note["form_id"]."</td><td>".$field_note["form_name"]."</td><td>".$field_note["element_title"]."</td><td class='{$status_class}'>".$field_note["note"]."</td><td>".$field_note["assigner"]."</td><td>".$field_note["assignees"]."</td><td>".$field_note["date_created"]."</td><td class='{$status_class}'>".$status."</td><td><a target='_blank' href='{$site_name}/auditprotocol{$field_note["field_link"]}'>Go To Field</a></td></tr>";
						}
						$admin_content .= "</tbody></table>";
					...
					if(count($user_emails) > 0) {
						$user_content = "<h3>Continuum GRC Field Note Report</h3>";
						$user_content .= "<p>Here is a field note report for the entry made by <b>{$entity_name}[{$contact_email}]</b> in the form <b>{$form_name}</b> on <b>{$site_name}</b>.</p>";
				    	$user_content .= "<p>Report Details:</p>";
						$user_content .= "<style>
										.status-active {
											color: #009300;
										}
						...
						foreach ($field_notes as $field_note) {
							...
							$user_content .="<tr><td>".$field_note["form_id"]."</td><td>".$field_note["form_name"]."</td><td>".$field_note["element_title"]."</td><td class='{$status_class}'>".$field_note["note"]."</td><td>".$field_note["assigner"]."</td><td>".$field_note["assignees"]."</td><td>".$field_note["date_created"]."</td><td class='{$status_class}'>".$status."</td><td><a target='_blank' href='{$site_name}/portal{$field_note["field_link"]}'>Go To Field</a></td></tr>";
						}
						$user_content .= "</tbody></table>";
	...
	function getFormStructure($dbh, $form_ids) {
		$res = array();
		$elements = array();

		//get form elements and element options
		foreach ($form_ids as $tmp_form_id) {
			$query_form_elements = "SELECT element_id, element_title, element_type, element_position, element_status, element_page_number FROM ".LA_TABLE_PREFIX."form_elements WHERE form_id = ? ORDER BY element_position ASC";
			$sth_form_elements = la_do_query($query_form_elements, array($tmp_form_id), $dbh);
			while ($row_form_element = la_do_fetch_result($sth_form_elements)) {
				$element = array();
				$element_options = array();
				$element = $row_form_element;

				$element_id = $row_form_element['element_id'];
				//get element_options
				$query_form_element_options = "SELECT * FROM ".LA_TABLE_PREFIX."element_options WHERE form_id = ? and element_id = ? ORDER BY position ASC";
				$sth_form_element_options = la_do_query($query_form_element_options, array($tmp_form_id, $element_id), $dbh);
				while ($row_form_element_option = la_do_fetch_result($sth_form_element_options)) {
					$element_option = $row_form_element_option;
					unset($element_option['aeo_id']);
					unset($element_option['form_id']);
					array_push($element_options, $element_option);
				}
				$element['element_options'] = $element_options;
