...
function sanitizeInput($input, $encoding = 'UTF-8'){
	return $input;
	/*if(!empty($input)){
		$input = is_array($input) ? array_map('sanitize_deeply', $input) : htmlspecialchars(trim($input));
	}
...
function sanitize_deeply($value, $encoding = 'UTF-8'){
	$value = is_array($value) ? array_map('sanitize_deeply', $value) : htmlspecialchars(trim($value));
	return $value;
}
