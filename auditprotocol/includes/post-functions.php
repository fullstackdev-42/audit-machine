 ...
 ********************************************************************************/
	...
	function insertAndUpdateFormData($params=array())
	{
		...
		if(count($table_data)){
			foreach($table_data as $k => $v){
				if(!in_array($k, array('ip_address', 'date_created', 'casecade_form', 'syndication'))){
					$e_info = explode("_", $k);
 	...
 	function add_eth_data($dbh, $form_id, $eth_data_Arr) { 		
 		if(!empty($eth_data_Arr)){
			$query = "INSERT IGNORE INTO `".LA_TABLE_PREFIX."eth_file_data`(user_id, form_id, company_id, field_name, field_code, data, action_datetime) VALUES ";
			$queryArr = [];
			$dataArr = [];
			$files = [];
 			...
 			foreach ($eth_data_Arr as $eth_data) {
 				if( strpos($eth_data['files'], '|') !== false ) {
			  		...
			  		foreach ($file_names as $key => $file) {
			  			array_push($queryArr, "(?,?,?,?,?,?,?)" );
			  			$dataArr = array_merge($dataArr, array($_SESSION['la_user_id'], $form_id, $eth_data['company_id'], $eth_data['field_name'], $eth_data['field_code'], $file, $action_datetime) );
			  		}
			  	} else {
			  		array_push($queryArr, "(?,?,?,?,?,?,?)" );
			  		$dataArr = array_merge($dataArr, array($_SESSION['la_user_id'], $form_id, $eth_data['company_id'], $eth_data['field_name'], $eth_data['field_code'], $eth_data['files'], $action_datetime) );
			  	}
 			}
			...
			$res_added_files = la_do_query($query_added_files, array($_SESSION['la_user_id'], $form_id, $action_datetime), $dbh);
			while($row_added_file = la_do_fetch_result($res_added_files)){
				$files[] = $row_added_file['data'];
			}
			addUserActivityFileUpload($dbh, $_SESSION['la_user_id'], $form_id, $files);
		}
 	}
			...
			{								
				...
				if(!$row_file['field_name']){
					...
					if(strpos($file,'|') !== false){					
						...
						if(count($file_explode) > 0){														
							foreach($file_explode as $file_explode_value){	
								if(!empty($file_explode_value))	{						
									$complete_filename = $itauditmachine_data_path.$la_settings['upload_dir']."/form_{$form_id}/files/".$file_explode_value;
						...
						$complete_filename = $itauditmachine_data_path.$la_settings['upload_dir']."/form_{$form_id}/files/".$file;
	...
	{		
		...
		$from_edit_page = false;

		if((strpos( $_SERVER['REQUEST_URI'], "edit_entry.php" ) !== false) || (strpos( $_SERVER['REQUEST_URI'], "view.php" ) !== false)) {
			$from_edit_page = true;
		}
		$zipPath     = "";
		...
		//if called from edit_entry.php never ask for password
		if((isset($input['called_from'])) && ($input['called_from'] == 'edit_entry') ){
			$require_password = false;
		}
		...
		if(!empty($edit_id)){ // edited on 29 oct 2014
			...
			if($isCasecadeForm){
				if(isset($_POST['parent_nxt_element'])){
					$tmpElements = json_decode(base64_decode($_POST['parent_nxt_element']), true);
		...
		while($row = la_do_fetch_result($sth)){
			...
			$element_info[$row['element_id']]['default_value'] = $row['element_default_value'];			
			$element_info[$row['element_id']]['element_machine_code'] = $row['element_machine_code'];			
			$element_info[$row['element_id']]['file_upload_synced'] = $row['element_file_upload_synced'];
			$element_info[$row['element_id']]['file_enable_advance'] = (int) $row['element_file_enable_advance'];
			$element_info[$row['element_id']]['file_select_existing_files'] = (int) $row['element_file_select_existing_files'];	
		...
		{ // foreach start here : 1
			...
			//if this is private fields and not logged-in as admin, bypass operation below, just supply the default value if any
			//if this is private fields and logged-in as admin and this is not edit-entry page, bypass operation below as well
			if(($element_info[$element_id]['is_private'] == 1) && (!$from_edit_page)){
				if(!empty($element_info[$element_id]['default_value'])){
					$table_data['element_'.$element_id] = $element_info[$element_id]['default_value'];
			...
			elseif ('date' == $element_type || 'europe_date' == $element_type){ //Date
				...
				if(!empty($element_info[$element_id]['date_enable_selection_limit']) && !empty($element_info[$element_id]['date_selection_max'])){
					...
					$sub_query = "select selected_date from (select date_format(`data_value`, '%Y-%c-%e') as selected_date, count(`data_value`) as total_selection from `".LA_TABLE_PREFIX."form_{$form_id}` where `field_name` = 'element_{$element_id}' and `data_value` is not null {$disabled_date_where_clause} group by `data_value`) as A where A.total_selection >= ?";
			...
			elseif ('casecade_form' == $element_type){				
				...
				//some suspense here
				$la_process_casecade_form_array = [];
 		 		$la_process_casecade_form_array = array('dbh' => $dbh, 'la_settings' => $la_settings, 'is_committed' => $is_committed, 'form_id' => $casecade_input['form_id'], 'company_id' => $company_id, 'input' => $casecade_input['input'], 'page_number' => isset($_POST['casecade_form_page_number']) ? $_POST['casecade_form_page_number'] : 1);

				if( (isset($input['called_from'])) && ($input['called_from'] == 'edit_entry') ) {
					$la_process_casecade_form_array['called_from'] = 'edit_entry';
				}

				$casecade_table_data = la_process_casecade_form($la_process_casecade_form_array);
		...
		$form_enable_template_wysiwyg = (int) $row['form_enable_template_wysiwyg'];
		$form_template_wysiwyg_id = (int) $row['form_template_wysiwyg_id'];
		...
		/*if($check_unique_ip === true){
			...
			$query = "select count(id) total_ip from `".LA_TABLE_PREFIX."form_{$form_id}` where ip_address=? and `status`=1";
		...
		if($input['page_number'] == $form_page_total && !$is_saving_form_resume){
			$is_committed = true;
		}
		
		/* Insert form submitted data */
		insertAndUpdateFormData(array('dbh' => $dbh, 'la_settings' => $la_settings, 'is_committed' => $is_committed, 'form_id' => $form_id, 'company_id' => $company_id, 'entry_id' => $entry_id, 'table_data' => $table_data, 'element_info' => $element_info, 'element_resume_checkbox' => (isset($_POST['element_resume_checkbox']) && !$_POST['element_resume_checkbox'] ? 1 : 0), 'element_option_lookup' => $element_option_lookup, 'casecade_page_number' => (isset($_POST['casecade_form_page_number']) ? $_POST['casecade_form_page_number'] : 1)));	
		//end insert data into table -------------------------		

		//@Todo: Matt - up to here is optimized
		//preserve green status indicators
		$status_preserve_green = $input["status_preserve_green"];
		foreach ($status_preserve_green as $key => $value) {
			if($value == "3") {
				$status_form_id = explode("_", $key)[0];
				$status_element_id = explode("_", $key)[1];
				$query_status_2 = "DELETE FROM `".LA_TABLE_PREFIX."element_status_indicator` WHERE `form_id` = ? AND `element_id` = ? AND `company_id` = ? AND `entry_id` = ?";
				la_do_query($query_status_2, array($status_form_id, $status_element_id, $company_id, $entry_id), $dbh);
				$query_status_3 = "INSERT INTO `".LA_TABLE_PREFIX."element_status_indicator` (`id`, `form_id`, `element_id`, `company_id`, `entry_id`, `indicator`) VALUES (null, ?, ?, ?, ?, ?)";
				la_do_query($query_status_3, array($status_form_id, $status_element_id, $company_id, $entry_id, 3), $dbh);
			}
		}
		...
		if($input['page_number'] == $form_page_total && !$is_saving_form_resume){
			...
			if($template_enable > 0 || $wysiwyg_enable > 0) {
				...
				$params = array('dbh' => $dbh, 'form_id' => $form_id, 'la_user_id' => $_SESSION['la_user_id'], 'company_user_id' => $company_id, 'entry_id' => $entry_id, 'isAdmin' => 1);
		...
		if(!empty($logic_page_enable)){
			...
			if(!empty($input['submit_secondary']) || !empty($input['submit_secondary_x'])){
				...
				//echo $process_result['target_page_id']; exit;							
			}else{ 
				...
				$query = "SELECT 
						   ...
						   WHERE 
								...
								//Added 1=1 so it can go backward - SR 7/26/18
		...
		if(($is_inserted && !empty($esl_enable) && !empty($form_email) && empty($form_review) && ($form_page_total == 1) && empty($edit_id) && ($delay_notifications === false) ) || 
		   ($is_inserted && !empty($esl_enable) && $is_committed && empty($edit_id) && ($delay_notifications === false) )
		){
		...
		if(!empty($esr_enable) && !empty($esr_email_address) && $is_committed){
			...
			la_send_notification($dbh,$form_id,$company_id,$entry_id,$esr_email_address,$user_email_param);
		}
		
		//end sending notification email to user ------------------------------------------
		...
		/*if($is_inserted && $is_committed && !empty($logic_email_enable)){
			...
			$record_insert_id = $company_id; $entry_id => $entry_id,
			la_send_logic_notifications($dbh,$form_id,$company_id, $entry_id => $entry_id,$logic_email_param);
		}

		//send {Add user to approver} logic emails
		if($is_inserted && $is_committed && !empty($approver_logic_count)){
			la_send_approver_notifications($dbh,$form_id,array());
		}
		
		//send webhook notification
		if($is_inserted && $is_committed && !empty($webhook_enable)){
			$record_insert_id = $company_id; $entry_id => $entry_id,
			la_send_webhook_notification($dbh,$form_id,$company_id, $entry_id => $entry_id,0);
		}
		...
		if($is_inserted && $is_committed && !empty($logic_webhook_enable)){
			$record_insert_id = $company_id; $entry_id => $entry_id,
			la_send_logic_webhook_notifications($dbh,$form_id,$company_id) $entry_id => $entry_id,;
		}*/
				...
				if($is_saving_form_resume){
					...
					if(!$has_invalid_resume_email){
						if(isset($_REQUEST['la_page'])){
							$form_resume_url = $la_settings['base_url']."edit_entry.php?form_id={$form_id}&company_id={$company_id}&entry_id={entry_id}&la_page={$_REQUEST['la_page']}";
						} else {
							$form_resume_url = $la_settings['base_url']."edit_entry.php?form_id={$form_id}&company_id={$company_id}&entry_id={entry_id}";
						}
					...
					if( !empty($form_email) ) {
						...
						$form_url = $base_url.'/auditprotocol/edit_entry.php?form_id={$form_id}&company_id={$company_id}&entry_id={entry_id}';
						...
						$sth = la_do_query($query,$query_params,$dbh);
						$row = la_do_fetch_result($sth);

						$stored_user_email = $row['user_email'];
						$user_fullname 	   = $row['user_fullname'];

						$user_name = (empty($user_fullname)) ? $stored_user_email : $user_fullname;
		...
		if(!empty($file_uploads_array)){
			...
			foreach ($file_uploads_array as $element_name){
				...
				foreach ($filename_array as $filename){
					$target_filename 	  = $options['itauditmachine_data_path'].$la_settings['upload_dir']."/form_{$form_id}/files/{$filename}.tmp";
					...
					$destination_filename = $options['itauditmachine_data_path'].$la_settings['upload_dir']."/form_{$form_id}/files/{$element_name}_{$file_token}-{$new_record_id}-{$filename_noelement}";
