 ...
 ********************************************************************************/
 	...
 	function la_display_chart($dbh,$chart_access_key){
		...
		//Check if this is a status indicator report - if true, gather the count of each status type then display
		if ($chart_datasource == 'status_indicators') {
			$option_label_array[0] = 'Grey';
			$option_label_array[1] = 'Red';
			$option_label_array[2] = 'Yellow';
			$option_label_array[3] = 'Green';
			...
			for ($indicator_value = 0; $indicator_value < 4; $indicator_value++) {
				...
				$option_total_array[$indicator_value] = $row['TOTAL'];	
				$all_entry_total += $row['TOTAL'];
		...
		//For all reports using field values, not status indicator reports
		else {
			...
			if(!empty($chart_enable_filter)){
				$chart_where_clause = la_get_chart_filter($dbh,$form_id,$chart_id,$chart_filter_type);
			}else{
				...
				if($chart_using_date_axis){ //only line and area chart able to use date axis
					...
					if($datasource_element_type == 'radio' || $datasource_element_type == 'select'){
						...
						$query = "SELECT 
										...
										count(field_name) total_entry 
									FROM 
										".LA_TABLE_PREFIX."form_{$form_id} 
									WHERE field_name='{$chart_datasource}' AND data_value !='' 
									group by company_id";
						...
						if(!empty($element_choice_has_other)){
							$query = "SELECT 
									...
									FROM 
										".LA_TABLE_PREFIX."form_{$form_id} 
									WHERE field_name='{$chart_datasource}_other' AND data_value !=''
									group by company_id";
						...
						foreach ($option_label_array as $option_id => $option_label) {
							if(!empty($option_id)){
								...
								$query = "SELECT 
												count(data_value) total_entry 
											FROM 
												".LA_TABLE_PREFIX."form_{$form_id} 
												WHERE field_name='{$chart_datasource}_{$option_id}' 
												AND data_value !=''
												";
							}else{
								...
								$query = "SELECT 
											...
											FROM 
												".LA_TABLE_PREFIX."form_{$form_id} 
												WHERE field_name='{$chart_datasource}_other' 
												AND data_value !=''
												";
							}
		...
		//END of chart info processing for when datasource is a field or multiple fields.
...
EOT;
 	...
 	function la_get_element_currency($dbh,$form_id,$element_name){
 		$exploded = explode('_', $element_name);
 		$element_id = (int) $exploded[1];
 		
 		//get the currency of the payment amount
 		$form_properties = la_get_form_properties($dbh,$form_id,array('payment_currency'));
 	...
 	function la_display_grid($dbh,$chart_access_key){
		...
		$form_properties = la_get_form_properties($dbh,$form_id,array('payment_enable_merchant'));
		...
		$query = "select count(*) total_entries from ".LA_TABLE_PREFIX."form_{$form_id} group by `company_id`";
		$params = array();
		...
		if($form_properties['payment_enable_merchant'] == 1){
			...
			la_array_insert($columns_label,4,$payment_columns_label);
			
		}

		//get current grid column preference
		$query = "select element_name from ".LA_TABLE_PREFIX."grid_columns where form_id=? and chart_id=? order by position asc";
		...
		foreach ($grid_column_preference as $element_name => $element_title) {
			...
			if($columns_type[$element_name] == 'date'){
				...
				$element_currency = la_get_element_currency($dbh,$form_id,$element_name);
		...
		if(!empty($chart_title)){
			if($chart_title_position == 'top'){
				$chart_title_top_markup    = "<h2 class=\"la_grid_title\">{$chart_title}</h2>";
			}else{
				$chart_title_bottom_markup = "<h2 class=\"la_grid_title\">{$chart_title}</h2>";
			}
		}
...
EOT;
 	...
 	function la_get_chart_filter($dbh,$form_id,$chart_id,$filter_type){
		...
		//code below pretty much the same as within la_display_entries_table()
		/********************************************************************************************/
		$form_properties = la_get_form_properties($dbh,$form_id,array('payment_enable_merchant'));
 	...
 	function la_get_chart_datasource_markup($dbh,$form_id,$params){
