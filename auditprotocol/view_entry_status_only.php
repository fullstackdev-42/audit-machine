...
EOT;
	...
	<script type="text/javascript">
		$(document).ready(function (e) {
			...
			$(document).on('click', 'img.status-icon-action-status', function(){
				var _selector = $(this);
				var all_statuses = _selector.parent('td.status_parent').find('div.all_statuses');
				all_statuses.html('Click circle below to change status: </br> <img class="all_statuses_child" src="images/Circle_Gray.png" data-indicator="0"> <img class="all_statuses_child" src="images/Circle_Red.png"  data-indicator="1"> <img class="all_statuses_child" src="images/Circle_Yellow.png"  data-indicator="2"> <img class="all_statuses_child" src="images/Circle_Green.png"  data-indicator="3">');
			});

			var statusIndicatorArr = ['gray', 'red', 'yellow', 'green'];

			$(document).on('click', 'img.all_statuses_child', function(){
				var _original_selector = $(this);
				var all_statuses = _original_selector.parent('.all_statuses');
				var _selector = _original_selector.parents('td.status_parent').find('img.status-icon-action-status');

				var _selector_tr = _selector.closest('tr');
				...
				var prev_indicator = _selector.attr('data-indicator');
				var indicator = _original_selector.attr('data-indicator');
				
				if( prev_indicator != indicator ) {
					$("#processing-status-dialog").dialog('open');
					$.ajax({
						url:"processupload.php",
						type:"POST",
						data:{mode:"updateindicatorany", form_id:form_id, element_id:element_id, company_id:company_id, entry_id:entry_id, indicator:indicator},
						beforeSend: function(){},
						success: function(response){
							response = JSON.parse(response);
							_selector.attr('src', 'images/Circle_'+response.status_icon+'.png');
							_selector.attr('data-indicator', response.indicator);
							var status_icon = response.status_icon.toLowerCase();

							var h_selector = $('h3.'+status_icon);
							var acc_selector = h_selector.next('div.panel-accordion');
							acc_selector.find('tr.no-results').remove();

							var cloned_row = _selector_tr.clone();
							cloned_row.find('.all_statuses').html('');
							acc_selector.find('tbody').append(cloned_row);

							//increase count
							var current_count = h_selector.find('.ball').text();
							var new_count = parseInt(current_count)+1;
							h_selector.find('.ball').text(new_count);

							//decrease count
							var prev_h_selector = $('h3.'+statusIndicatorArr[prev_indicator]);
							var old_count_prev_h = prev_h_selector.find('.ball').text();
							var new_count_prev_h = parseInt(old_count_prev_h)-1;
							prev_h_selector.find('.ball').text(new_count_prev_h);

							//hide show accordion area
							if( !(h_selector.hasClass('active')) ){
								h_selector.addClass('active');
								h_selector.next('div.panel-accordion').slideDown();
							}
						...
						complete:function(){
							$("#processing-status-dialog").dialog('close');
							all_statuses.html('');
						}
					});
				} else {
			...
			$('#dialog-status-csv').click(function(){
				window.location = '/auditprotocol/generate_entry_status_csv.php?download_csv=1&form_id='+<?=$form_id?>+'&company_id='+<?=$company_id?>+'&entry_id='+<?=$entry_id?>;
			});
		}); // document ready
