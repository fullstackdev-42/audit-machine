...
function download_zip ($form_id, $company_id, $entry_id, $dbh) {
	$query = "SELECT * FROM `".LA_TABLE_PREFIX."template_document_creation` WHERE `form_id` = {$form_id} AND `company_id` = ? AND `entry_id` = ? AND `isZip` = 1 order by `docx_create_date` DESC LIMIT 1";
	...
	if( !empty($file) ) {
		...
		if (file_exists($file)) {
			send_entry_document($dbh, $form_id, $company_id, $entry_id, $file);
			// add user activity to log: activity - 8 (document download)
			addUserActivity($dbh, $_SESSION['la_client_user_id'], $form_id, 8, "downloaded document file <b>{$file_name}</b>", time(), $_SERVER['REMOTE_ADDR']);
