...
<script type="text/javascript">		
				...
				?>
					<div id="form_locked_body">
						...
						<?php if( !empty($row) ){ ?>
							...
							<?php } else { ?>
							<p>Link Expired - Please go to <a href="portal/view.php?id=<?php echo $form_id; ?>" style="color: #000;">this page</a> to check latest data.</p>
						<?php } ?>
						</div>
					</div>
        ...
        <table id="ve_detail_table" width="100%" border="0" cellspacing="0" cellpadding="0">
          <tbody>
			...
			foreach ($entry_details as $data){ 
				...
				$edit_entry_url = "view.php?id={$form_id}&entry_id={$entry_id}";
				...
				if($data['element_type'] == 'section' || $data['element_type'] == 'textarea') {
					...
					if (isset($signature_id)) {
						$row_digital_signature = <<<EOT
							<tr>
								...
								<td>
									<div style="margin: 15px; padding: 20px; height: 150px; width: 250px; border: 1px dashed #8EACCF;">
										<img id="digital_signature_img" width="200" height="100" src="https://{$_SERVER['SERVER_NAME']}/portal/digital_signature_img.php?q={$signature_id}"/>
							...
							<script>
								var url = "https://{$_SERVER['SERVER_NAME']}/portal/digital_signature_img.php?q={$signature_id}"
			...
			if( $row_document_process['id'] ) {
				...
				if( $template_rows > 0 ) {
					...
					if( $row_document_process['status'] == 0 ) {
						echo "<td>Document is scheduled to be created.</td>";
				...
				?>
					<tr class="<?php echo $row_style; ?>">
						...
						<td>
							<a target="_blank" href="javascript:void(0)" class="action-download-document-zip" data-documentdownloadlink="download_document_zip.php?id=<?=$document_data['docxname']?>&form_id=<?=$_GET['form_id']?>&entry_id=<?=$entry_id?>&company_id=<?=$company_id?>" ><?php echo $document_data['docxname']; ?></a>
						</td>
						<td></td>
					</tr>
				<?php
			...
			<div id="ve_entry_navigation">
				<?php
					if(!is_null($previous_entry_id)) {
						?>
						<a href="<?php echo 'view_entry.php?form_id='.$form_id.'&entry_id='.$previous_entry_id; ?>" title="Previous Entry">
							<img src="images/navigation/005499/24x24/Back.png">
						</a>
						<?php
					} else {
						?>
						<a href="<?php echo 'view_entry.php?form_id='.$form_id.'&entry_id='.$entry_id; ?>" title="Previous Entry" style="visibility: hidden;">
							<img src="images/navigation/005499/24x24/Back.png">
						</a>
						<?php
					}
					...
					if(!is_null($next_entry_id)) {
						?>
						<a href="<?php echo 'view_entry.php?form_id='.$form_id.'&entry_id='.$next_entry_id; ?>" title="Next Entry" style="margin-left: 5px">
							<img src="images/navigation/005499/24x24/Forward.png">
						</a>
						<?php
					} else {
						?>
						<a href="<?php echo 'view_entry.php?form_id='.$form_id.'&entry_id='.$entry_id; ?>" title="Next Entry" style="visibility: hidden;">
							<img src="images/navigation/005499/24x24/Forward.png">
						</a>
						<?php
					}
			...
			<div id="ve_entry_actions">
				<ul>
					<li style="border-bottom: 1px dashed #8EACCF">
						<a id="ve_action_status" href="<?=$status_url?>"><img src="images/navigation/005499/16x16/Status.png"><?=$status_text?></a>
					</li>
					<li style="border-bottom: 1px dashed #8EACCF">
						<a id="ve_action_edit" href="<?php echo "view.php?id={$form_id}&entry_id={$entry_id}"; ?>"><img src="images/navigation/005499/16x16/Edit.png">Edit Entry Data</a> 
					</li>
					<li style="border-bottom: 1px dashed #8EACCF">
	...
	<input type="hidden" value="<?= $company_id ?>" id="status_company_id"/>
	<input type="hidden" value="<?= $form_id ?>" id="status_form_id"/>
...
$(document).ready(function(e) {
	...
	$('#ve_action_pdf').click(function(){
		...
		$.ajax({
			type: "POST",
			async: true,
			url: "/auditprotocol/generate_entries_pdf.php",
			data: {
