		...
		if(empty($_SESSION['filter_users'])){
			if($action == 'delete'){
				if(LA_CONF_TRUE_DELETE == true){
					//empty the table
					$query = "delete from `".LA_TABLE_PREFIX."users` where `user_id` <> 1";
					$params = array();
					la_do_query($query,$params,$dbh);

					//empty the table
					$query = "delete from `".LA_TABLE_PREFIX."permissions` where `user_id` <> 1";
					$params = array();
					la_do_query($query,$params,$dbh);
				}else{
					//simply set the status of the record
					/*$query = "update `".LA_TABLE_PREFIX."users` set `status`=? where `user_id` <> 1";
				...
				//simply set the status of the record
				$query = "update `".LA_TABLE_PREFIX."users` set `status`=? where `status`=1 and `user_id` <> 1";
				$params = array($new_user_status);
				la_do_query($query,$params,$dbh);
			}
		}else{ //if there is filter enabled
			//get the entry_id of all rows within the filter
			$target_user_id_array = la_get_filtered_users_ids($dbh,$_SESSION['filter_users']);
			
			//delete or suspend them
			if(!empty($target_user_id_array)){
				
				if($action == 'delete'){
					$_SESSION['LA_SUCCESS'] = 'Selected users has been deleted.';
				}else if($action == 'suspend'){
					$_SESSION['LA_SUCCESS'] = 'Selected users has been suspended.';
				}
