 ...
 ********************************************************************************/
	...
	if( isset($params['page_from']) && $params['page_from'] == 'view_entry' ) {
		$company_id = $params['company_id'];
		$query 	= "select data_value from `".LA_TABLE_PREFIX."form_{$form_id}` where field_name='{$field_name}' and company_id = {$company_id} order by id desc";
		$params = array();
	} else {
		$id      	= (int) $params['id'];
		$id1 = $id -1;
		$query 	= "select data_value from `".LA_TABLE_PREFIX."form_{$form_id}` where field_name='{$field_name}' GROUP BY company_id order by id limit {$id1},1";
		$params = array($id);
	}
