	...
	if(!empty($delete_all)){ //this is delete all entries operation
		...
		if(empty($entries_enable_filter)){
			if(LA_CONF_TRUE_DELETE == true){
				...
				if(!empty($incomplete_entries)){
					$query = "delete from `".LA_TABLE_PREFIX."form_{$form_id}` where `status` = 2";
				}else{
					$query = "delete from `".LA_TABLE_PREFIX."form_{$form_id}` where `status` <> 2";
				}
				...
				if(empty($incomplete_entries)){
					$query = "delete from ".LA_TABLE_PREFIX."form_payments where form_id = ?";
				...
				$query = "select element_id from ".LA_TABLE_PREFIX."form_elements where element_type='file' and form_id=?";
				$params = array($form_id);
				...
				if(!empty($file_element_id_array)){
					foreach ($file_element_id_array as $element_id){
						$file_uploads = array();
						$file_uploads = glob($la_settings['upload_dir']."/form_{$form_id}/files/element_{$element_id}_*");
				...
				if(!empty($incomplete_entries)){
					$query = "update ".LA_TABLE_PREFIX."form_{$form_id} set `status`=0 where `status`=2";
				}else{
					$query = "update ".LA_TABLE_PREFIX."form_{$form_id} set `status`=0 where `status`=1";
				}
				...
				if(empty($incomplete_entries)){
					$query = "update ".LA_TABLE_PREFIX."form_payments set `status`=0 where form_id = ?";
					$params = array($form_id);
					la_do_query($query,$params,$dbh);
				}
			}
			...
			$target_entry_id_array = la_get_filtered_entries_ids($dbh,$form_id,$entries_options);
			...
			if(!empty($target_entry_id_array)){
				...
				if(LA_CONF_TRUE_DELETE == true){
					...
					$query = "delete from `".LA_TABLE_PREFIX."form_{$form_id}` where `id` in('{$target_entry_id_joined}')";
					$params = array();
					la_do_query($query,$params,$dbh);

					//delete records from ap_form_payments table
					$query = "delete from `".LA_TABLE_PREFIX."form_payments` where form_id = ? and `record_id` in('{$target_entry_id_joined}')";
					$params = array($form_id);
					la_do_query($query,$params,$dbh);
					...
					if(!empty($file_element_id_array)){
						foreach ($target_entry_id_array as $entry_id){
							foreach ($file_element_id_array as $element_id){
								$file_uploads = array();
								$file_uploads = glob($la_settings['upload_dir']."/form_{$form_id}/files/element_{$element_id}_*-{$entry_id}-*");
					...
					$query = "update `".LA_TABLE_PREFIX."form_{$form_id}` set `status`=0 where `id` in('{$target_entry_id_joined}')";
					$params = array();
					la_do_query($query,$params,$dbh);

					//set the status of the record on ap_form_payments as well
					$query = "update `".LA_TABLE_PREFIX."form_payments` set `status`=0 where form_id = ? and `record_id` in('{$target_entry_id_joined}')";
					$params = array($form_id);
					la_do_query($query,$params,$dbh);
				}
			}
		...
		if(!empty($selected_entries)){
			...
			if(!empty($target_entry_id_array)){
				...
				if(!empty($origin) && ($origin == 'view_entry')){
					...
					$_SESSION['LA_SUCCESS'] = "Entry #{$target_entry_id_array[0]} has been deleted.";
					...
					$_SESSION['LA_SUCCESS'] = 'Selected entries has been deleted.';
				}
