 ...
 ********************************************************************************/
	...
	//If this is new form, create the form table and form folder+css
   	if($is_new_form){
		...
		addUserActivity($dbh, $_SESSION['la_client_user_id'], $form_id, 1, "Session Time {$session_time}", time());
	   ...
	   la_do_query($query,array(),$dbh);*/
		...
		$query_com = "TRUNCATE TABLE ".LA_TABLE_PREFIX."form_{$form_id}";
	...
	$query = "SELECT element_id, element_type, element_constraint, element_position, element_matrix_parent_id, element_matrix_allow_multiselect, element_choice_has_other, element_status FROM ".LA_TABLE_PREFIX."form_elements WHERE form_id = :form_id and element_type = 'matrix' ORDER BY element_position asc";
	...
	if(count($element_properties_array)){
			...
			if(in_array($element_type,array('radio','checkbox','select'))){								
				...
				foreach($element_options as $option_id => $value){
					if(empty($value['is_db_live'])){ //this is new choice
						if(!$insertFlag){
							$insertFlag = true;
							array_push($insertOptionQuery, "INSERT INTO `".LA_TABLE_PREFIX."element_options` (`form_id`,`element_id`,`option_id`,`position`,`option`,`option_value`,`option_is_default`,`live`) VALUES (?,?,?,?,?,?,?,'1')");
						}else{
							array_push($insertOptionQuery, "(?,?,?,?,?,?,?,'1')");
						}
						
						$value['option_value'] = (!empty($value['option_value']) ? $value['option_value'] : 0); 
						$optionQueriesParams = array_merge($optionQueriesParams, array($form_id,$element_id,$option_id,$value['position'],$value['option'],$value['option_value'],$value['is_default']));
					}					
				}
				
				if(count($insertOptionQuery)){
					array_push($optionQueriesArr, implode(",", $insertOptionQuery));
				}
				
				foreach($element_options as $option_id => $value){
					if(!empty($value['is_db_live'])){ //this is new choice
						array_push($optionQueriesArr, "UPDATE `".LA_TABLE_PREFIX."element_options` SET `live`=1,`option` = ?,`option_value` = ?,`option_is_default` = ?,`position` = ? WHERE form_id = ? and element_id = ? and `option_id` = ?");
						$optionQueriesParams = array_merge($optionQueriesParams, array($value['option'],$value['option_value'],$value['is_default'],$value['position'],$form_id,$element_id,$option_id));
					}					
				}
								
				if(count($optionQueriesArr)){
				...
				foreach($element_options as $m_element_id => $value){
					...
					if(empty($value['is_db_live'])){ //this is new row
						...
						if(!isset($value['column_data'])){
							...
							if(count($insertOptionQuery)){
								//print_r($insertOptionQuery);
								//print_r($insertOptionParam);
								array_push($optionQueriesArr, implode(",", $insertOptionQuery));
							}
				...
				if(count($optionQueriesArr)){
					/*echo implode(";", $optionQueriesArr);
					echo "\n------------------------------------\n";
					print_r($optionQueriesArr);
					echo "\n------------------------------------\n";
					print_r($optionQueriesParams);
					echo "\n------------------------------------\n";
	...
	//print_r($positionQueriesArr);
	//print_r($positionParams);
	
	if(count($positionQueriesArr)){
	...
	if(!empty($last_pagebreak_properties)){
		...
		array_push($queriesList, "UPDATE ".LA_TABLE_PREFIX."forms SET form_page_total=?,form_lastpage_title=?,form_submit_primary_text=?, form_submit_secondary_text=?,form_submit_primary_img=?, form_submit_secondary_img=?,form_submit_use_image=? WHERE form_id=?");
		$queriesListParam = array_merge($queriesListParam, array($total_page,$last_pagebreak_properties['page_title'],$last_pagebreak_properties['submit_primary_text'], $last_pagebreak_properties['submit_secondary_text'],$last_pagebreak_properties['submit_primary_img'], $last_pagebreak_properties['submit_secondary_img'],$last_pagebreak_properties['submit_use_image'], $form_id));
