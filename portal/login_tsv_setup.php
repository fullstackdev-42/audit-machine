 ...
 ********************************************************************************/
	...
	$totp_data  = "otpauth://totp/IT Audit Machine:{$_SESSION['email']}?secret={$tsv_secret}";
	...
	if(!empty($_POST['submit'])){
		...
		if($tsv_result === true){
				...
				$_SESSION['la_client_client_id'] = $_SESSION['la_client_entity_id'];
				//===============================================

				$query  = "UPDATE ".LA_TABLE_PREFIX."ask_client_users set tsv_code_log=?, tsv_secret=?, tsv_enable=1, how_user_registered=? WHERE `client_user_id`=?";
				$params = array($tsv_code,$tsv_secret,$how_user_registered,$user_id);
