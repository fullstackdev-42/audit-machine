...
$(function(){
	...
	$(".la_link_disable a").click(function(){
		var selected_form_li_id = $(this).parent().parent().attr('id');
		...
		if(current_action == 'disable'){
			...
			$.ajax({
				   ...
				   error: function(xhr,text_status,e){
					  ...
					  $("#" + selected_form_li_id + " .la_link_disable a").text(current_action);
					  $("#" + selected_form_li_id + " .la_link_disable img").remove();
				   },
				   success: function(response_data){
					   ...
					   if(response_data.status == 'ok'){
						   if(response_data.action == 'disable'){
							   ...
							   $("#liform_" + response_data.form_id + " .la_link_disable a").html('<span class="icon-play"></span>Enable');
							   $("#liform_" + response_data.form_id + " .la_link_disable img").remove();
						   }else{
							   $("#liform_" + response_data.form_id).removeClass('form_inactive');
							   $("#liform_" + response_data.form_id + " .la_link_disable a").html('<span class="icon-pause"></span>Disable');
							   $("#liform_" + response_data.form_id + " .la_link_disable img").remove();
						   }
					   }else{
	...
	$("#filter_form_input").bind('focusin click',function(){
		...
		if($("#filter_form_input").val() == 'find form...'){
			...
			$("#la_search_box,#filter_form_input").animate({'width': '+=165px'},{duration:200,queue:false});
			
			$("#la_search_box,#filter_form_input").promise().done(function() {
				$("#la_search_title,#la_search_tag").slideDown('medium');
				
				$("#la_search_title,#la_search_tag").promise().done(function(){
					$("#la_search_box").addClass('search_focused');
					$("#la_search_box,#filter_form_input").removeAttr('style');
				});
			});	
		}
	...
	$("#la_search_title").click(function(){
		$(this).addClass('la_pane_selected');
		$("#la_search_title a").html('&#8674; form title');
		
		$("#la_search_tag a").html('form tags');
	...
	$("#filter_form_input").keyup(function(){
		...
		if(search_term != ''){
			//first hide all form
			$("#la_form_list > li").removeClass('result_set').hide();
			
			//hide pagination
			$("#la_pagination").hide();
			
			if($("#la_search_title").hasClass('la_pane_selected')){ //search on form title
				var result_h3 = $("#la_form_list h3:contains('"+ search_term + "')");
	...
	$("#la_filter_reset").click(function(){
		...
		$("#la_search_title,#la_search_tag").hide();
	...
	$('#la_form_list').delegate('a.removetag', 'click', function(e) {
	...
	$(".la_link_duplicate a").click(function(){
		var selected_form_li_id = $(this).parent().parent().attr('id');
		...
		$.ajax({
			   ...
			   error: function(xhr,text_status,e){
				  //restore the links upon error
				  $("#" + selected_form_li_id + " .la_link_duplicate a").text('Duplicate');
				  $("#" + selected_form_li_id + " .la_link_duplicate img").remove();
	...
	$(".la_link_delete a").click(function(){
