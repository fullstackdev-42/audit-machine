$(function(){
	...
	$(document).on('click', '#exit_form_view', function(){
		window.location.href = "/portal/client_account.php";
	});
	...
	//start::show document preview
	$("#document-processing-dialog").dialog({
		modal: true,
		autoOpen: false,
		closeOnEscape: false,
		width: 400,
	...
	$(document).on('click', '.entry-link-preview', function(e){
		...
		if( identifier == 'image_format' ) {
			...
			$('#document-processing-dialog').dialog('open');
			...
			$.ajax({
				...
				error: function(xhr,text_status,e){
					...
					$('#document-processing-dialog').dialog('close');
	...
	$(document).on("click", "#go-to-target-element", function(){
		...
		if(field_link != "") {
			if(current_link.indexOf("view.php") > -1) {
				...
				forward_link = current_link + "?id=" + field_link + "&entry_id=" + entry_id;
