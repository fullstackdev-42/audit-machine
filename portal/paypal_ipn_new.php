 ...
 ********************************************************************************/
	...
	$log_error 	= false;
	//$log_file_path = $la_settings['upload_dir']."/ipn_error_log.php";
	$log_file_path = $_SERVER['DOCUMENT_ROOT']."/portal/ipn_error_log.php";
	if(file_exists($log_file_path) !== true){
		if(is_writable($la_settings['upload_dir'])){
	...
	if(!empty($form_id)){
		$form_properties = la_get_form_properties($dbh,$form_id,array('payment_paypal_enable_test_mode'));
	}
	...
	if($verified === true){
		...
		if(!empty($error_message)){
			...
			$query = "select count(afp_id) record_exist from ".LA_TABLE_PREFIX."form_payments where `form_id` = ? and `status` = 1";
			$params = array($form_id);
			$sth = la_do_query($query,$params,$dbh);
			$row = la_do_fetch_result($sth);
			
			
			$userEntities = getEntityIds($dbh, $company_id);
			...
			$query_chk = "select count(chk_id) record_exsis from `".LA_TABLE_PREFIX."form_payment_check` where `form_id` = ? and `company_id` IN ({$inQuery})";
			$params_chk = array_merge(array($form_id), $userEntities);
			$sth_chk = la_do_query($query_chk,$params_chk,$dbh);
			$row_chk = la_do_fetch_result($sth_chk);
			
			$payment_status = 'paid';
			
			if(!empty($row_chk['record_exsis'])){
				
				if($use_debug_mode){
					error_log('Updating form_payment_check table');
				}
				
				$query = "UPDATE `".LA_TABLE_PREFIX."form_payment_check` SET `payment_date` = ?, `form_counter` = ? WHERE `form_id` = ? AND `company_id` IN ({$inQuery})";
				/*$params = array();
				$params[':form_id'] = $form_id;
				$params[':company_id'] = $company_id;
				$params[':payment_date'] = time();*/
				
				$form_counter = (int) "10000000";
				
				if(isset($_POST['subscr_id']) && !empty($_POST['subscr_id'])){
					$form_counter = (int) "10000000";
				}
				
				la_do_query($query,array_merge(array(time(), $form_counter, $form_id), $userEntities),$dbh);

				if($use_debug_mode){
					error_log('Done updating form_payment_check table');
				}
			}
			else{
				if($use_debug_mode){
					error_log('Inserting to form_payment_check table');
				}
				
				$query = "INSERT INTO ".LA_TABLE_PREFIX."ask_client_forms (`client_id`, `form_id`) VALUES (:company_id, :form_id)";
				
				$query2 = "INSERT INTO `".LA_TABLE_PREFIX."form_payment_check`(`chk_id`, `form_id`, `company_id`, `payment_date`, `form_counter`) VALUES (null, :form_id, :company_id, :payment_date, :form_counter)";
				
				if(count($userEntities)){
					foreach($userEntities as $v){
						$params = array();
						$params[':form_id'] = $form_id;
						$params[':company_id'] = $v;
						...
						if(isset($_POST['subscr_id']) && !empty($_POST['subscr_id'])){
							$params[':form_counter'] = (int) "10000000";
						}else{
							$params[':form_counter'] = (int) "10000000";
						}
						
						la_do_query($query2,$params,$dbh);
					}
				}
				...
				if($use_debug_mode){
					error_log('Done inserting form_payment_check table');
				}
			}
			...
			if(!empty($row['record_exist'])){
				...
				$query = "UPDATE `".LA_TABLE_PREFIX."form_payments` SET `payment_status` = :payment_status, `payment_id` = :payment_id, `payment_date` = :payment_date, `payment_amount` = :payment_amount WHERE `form_id` = :form_id and `status` = 1";
				...
				$query = "INSERT INTO `".LA_TABLE_PREFIX."form_payments`(
										`form_id`, 
										`company_id`, 
										`record_id`, 
										`payment_id`, 
				...
				if(!empty($payment_delay_notifications)){
					...
					//la_process_delayed_notifications($dbh,$form_id,$entry_id);
				}
			}
