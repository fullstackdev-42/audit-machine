 ...
 ********************************************************************************/
	...
	while($row = la_do_fetch_result($sth)){
		...
		$sub_query = "select count(*) today_entry from `".LA_TABLE_PREFIX."form_{$row['form_id']}` where `status`=1 and date_created >= date_format(curdate(),'%Y-%m-%d 00:00:00') ";
		...
		if($form_sort_by == 'total_entries'){
			$sub_query = "select count(*) total_entry from `".LA_TABLE_PREFIX."form_{$row['form_id']}` where `status`=1";
			$sub_sth = la_do_query($sub_query,array(),$dbh);
			$sub_row = la_do_fetch_result($sub_sth);
				...
				<?php if(!empty($form_list_array)){ ?>	
					...
					?>
						...
						<li data-theme_id="<?php echo $theme_id; ?>" id="liform_<?php echo $form_id; ?>" <?php echo $form_class_tag; ?>>
							...
							<div class="form_option la_link_delete">
							...
							<div class="form_option la_link_duplicate">
							...
							<div class="form_option la_link_group">
								<?php if(!empty($_SESSION['la_user_privileges']['priv_administer']) || !empty($user_permissions[$form_id]['edit_form'])){ ?>
								<a class="la_link_theme" href="#"><span class="icon-palette2"></span>Theme</a>
								<a class="la_link_emails" href="notification_settings.php?id=<?php echo $form_id; ?>"><span class="icon-envelope-opened"></span>Notifications</a>
								<a class="la_link_code" href="embed_code.php?id=<?php echo $form_id; ?>"><span class="icon-paste"></span>Code</a>
								<a class="la_link_payment" href="payment_settings.php?id=<?php echo $form_id; ?>"><span class="icon-cart"></span>Payment</a>
								<a class="la_link_logic" href="logic_settings.php?id=<?php echo $form_id; ?>"><span class="icon-shuffle"></span>Logic</a>
								<a class="la_link_report" href="manage_report.php?id=<?php echo $form_id; ?>"><span class="icon-chart"></span>Report</a>
								<?php } ?>

								<a class="la_link_view" target="_blank" href="view.php?id=<?php echo $form_id; ?>"><span class="icon-search"></span>View</a>
							</div>
							
							<?php if(!empty($_SESSION['la_user_privileges']['priv_administer']) || !empty($user_permissions[$form_id]['edit_form'])){ ?>
							<div class="form_option la_link_disable">
								<?php 
									if(empty($form_data['form_active'])){
